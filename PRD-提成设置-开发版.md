...
**方案改动重算弹窗（§6.13，二十六次新增 / 二十八次改版）**：改「{方案名}」后，提成怎么算？、本期已经算好的提成，要重新算吗？、已经手工改过的订单，金额不受影响。、本期不动，下期换、立即重新计算、从现在起用新方案；**未回答即退出时的兜底 toast**：已按「从现在起用新方案」保存；列表页方案卡角标：**下期生效**。**该弹窗全篇不得出现「结算周期 / 生效口径 / 生效方式 / 落地 / 重算引擎 / 快照 / 回溯 / 计提」等术语**，也不得出现公式；**二十八次起也不再展示任何影响面信息**（无「N 名员工在用」、无「本期 {起}–{止}」、无「已算好 M 条」）。
...
56o2. **Given** 方案 S 已分配 ≥1 名员工，**When** 改动**影响金额**的任一字段（提成参数 / 适用范围 / 计算基数 / 分配模式 / 工位值 / 规则项）后点「保存」，**Then** **不立即落库**，先弹 `#comm2RecalcMask`（居中 Dialog、`max-width 326px`、`max-height min(560px,78vh)`、正文可滚动、`z-index 50`）：标题「改「{方案名}」后，提成怎么算？」，**其下只有一个开关行** —— 左问句「本期已经算好的提成，要重新算吗？」+ 右侧 `44×26` iOS 开关（**默认关闭**、`#E5E5E5` 底、`22px` 白滑块），再下方一行带 ⓘ 图标的灰字提示「已经手工改过的订单，金额不受影响。」，底部两枚按钮（左「本期不动，下期换」为描边次要样式、右「从现在起用新方案」为品牌主按钮）。**And** 弹窗内**不得出现**任何影响面信息 —— 无「**{N}** 名员工在用」说明行、无灰条、无「本期 {起}–{止} · 已算好 {M} 条」、**无三张单选卡**（`#comm2RecalcLead` / `#comm2RecalcImpact` / `#comm2RecalcOpts` / `#comm2RecalcOk` 四个节点在 DOM 中均不存在）。**And** 两枚按钮并排一行、主按钮在右、**文案均不溢出也不折行**（14px / `padding 0 8px`，实测 `scrollWidth - clientWidth = 0`）。**And** 弹窗**无取消按钮**；**When** 点遮罩空白或按 Esc，**Then** **不关闭**（必须二选一）。**And** 全篇**不得出现**「结算周期 / 生效口径 / 落地 / 重算引擎 / 快照 / 回溯」等术语与任何公式。
