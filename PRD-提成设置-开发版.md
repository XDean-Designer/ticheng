...
> 依赖：本文只用到员工薪资侧的 3 个**只读**接口（`getPeriodInfo` / `getCurrentPeriodInfo` / `nextPeriodInfo`；二十八次起 `countEffectiveCommLines` 已随影响面展示删除），结算周期本身的定义见**源 PRD §6.12.3**。
...
| **本期先不动，下期再用新方案** | `next` | 本期马上要发薪、不想再变 | 保持原样（老规则） | **老规则** | **新规则** |

**切分依据**：`forward` 按**行的日期**（`ymd`）与 `effectiveAt`（= 保存当天）比较 —— 同一期内**会切分**（改前的老规则、改后的新规则各算各的）；`next` 按**期**比较（该期结束日 < `effectiveFrom` 期结束日 → 该期整期走老规则，不切分）；`recalc` **不切分**，整期新规则。

**三种口径 ↔ 弹窗控件的对应（二十八次）**：`recalc` / `forward` 由**开关 + 右按钮**产生（开关开 = `recalc`，开关关 = `forward`），`next` 由**左按钮**独立产生。三者是**数据侧的三种口径**，弹窗只是把其中两种归到同一个按钮上、用开关区分范围 —— 数据模型与解析逻辑不因 UI 改版而改变。
...
**二十八次精简**：弹窗**不再展示任何影响面信息** —— 原「这个方案有 N 名员工在用…」说明行、原灰条「本期 9.01–9.30 · 已算好 M 条」**整段删除**（连带删除 `countEffectiveCommLines()` 这个只为统计条数而存在的接口）。用户看到的只有：**标题 + 一个开关 + 一句提示 + 两个按钮**。

| 区块 | 文案（须逐字一致） |
|------|--------------------|
| 标题（**固定**，不引用方案名） | 改方案后，提成怎么计算？ |
| 开关行问句（左） | 本期已经算好的提成，要重新算吗？ |
| 开关（右） | iOS 开关：**关 = 否**（默认）· **开 = 是** |
| 提示（左侧带 ⓘ 图标） | 已经手工改过的订单，金额不受影响。 |
| 左按钮（描边次要） | 本期不动，下期换 |
| 右按钮（品牌主按钮，**文案随开关变化**） | 开 = **立即重新计算** ／ 关 = **从现在起用新方案** |

**开关语义**：就是「本期已经算好的提成要不要重算」这一个问题 ——
- 开关**开（是）** → 右按钮 = **`recalc`**：本期已算出来的按新规则重算一遍，金额会变
- 开关**关（否，默认）** → 右按钮 = **`forward`**：之前算出来的保持原样，从现在开始的新单子按新规则算

**右按钮文案必须随开关变化**：关闭态下并没有发生「重新计算」，若仍写「立即重新计算」会直接误导用户 —— 故关闭态显示「从现在起用新方案」。这是本弹窗唯一的一处文案联动，`recalcSyncUi()` 负责同步文案与 `aria-checked`。

**两个按钮都是决定性动作**：因此**必须点一个才能完成保存** —— **无取消按钮**，**点遮罩空白与按 Esc 均不关闭**（全仓无通用 outside-close 机制，本弹窗不接线即天然成立）。

**开关样式自带**：`.comm2-recalc__switch`（`44×26` 胶囊 / `22px` 白色滑块 / `box-shadow 0 1px 2px rgba(0,0,0,.16)` / 关 `#E5E5E5` → 开 `var(--brand)` / 滑块 `translateX(18px)` / `.18s ease`），定义在 `comm2.css` 内，**不跨模块引用 `salary.css` 的 `.switch`**。

**排版细节**：两个按钮并排（`gap: 8px`）、次要按钮在左、主按钮在右；因文案较长，本弹窗内的按钮收到 `font-size: 14px` + `padding: 0 8px`（`.dialog--recalc .dialog__actions .btn-main`），实测两枚按钮文案**均不溢出、不折行**（`scrollWidth - clientWidth = 0`）。

### 6.13.4 数据落地与解析

**§7.1 新增 4 个字段**（`effectiveMode` / `effectiveAt` / `effectiveFrom` / `_prev`），落库时机 = 用户在弹窗点「确定」时（`applyEffectiveChoice()`）：

| 口径 | `effectiveMode` | `effectiveAt` | `effectiveFrom` | `_prev` |
|------|-----------------|---------------|-----------------|---------|
| `recalc` | `'recalc'` | 保存当天 `YYYY-MM-DD` | `''` | 改动前的规则集快照 |
| `forward` | `'forward'` | 保存当天 | `''` | 同上 |
| `next` | `'next'` | `''` | **下一期 key**（`cmNextPeriod(本期).key`） | 同上 |
...
schForLine(sch, periodKey, ymd):
  ...
  useOld =
    (effectiveMode === 'next')  ? 本期结束日 < effectiveFrom 期结束日
                                : String(ymd).slice(0,10) < effectiveAt
...
| `Comm2Demo` 暴露 | `schForLine` 内部使用；`EmployeeDemo` 提供 `getPeriodInfo(key)` / `nextPeriodInfo(key)` / `getCurrentPeriodInfo()`（二十八次：`countEffectiveCommLines()` 随影响面展示一并删除） |
| 薪资侧重算 | `notifySalarySync()` → `invalidateCommLineCache()`（**既有能力**，无新增代码） |

### 6.13.5 列表页「下期生效」角标

`effectiveMode === 'next'` 且 `effectiveFrom` 非空时，方案卡**名称右侧**渲染 `<span class="comm2-scheme__badge">下期生效</span>`（`#FFF3E8` 底 / `#C2701A` 字 / `11px` / `18px` 高 / `4px` 圆角 / `flex-shrink: 0`）。目的：本期仍按老规则跑，**必须让用户看见「改动存下了、下期才生效」**，否则会误以为保存失败。
...
| 用途 | 取值 |
|------|------|
| `forward` / `recalc` 的分界日 `effectiveAt` | `cmToday()`（= 保存当天） |
| `next` 的生效期 `effectiveFrom` | `cmNextPeriod(cmCurrentPeriod().key).key` —— 即「**含今天那一期**的下一期」 |

即：一律走 `cmCurrentPeriod()` / `cmNextPeriod()`（内部转调 `EmployeeDemo.getCurrentPeriodInfo()` / `nextPeriodInfo()`），**不得**读 `state.salaryMonth`（那是用户正在查看的期，可能被切到过去）。若哪天要在弹窗里重新展示日期，也必须从这两个来源取。
...
| `next` 再次改动 | 覆盖 `effectiveFrom` 为新的下一期，`_prev` 更新为**本次改动前**（即上一次「新规则」）的快照 |
...
| effectiveMode | `'recalc'` \| `'forward'` \| `'next'` | **二十六次新增**：本方案上次被改动时用户选定的「本期提成怎么算」口径。缺省 / 历史方案按 `'forward'` 处理（**有 `_prev` 才参与解析**，无 `_prev` 时口径不影响结果） |
| effectiveAt | string | **二十六次新增**：`forward` / `recalc` 的分界日 `YYYY-MM-DD`（= 保存当天）；`next` 时为空串。`forward` 下**行日期 `ymd` 早于它**的行走老规则 |
| effectiveFrom | string | **二十六次新增**：`next` 的生效期 key（= 保存时的**下一期**，如 `2026-10`）；其余口径为空串。该期**结束日之前**的期整期走老规则 |
...
33. **方案改动后的本期重算口径（二十六次，详见 §6.13）**：**已分配员工**的方案被改动且改动**影响金额**时，保存前必须由用户选定本期怎么算 —— `recalc` 本期全按新方案重算 / `forward` 算好的不动、从现在起用新方案（默认）/ `next` 本期整期仍按老规则、下期起用新方案；**不得静默按默认口径落库**（弹窗无取消、遮罩与 Esc 均不关闭）。要点：
    - **触发**只认「影响金额」的改动（`defaults` / `overrides` / `stationIds` / `stationLabels` 的指纹变化）；**纯改名 / 改员工分配 / 无改动不弹**（新方案首次保存 `assigneeIds` 为空，也不弹）。
    - **「本期」= 含今天的那一期**（`getPeriodContaining(new Date())`），**不是**用户正在查看的期（月份 pill 可切 ±3 期）。弹窗里的区间与「下期」期名一律从 `getCurrentPeriodInfo()` / `nextPeriodInfo()` 取。
    - **人工修正优先**：所有口径下，已人工改过的提成行（`commManual` 或带 `commEditLogs`）**金额恒不变**（由 `invalidateCommLineCache()` 的既有保留/回填机制保证）。
    - **切分粒度**：`forward` **按行日期**切分（`ymd` vs `effectiveAt` = 保存当天），同一期内新老规则各算各的；`next` **按期**切分（该期整期一致，不切分）；`recalc` **不切分**。
...
**方案改动重算弹窗（§6.13，二十六次新增 / 二十八次改版）**：改方案后，提成怎么计算？、本期已经算好的提成，要重新算吗？、已经手工改过的订单，金额不受影响。、本期不动，下期换、立即重新计算、从现在起用新方案；**未回答即退出时的兜底 toast**：已按「从现在起用新方案」保存；列表页方案卡角标：**下期生效**。**该弹窗全篇不得出现「结算周期 / 生效口径 / 生效方式 / 落地 / 重算引擎 / 快照 / 回溯 / 计提」等术语**，也不得出现公式；**二十八次起也不再展示任何影响面信息**（无「N 名员工在用」、无「本期 {起}–{止}」、无「已算好 M 条」）。
...
56o2. **Given** 方案 S 已分配 ≥1 名员工，**When** 改动**影响金额**的任一字段（提成参数 / 适用范围 / 计算基数 / 分配模式 / 工位值 / 规则项）后点「保存」，**Then** **不立即落库**，先弹 `#comm2RecalcMask`（居中 Dialog、`max-width 326px`、`max-height min(560px,78vh)`、正文可滚动、`z-index 50`）：标题「改方案后，提成怎么计算？」，**其下只有一个开关行** —— 左问句「本期已经算好的提成，要重新算吗？」+ 右侧 `44×26` iOS 开关（**默认关闭**、`#E5E5E5` 底、`22px` 白滑块），再下方一行带 ⓘ 图标的灰字提示「已经手工改过的订单，金额不受影响。」，底部两枚按钮（左「本期不动，下期换」为描边次要样式、右「从现在起用新方案」为品牌主按钮）。**And** 弹窗内**不得出现**任何影响面信息 —— 无「**{N}** 名员工在用」说明行、无灰条、无「本期 {起}–{止} · 已算好 {M} 条」、**无三张单选卡**（`#comm2RecalcLead` / `#comm2RecalcImpact` / `#comm2RecalcOpts` / `#comm2RecalcOk` 四个节点在 DOM 中均不存在）。**And** 两枚按钮并排一行、主按钮在右、**文案均不溢出也不折行**（14px / `padding 0 8px`，实测 `scrollWidth - clientWidth = 0`）。**And** 弹窗**无取消按钮**；**When** 点遮罩空白或按 Esc，**Then** **不关闭**（必须二选一）。**And** 全篇**不得出现**「结算周期 / 生效口径 / 落地 / 重算引擎 / 快照 / 回溯」等术语与任何公式。
56o3. **Given** 重算弹窗已打开，**When** 拨开关，**Then** 开关 `.on` / `aria-checked` 与**右按钮文案同步**（开 → 「**立即重新计算**」，关 → 「**从现在起用新方案**」），弹窗**不关闭**；**And When** 点左按钮「本期不动，下期换」，**Then** 落 `next` 口径（`effectiveFrom` = **下一期** key、`effectiveAt` 空、`_prev` = 改动前规则集快照），弹窗关闭、落库、回列表，该方案卡名称右侧出现「**下期生效**」角标（`#FFF3E8` 底 / `#C2701A` 字 / `11px` / `18px` 高 / `4px` 圆角 / `flex-shrink: 0`）；**And When** 点右按钮，**Then** 落**开关所指**的口径 —— 开关开 → `recalc`（`effectiveAt` = 保存当天）、开关关 → `forward`（`effectiveAt` = 保存当天），两者 `effectiveFrom` 均为空、均无角标。「本期」一律取**含今天的那一期**（`getPeriodContaining(new Date())`），**不得**用正在查看的期（`state.salaryMonth`，月份 pill 可切 ±3 期）。
56o5. **Given** 弹窗默认态（开关关），**When** 直接点右按钮，**Then** 落 `forward`：本期已算好的提成**保持原样**，从现在起的新单子按新规则算；**And Given** 把开关拨到开再点右按钮，**Then** 落 `recalc`：本期已算出来的提成**按新规则重算一遍**（`_prev` 供 `schForLine` 判定）。两种情况下 `_prev` 均为**改动前**的规则集快照。
...
56o7. **Given** 重算弹窗已打开且**未做任何选择**，**When** 触发以下任一条退出路径 —— ① 关闭 / 刷新页面（`pagehide` / `beforeunload`）、② 点返回按钮、③ 任何回到列表的路径（`openList()`）—— **Then** 一律按**默认口径 `forward` 落库**：方案改动**被保存**（不丢）、`effectiveMode = 'forward'`、`effectiveAt` = 保存当天、`effectiveFrom` 为空、`_prev` = 改动前快照，且弹窗关闭、回到列表、**toast「已按「从现在起用新方案」保存」**。**And** 落库结果与「用户手动把开关留在关闭态后点右按钮」**完全等价**（不留任何「系统默认」标记字段）。**And** 点返回按钮时**不再弹「尚未保存」**（改动已保存，没有未保存内容）。**And** 兜底函数幂等：`openList()` → 兜底 → `commitSchemeSave()` → `openList()` 的重入**不得**造成二次落库或状态错乱。**And Given** 弹窗内两个按钮各自的正常路径，**Then** 不会被兜底二次触发（`commitRecalcChoice()` 先清 pending 再落库）。
56o4. **Given** 方案 S 的 `_prev` 已写入（老口径 10% / 新口径 20%，两行各 1000 元，一行在改动日**之前**、一行在**之后**），**When** 计提，**Then** `recalc` = **400**（两行都用新规则、**不切分**）、`forward` = **300**（改前 100 + 改后 200、**按行日期 `ymd` 切分**）、`next` = **200**（本期整期老规则、**按期切分**）；**And Given** 该方案**无 `_prev`**（历史方案），**Then** 三口径结果一致、一律按新规则算（向后兼容）。**And Given** 本期已算好的行里有**人工改过**的（`commManual` 或带 `commEditLogs`），**When** 选「本期全部重算」并确定，**Then** 该行金额**保持人工值不变**，其余行按新规则重算（由 `invalidateCommLineCache()` 的保留 / 回填机制保证）。
