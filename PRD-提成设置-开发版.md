...
## 版本摘要（源 PRD 26 条修订的终态压缩）

> 源 PRD 的 26 条修订是**迭代流水**，开发只需**终态口径**。下表把每条压成一句话终态 + 影响面；要看完整流水请回源 PRD 文件头。
...
> 依赖：本文只用到员工薪资侧的 4 个**只读**接口（`getPeriodInfo` / `getCurrentPeriodInfo` / `nextPeriodInfo` / `countEffectiveCommLines`），结算周期本身的定义见**源 PRD §6.12.3**。
...
**形态**：居中 Dialog（`#comm2RecalcMask`），沿用 ModalPopup 规范 —— `max-width 326px`、`max-height min(560px, 78vh)`、正文区 `overflow-y: auto`、`z-index 50`（高于提成设置 Sheet 的 `30`）。**必须有明确选择才能继续**：**无取消按钮**，**点遮罩空白与按 Esc 均不关闭**（全仓无通用 outside-close 机制，本弹窗不接线即天然成立）。

| 区块 | 文案（须逐字一致） |
|------|--------------------|
| 标题（动态） | 改「**{方案名}**」后，提成怎么算？ |
| 说明（动态） | 这个方案有 **{N}** 名员工在用，改动会影响他们本期已经算出来的提成。 |
| 影响面（动态） | 本期 **{起}–{止}** · 已算好 **{M}** 条 |
| 选项 1 标题 | 本期全部重算 |
| 选项 1 说明（动态） | 本期（**{起}–{止}**）已经算出来的提成，按新规则重算一遍 —— 金额会变 |
| 选项 2 标题 | 算好的不动，从现在起用新方案 |
| 选项 2 说明 | 之前已经算出来的保持原样；从现在开始的新单子按新规则算 |
| 选项 3 标题 | 本期先不动，下期再用新方案 |
| 选项 3 说明（动态） | 本期（**{起}–{止}**）整期还按老规则算，本次发薪不受影响；**{下期名}**起用新规则 |
| 底部提示 | 已经手工调整过的单子，金额不受影响。 |
| 主按钮 | 确定 |

**动态占位符取法**：`{起}–{止}` = **含今天的那一期**（`EmployeeDemo.getCurrentPeriodInfo()`）的 `range`（`9.01–9.30` 形式）；`{下期名}` = `EmployeeDemo.nextPeriodInfo()` 的 `label`（自然月 → `2026年10月`；自定义 → `10期 · 9.26–10.25`）；`{M}` = `EmployeeDemo.countEffectiveCommLines(assigneeIds)`（**`status === 'effective'` 才算**，`pending` 待确认不计入）。**刻意注入真实日期**，避免「下期」「本期」这类抽象词让店家误解。

**选项卡片**：3 张竖排单选卡，交互与结算周期页 `.emp-pay-cycle-opt` 一致（`18×18` 圆环单选控件 + `15px` 标题 + `12px` 灰字说明、选中态 `var(--brand)` 填充 + 内嵌 `4px` 白点、卡间 `1px #F0F0F0` 分隔线）。**样式定义在 `comm2.css`（`.comm2-recalc__*`）内**，只借用视觉语言、不跨模块引用 `salary.css` —— 两个模块保持互不依赖。
...
| `Comm2Demo` 暴露 | `schForLine` 内部使用；`EmployeeDemo` 新增 `getPeriodInfo(key)` / `nextPeriodInfo(key)` / `getCurrentPeriodInfo()` / `countEffectiveCommLines(staffIds)` |
...
**「本期」= 含今天的那一期**（`getPeriodContaining(new Date())`），**不是**用户当前正在查看的期。两者可以不同：员工薪资列表的月份 pill 允许切 ±3 期（含过去），而改动重算问的**永远是当下这一期**。因此弹窗里的区间与「下期」期名都从 `getCurrentPeriodInfo()` / `nextPeriodInfo()` 取，**不得**读 `state.salaryMonth`。
...
| 员工被移出方案后再改该方案 | 只要 `assigneeIds` 非空仍弹；影响面按**当前** `assigneeIds` 统计 |
| 多方案「重叠取最高」 | 改一个方案可能让**原本另一方案胜出**的行换人胜出 —— 属于设计内的连带影响（规则 3），本期提示只讲「会影响本期已算好的提成」，不逐行列差异 |
...
**方案改动重算弹窗（§6.13，二十六次）**：改「{方案名}」后，提成怎么算？、这个方案有 {N} 名员工在用，改动会影响他们本期已经算出来的提成。、本期 {起}–{止}、已算好 {M} 条、本期全部重算、本期（{起}–{止}）已经算出来的提成，按新规则重算一遍 —— 金额会变、算好的不动，从现在起用新方案、之前已经算出来的保持原样；从现在开始的新单子按新规则算、本期先不动，下期再用新方案、本期（{起}–{止}）整期还按老规则算，本次发薪不受影响；{下期名}起用新规则、已经手工调整过的单子，金额不受影响。、确定；列表页方案卡角标：**下期生效**。**该弹窗全篇不得出现「结算周期 / 生效口径 / 生效方式 / 落地 / 重算引擎 / 快照 / 回溯 / 计提」等术语**，也不得出现公式。
...
56o2. **Given** 方案 S 已分配 ≥1 名员工，**When** 改动**影响金额**的任一字段（提成参数 / 适用范围 / 计算基数 / 分配模式 / 工位值 / 规则项）后点「保存」，**Then** **不立即落库**，先弹 `#comm2RecalcMask`（居中 Dialog、`max-width 326px`、`max-height min(560px,78vh)`、正文可滚动、`z-index 50`）：标题「改「{方案名}」后，提成怎么算？」，说明「这个方案有 {N} 名员工在用，改动会影响他们本期已经算出来的提成。」（`{N}` = `assigneeIds` 长度），影响面「本期 **{起}–{止}** · 已算好 **{M}** 条」（`{M}` = `countEffectiveCommLines(assigneeIds)`，仅 `status='effective'` 计入、`pending` 不计），3 张竖排单选卡，底部提示「已经手工调整过的单子，金额不受影响。」+ 单按钮「确定」。**And** 默认选中第 2 项「算好的不动，从现在起用新方案」（`forward`）；**And** 每张卡为 `18×18` 圆环单选控件 + `15px` 标题 + `12px` 灰字说明，同时只有一张 `on`、`aria-checked` 同步。**And** 弹窗**无取消按钮**；**When** 点遮罩空白或按 Esc，**Then** **不关闭**（必须选一项才能继续）。**And** 三个选项说明里的区间 / 期名**动态注入真实日期**，全篇**不得出现**「结算周期 / 生效口径 / 落地 / 重算引擎 / 快照 / 回溯」等术语与任何公式。
56o3. **Given** 重算弹窗已打开，**When** 选任一项并点「确定」，**Then** 口径写入方案（`recalc` / `forward` → `effectiveAt` = 保存当天、`effectiveFrom` 为空；`next` → `effectiveFrom` = **下一期** key、`effectiveAt` 为空；三者均写入改动前的规则集快照 `_prev` = `{defaults, overrides, stationIds, stationLabels}`），弹窗关闭、落库、回列表；**And Given** 选了第 3 项（下期生效），**Then** 该方案卡名称右侧出现「**下期生效**」角标（`#FFF3E8` 底 / `#C2701A` 字 / `11px` / `18px` 高 / `4px` 圆角 / `flex-shrink: 0`）；其余两项**无角标**。**And** 「本期」一律取**含今天的那一期**（`getPeriodContaining(new Date())`），**不得**用正在查看的期（`state.salaryMonth`，月份 pill 可切 ±3 期）。
