...
> **本文是《PRD-提成设置.md》的抽取版**，只覆盖 **「提成设置」（提成方案配置链路）** 与 **「选择服务员工」（开单侧采集链路）** 两条开发线。
> 「员工管理」「员工薪资」两条链路**不在本文范围**，涉及处一律改为一句指针（指向源 PRD 的对应章节）。
> 源 PRD 仍是三链路的**唯一全量来源**；两文冲突时以**源 PRD**为准。

| 字段 | 内容 |
|------|------|
| 交付物 | 本文 `PRD-提成设置-开发版.md` + 同目录 `PRD-提成设置-开发版.html`（阅读预览） |
| 上游全量来源 | 同目录 `PRD-提成设置.md`（三链路合一，4098 行） |
| 交互原型（唯一可视依据） | 同目录 `index.html`（+ `comm2.js` / `comm2.css`） |
| 画布 | 390 × 844 |
| 目的 | 供**负责提成设置 / 选择服务员工**的前端、后端、测试直接开发；只需本文 + 交互原型 |
| 抽取口径 | 正文基于源 PRD **逐字节搬运**（13 段区间，抽取时逐段校验一致）；**仅**做三类最小干预：① 无关章节替换为范围指针；② 剔除员工管理 / 薪资专属的验收行、权限表、文案段；③ **收敛指向已被剔除章节的交叉引用**（补「源 PRD」限定或改指 §6.10）。所有干预均在原处留下 **`【不在本文范围】` / 「剔除以外的…」** 标注，可逐条回查 |
| 章节编号 | **沿用源 PRD 编号**（§4.5.1 / §6.3 / §10.2.1 / A2 / N19 …）—— 跳号属正常，对照表见下 |
| 现状核对 | **未编辑的保留区间**与源 PRD 逐字一致（抽取时以脚本校验 13 段）；范围外章节与专属行已剔除，并在原处留下 `【不在本文范围】` / 「剔除以外的…」标注 |
| 维护 | **源 PRD 是主档**。源 PRD 更新后，本文需按「对照索引表」同步受影响的收录章节；范围外的更新**不进本文**。两文口径冲突时，先改源 PRD 再回填本文 |

---

## 版本摘要（源 PRD 28 条修订的终态压缩）

> 源 PRD 的 28 条修订是**迭代流水**，开发只需**终态口径**。下表把每条压成一句话终态 + 影响面；要看完整流水请回源 PRD 文件头。

| 日期 / 次序 | 终态口径（一句话） | 主要影响章节 |
|---|---|---|
| 09-07 | 金额一律两位小数；小数点及之后数字小一号（仅展示态） | §5.1 |
| 09-15 | 规则卡定稿卡头结构（图标 + 标题 + 「＞」；内容区左参数、右 120px 竖排两控件）；左竖条随卡片圆角裁切、不压卡头分隔线 | §6.2.1 · §6.2.4 |
| 09-16 一次 | 新增跨系统约束「开单选择服务员工」 | §4.5.1 |
| 09-16 二次 | 跨系统约束下钻为 **项 × 员工** 维度（每项 × 每候选员工按命中块 OR 判定） | §4.5.1 · §8 规则 7a |
| 09-16 三次 | **模型变更**：按工位与顾客指定由「正交互斥（3×2=6）」改为**独立叠加（3 工位 + 1 全局 = 4 值）**；顾客指定提升为 `CatRule` 级单值 | §7.2.1 · §6.3 |
| 09-16 四次 | 新增 §6.7.1「选择服务员工」页：复刻开单选人卡片网格 + Morph 展开动效 | §6.7.1 |
| 09-16 六次 | 选人交互动效四态定稿（按工位±顾客指定 × 不开 → 2/3/4 张卡 或 点选即勾选） | §6.7.1 |
| 09-16 七次 | 不分工位 + 开顾客指定时，下钮文案「散客」→「普通」（**已被十六次再次改名，见下**） | §6.7.1 |
| 09-16 八次 | **「点客」全局改名「顾客指定」**；字段 `guestSplit`/`designated*` → `extraSplit`/`extraValue`/`extraAmt`/`extraValueMode`（旧数据自动迁移）；Sheet 参数区改「3 工位横排一行 + 第 2 行顾客指定卡」；选人改「整行 N 张勾选卡」 | §7.2.1 · §6.3 · §6.7.1 |
| 09-16 九次 | 规则卡上的段标签改「顾客指定」；虚线卡文案「+ 顾客指定提成」；修复胶囊溢出（Sheet 参数卡改分行） | §6.2.3 · §6.3 |
| 09-16 十次 | Sheet 参数卡取值行布局定稿：**宽卡**值与胶囊同行、**窄卡**胶囊换到第 2 行右对齐且**不压缩**；PRD 内「额外」统一改「顾客指定」（**仅 Sheet 卡内标题**保留） | §6.3 · §6.4 |
| 09-16 十一 | ~~工位为必选项、未点工位不计入已选、再点已勾选工位播 iOS 抖动~~ **（十七次已整条废止）**；勾选控件动效（变红 → 画勾；取消反序） | §4.5.1 · §6.7.1 |
| 09-16 十二 | 勾选动效**提速一倍**（变红 60ms + 画勾 110ms = 170ms，抽为 CSS 变量、JS 同源）；收起/展开改为等**真实动画播完**；展开态点卡片区任意处都收起 | §12.2.1 · §12.3 A13 |
| 09-16 十三 | **新增 §12 交互动效规范**（26 项，10 字段制，可照抄 CSS/JS）；动效收进 PRD 作唯一规范来源 | §12 |
| 09-17 十四 | **反向（取消）动效提速一倍**：收勾 55ms + 褪红 30ms = 85ms（`--sp-uncheck-*` 恒为正向一半） | §12.2.1 · §12.3 A7 |
| 09-17 十五 | 修反向褪红 CSS 优先级 BUG（`.staff-opt.is-on .staff-opt__box.is-undraw` 反超）；整卡红色同节拍淡出；**换工位改并行「红色交接」**（≈170ms，非 255ms） | §12.3 A7 · A8 |
| 09-17 十六 | Sheet 卡内标题「额外」→「**指定**」；不分工位参数卡改**两行居左**（抖动 150px → 48px）；行标签「普通」→「**提成**」；**收起时机收紧**（点任一按钮即收起） | §6.2.3 · §6.3 |
| 09-17 十七 | **两组参数各自独立、可单选可叠加**；**「顾客指定提成」可脱离工位单独分配**（只发顾客指定提成）；废止「工位必选」；「普通」→「提成」；新增订单行 `basePicked` 显式标记 | §4.5.1 · §7.2.1 · §8 规则 7 · 7a · 8 |
| 09-17 十八 | 员工卡第三行摘要去掉灰字「无工位」（只留红字「顾客指定」）；「未选工位」信息只在**入口摘要行**保留 | §6.7.1 |
...
| 09-18 二十四 | **列表页帮助弹窗白话重写**：「多方案规则」→「**一个人挂几个方案，钱怎么算**」，5 段技术条目整组作废，改两节 **2 + 5 条**白话序号卡片（复用编辑页那批 `.comm2-hint-item*`，零新增 CSS）；「按项分摊 · 适用范围」（含基数公式）**整条删除**（改由 Sheet「计算基数」ⓘ 承担）；`aria-label` 同步改弹窗标题原文 | §3 · §6.1 · §6.9 · §9.1 · §10.2 |
| 09-19 二十五 | **按工位可多选**：工位追加勾选（废止红色交接）；金额 Σ；`staffRoles`/`stations` 数组；卡片 112px 两行摘要 | §4.5.1 · §6.7.1 · §7.4 · §8 · §12.3 A8 |
| 09-19 二十五补 | **摘要工位固定两行拆分**（3 工位 → 大工 · 中工 / 小工；+顾客指定最多 3 行）；卡高 128px；禁止省略号 | §6.7.1 · §10.2.1 |
| 09-20 二十六 | **方案改动后的本期重算口径**：已分配员工的方案被改动且**影响金额**时，保存前必须选定 `recalc` 本期全按新方案重算 / `forward` 算好的不动、从现在起用新方案（**默认**）/ `next` 本期不动、下期起用；新增方案字段 `effectiveMode` / `effectiveAt` / `effectiveFrom` / `_prev`；`schForLine()` 按期返回「套老规则的影子方案」→ 试算链路零改动；「本期」= **含今天的那一期**（非正在查看的期）；人工改过的提成行在所有口径下**恒不变**；列表页 `next` 加「**下期生效**」角标；弹窗**无取消**、遮罩与 Esc 均不关闭 | §6.13 · §7.1 · §8 规则 33 · §9.1 · §10.2 |
| 09-20 三十一 | **改动重算弹窗「三选一 → 两枚按钮即两个结果」**（领导拍板取消「本期不动，下期换」）：口径收敛为 `recalc` / `forward` 两种（**删 `next`**）；删除开关与两枚并排按钮，改**两枚竖排按钮** —— 上「本期全部重算」= `recalc`、下「从现在起用新方案」= `forward`（新增 `.dialog__actions--stack`）；连带清 `schForLine` 的 `next` 分支 / `cmNextPeriod()` / `schemePendingNext()` / 列表页「下期生效」角标 / `COMM2_EFFECTIVE_LABEL` / `effectiveFrom` 字段 / `EmployeeDemo.nextPeriodInfo()` / `recalcNowLabel()` / `recalcSyncUi()` / `recalcState.scope`；§7.1 字段 4 → **3 个**；抓取路由 `comm2-recalc-next` / `comm2-recalc-on` → **`comm2-recalc-all`** / **`comm2-recalc-forward`** | §6.13.2 · §6.13.3 · §6.13.4 · **§6.13.5（作废）** · §6.13.6 · §6.13.7 · §7.1 · §8 规则 33 · §9.1 · §10.2 |
| 09-20 三十二 | **改动重算弹窗提示行改水平居中**：`.comm2-recalc__note` 加 `justify-content: center` + `text-align: center`（图标 + 文字作为**整体**居中）；实测弹窗内容宽 `284px`、左右留白各 `30px`、偏离中心 `0px`、单行不折行 | §6.13.3 |
| 09-20 三十 | **重算弹窗标题改为固定文案**：`改「{方案名}」后，提成怎么算？` → **`改方案后，提成怎么计算？`**（不再引用方案名）；删除 `openRecalcDialog()` 里的动态标题赋值，标题由 `index.html` 静态承载 | §6.13.3 · §9.1 · §10.2 |
| 09-20 二十九 | **弹窗文案定稿 + 未回答即退出的兜底口径**：任何「未选择就消失」的路径（`pagehide`/`beforeunload`、返回按钮、任何回列表的 `openList()`）一律按默认 `forward` 落库（新增 `recalcFallbackForward()`，幂等；toast「已按「从现在起用新方案」保存」；不留 `_autoDefault`）；左按钮「本期不动，下期**再**换」→「**本期不动，下期换**」（9 字会折行、8 字单行）；提示「已经手工**调整过的单子**」→「**已经手工改过的订单**」；按钮字号保持 14px | §6.13.3 · **§6.13.8** · §6.13.7 · §9.1 · §10.2 |
| 09-20 二十八 | **重算弹窗改版：三选一 → 一个开关 + 两个按钮**；**删除全部影响面展示**（说明行 / 灰条 / 已算好条数）并移除 `countEffectiveCommLines()`；开关「本期已经算好的提成，要重新算吗？」开=`recalc`、关=`forward`（默认关），右按钮文案随开关联动（「立即重新计算」/「从现在起用新方案」），左按钮固定 `next`（「本期不动，下期再换」、描边次要、主按钮在右）；提示行左侧加灰 ⓘ 图标；开关样式自带于 `comm2.css` | §6.13.3 · §6.13.4 · §6.13.6 · §9.1 · §10.2 |
...
| 开发线 | 首屏 / 入口 | 本文核心章节 |
|---|---|---|
| **提成设置** | 工作台 hub「提成设置」→ 方案列表（`comm2-list`） | §1–§5（提成部分）· §6.1–6.6 · §6.8 · §6.9 · §6.10 · §7.1–7.6 · §8 规则 1–32 · §8.1 · §8.2 · §9 · §10.2 · §12.4 |
| **选择服务员工** | 开单页「选择服务员工」（`staff-pick`）→ 底部 Sheet | **§4.5.1** · **§6.7.1** · **§10.2.1** · **§12.3（含 §12.3.0）** |
| 两条线共用 | —— | §12.1 · §12.2 全局动效约定 · §12.6 D 组壳层基座 · §12.7–§12.11 · 附录 A 原型文件索引 |
...
| 2 | 用户场景与成功标准 | ✓ | 了解 | ✓ |
| 3 | 信息架构与页面清单 | ✓ | 了解 | ✓ |
| 4 | 状态机（含 §4.5.1 跨系统约束） | ✓ | **必看** | **必看** |
| 5 | 交互细则 / 异常流 | ✓ | 了解 | ✓ |
| 6 | 功能规格 | ✓ | 了解 | ✓ |
| 7 | 数据模型与接口约定 | ✓ | **必看** | ✓ |
| 8 | 业务规则（计提口径） | ✓ | **必看** | **必看** |
| 9 | 文案清单与权限 | ✓ | 部分 | ✓ |
| 10 | 验收 P0 + 造数场景 | 了解 | 了解 | **必看** |
| 11 | 非功能（极简） | ✓ | ✓ | ✓ |
| 12 | 交互动效规范（**唯一规范来源**） | **必看** | — | ✓ |
| 附录 A | 原型文件索引 | ✓ | — | ✓ |
...
本文**沿用源 PRD 编号**，因此跳号属正常。下表给出「本文收录了哪些章节 / 哪些被替换成指针」。

| 源 PRD 章节 | 本文 | 说明 |
|---|:--:|---|
| 文件头（标题 / 21 条修订） | **替换** | 换为本文文件头 + 「版本摘要」压缩表 |
| 怎么读这份 PRD | **替换** | 换为本文「导读」 |
| §1.1–§1.5 | 收录 | 背景 / 目标 / 本期做 / 本期不做 / 能力映射 |
| §2.1–§2.2 | 收录（**剔 1 行**） | 角色、核心场景；剔掉「薪资明细对齐」行 |
| §3.0 页面流 | 收录 | 页面流示意（含两条线） |
| §3.1 Hub 功能入口 | 收录 | 含「提成设置」入口 |
| ~~§3.2 / §3.2.0 员工管理~~ | **指针** | 链路二，不在本文范围 |
| ~~§3.3 / §3.3.0 员工薪资~~ | **指针** | 链路三，不在本文范围 |
| §3.4 三链路数据联动 | 收录 | 两条线的**最小依赖**（员工池 / 分配 / 权限） |
| §4.1–§4.7 | 收录 | 方案生命周期 / 脏状态 / 规则块 / 覆盖项 / 工位 / **§4.5.1 跨系统约束** / 分配状态 / 流水指标 |
| ~~§4.8 员工生命周期 / §4.9 薪资链路状态~~ | **指针** | 员工管理 / 薪资 |
| §5.1–§5.5 | 收录 | 校验 / Toast / 拦截 / 空态 / 覆盖左滑删除 |
| §6.1–§6.10 | 收录 | 方案列表 → 规则卡 → Sheet → 工位 → 添加规则项 → 分配员工 → **§6.7.1 选择服务员工** → 流水订单 → 帮助 → 关联页变更 |
| ~~§6.11 员工管理 / §6.12 员工薪资~~ | **指针** | 链路二 / 链路三 |
| §7.1–§7.6 | 收录 | 方案 / 规则块 / 分类规则 / 覆盖项 / 目标 / 流水订单字段 / 能力清单 / 时序 |
| §7.7 员工 · §7.8 敏感权限 | 收录 | **最小依赖**：员工池来源 + 权限 key |
| ~~§7.9 薪资周期 / §7.10 奖惩~~ | **指针** | 薪资链路 |
| §7.11 三链路同步时序 | 收录 | 两条线的**最小依赖** |
| §8 规则 1–24 / 26–32 | 收录（**剔 1 条**） | 计提口径；剔掉规则 25「头衔与角色分离」（员工管理） |
| §8.1 试算口径 · §8.2 演示数据 | 收录 | 原型试算引擎 + 3 个演示方案 |
| ~~§8.3 员工管理规则 / §8.4 员工薪资规则~~ | **指针** | 链路二 / 链路三 |
| §9.1 文案清单 | 收录（**剔 3 段**） | 剔掉「员工（表单/详情/列表）」「头衔/角色」「薪资」三段 |
| §9.2 权限 | 收录（**剔 2 表**） | 保留「提成设置链路」表 + 权限点；剔掉「员工管理链路」「员工薪资链路」表 |
| §10.1 造数 | 收录 | 空数据 / 有数据 |
| §10.2 业务验收 | 收录（**剔 5 行**） | 剔掉纯薪资页 UI 行（37 / 38 / 39）与员工详情行（65f） |
| §10.2.1 选择服务员工验收 | 收录 | **完整** |
| ~~§10.3 员工管理 P0 / §10.4 员工薪资 P0~~ | **指针** | 链路二 / 链路三 |
| §11 非功能 | 收录 | 画布 / 动效 / 性能口径 |
| §12.1 · §12.2 | 收录 | 动效字段定义 + 全局约定（时长阶梯 / 缓动 / token / 触觉 / 按压 / 降级 / z-index） |
| §12.3 A 组（含 §12.3.0） | 收录 | **选人 Sheet 全套动效**（§12.3.0 实现总览 1443 行 + A1–A13b） |
| §12.4 B 组 | 收录 | 提成设置动效 |
| ~~§12.5 C 组 员工管理 / 薪资~~ | **指针** | 链路二 / 链路三 |
| §12.6 D 组壳层基座 | 收录 | 页面切换 / Sheet / Toast / 抽屉（两条线共用） |
| §12.7–§12.11 | 收录 | 组合矩阵 / 降级 / 动效验收 N1–N26 / 自检清单 / 变更流程 |
| ~~模块 13 交付物（三链路版）~~ | **替换** | 换为本文模块 13 |
| —— | **新增** | **附录 A · 原型文件索引**（本文独有） |

**指针约定**：正文中形如 `> **【不在本文范围】** … 见源 PRD §x` 的引用块，表示该章节属员工管理 / 员工薪资链路，本文只保留**最小依赖**（员工池、`assigneeIds`、权限 key、展示口径），完整规格请查源 PRD。

### 先读这几条（贯穿全文）

1. 本期为**新版提成设置**，采用 **多方案** 模型：每个方案可整体分配给若干员工，方案内按「项目 / 产品 / 办卡 / 充卡 / **快捷开单**」**5 项默认** + N 条**覆盖规则项**配置提成。**每人可分配多个方案**；开单计提时对该员工所属全部方案按**订单行**分别计算，**计算项重叠时取提成金额更高者**（并列按方案列表顺序先出现者胜出，§8 规则 3）。不同行分别取高后可相加。
2. **提成计算不再依赖旧「业绩设置」**：旧功能本期**完全移除**，工作台不再提供入口；试点门店存量配置由**线下人工**处理，**本期不做数据迁移方案**。
3. **两条线的关系**：提成设置是**配置侧**（产出「工位 / 顾客指定」两套参数的开关与取值）；选择服务员工是**采集侧**（开单时按命中块决定「要不要选工位 / 要不要勾顾客指定」），二者由 §4.5.1 的**项 × 员工**维度约束连接。**改配置口径必然联动采集交互**，两处必须同批改。
4. **两组参数各自独立、可单选可叠加**（十七次定稿）：**「提成侧」**（按工位 = 大工 / 中工 / 小工 **可多选**，可点掉、可全不选；不分工位 = 单张「提成」）与 **「顾客指定侧」** 可任选其一或同时勾选；**勾了任一组即算已选**。**只勾「顾客指定」而未选工位/提成 → 提成侧折 0、只发顾客指定提成**（订单行落库 `basePicked=false`）—— **获得顾客指定提成不要求有工位提成**。
5. **金额展示**：一律两位小数；小数点及之后数字小一号（仅展示态）。
6. 每条提成规则的**适用范围**为支付方式多选：**现金 / 卡付 / 团购**；按订单行内**范围内实付金额分摊**计提（非整单命中 / 整单过滤）。经理签单（`sign=true`，实收 0）**不走三类适用范围**，计提见 §8 规则 32。
7. 原型工具栏「无数据 / 有数据」造数**仅演示**，不进正式产品。接口路径标 `【待联调确认】`，正式接口文档暂无交付时间；不臆造现网 URL。
8. 本期**仅单店**；**不做阶梯提成方案**。
9. **交互行为与文案以本目录 `index.html`（及 `comm2.js` / `comm2.css`）当前表现为准**；PRD 与原型冲突时以原型为准修正 PRD。
...
- **hub 链路二 · 员工管理**（**不在本文范围**；本文只锁**最小依赖**）：员工池（`EmployeeStore.staff[]`）为选人 Sheet 的候选来源与「分配员工」Sheet 的勾选对象；**敏感权限 key** 决定提成设置入口与流水提成编辑的拦截（`achCommSet` 等，见 §9.2）；员工表单里的「提成方案」写回方案 `assigneeIds`。完整规格见**源 PRD §6.11**。
- **hub 链路三 · 员工薪资**（**不在本文范围**；本文只锁「提成侧口径」）：员工薪资列表 / 员工薪资明细 / 结算周期（发薪日）/ 员工业绩提成明细 / 设奖惩 / 奖惩明细；其提成分列与支付四格口径必须与提成设置 5 类对齐（**§6.10.0**）—— 页面本体见**源 PRD §6.12**
- **三链路数据联动**：`EmployeeStore.staff` 为员工权威源；员工表单「提成方案」写回 `Comm2Demo` 方案 `assigneeIds`；薪资列表与提成分配共用员工池

## 1.4 本期不做（Out）

- 员工薪资核算页的完整规格（**属员工薪资链路，见源 PRD §6.12**；本文的 §6.10 只保留提成侧跨系统口径）
- 员工管理链路的完整规格（**见源 PRD §6.11**；本文只保留员工池 / 分配 / 权限 key 三处最小依赖）
...
> **剔除以外的场景（属员工管理 / 员工薪资链路）**：原 §2.2 的「薪资明细对齐」行 —— 薪资页的业绩 / 提成统计口径**已由本文 §6.10.0 通用口径覆盖**；页面本体见源 PRD §6.12。
...
> **【不在本文范围】§3.2 链路二 · 员工管理 / §3.3 链路三 · 员工薪资。**
> 本文两条线只**最小依赖**其数据：员工池（`EmployeeStore.staff[]`，§7.7）、方案分配（`assigneeIds`，§3.4）、敏感权限 key（`roleSensitivePerms`，§7.8）。
> 两链路的页面清单 / 页面流 / 路由建议见**源 PRD §3.2–§3.3**。
...
> **【不在本文范围】§4.8 员工生命周期 / §4.9 薪资链路状态。**
> 选人 Sheet 的候选员工来自「在岗」员工池，状态机本体见**源 PRD §4.8 / §4.9**。
...
| 弹窗 | 内容 |
|------|------|
| **一个人挂几个方案，钱怎么算**（列表页帮助，`#comm2HelpMask`，**二十四次白话重写**） | **白话条目版**，面向文化水平有限的小店主 / 店员，**全篇不得出现「取高 / 分摊 / 覆盖 / 分类默认 / 订单行 / 试算 / 计提 / 并列 / 命中 / 金额最大 / 写入 / 胜出 / 适用范围」等内部术语，也不得出现任何公式（`×` `÷`）**。分两节 **「一、谁能挂几个方案（2 条）」/「二、钱怎么算（5 条）」共 7 条**，复用 `.comm2-hint-list` / `.comm2-hint-item` **序号圆点卡片**（零新增 CSS），序号两节**各自从 1 起**，正文区可滚动。原 5 段技术条目已整组作废：原「按项分摊 · 适用范围」（含基数公式 / 卡付面值 / 计次折算）**整条删除** —— 改由 Sheet「计算基数」行右侧 ⓘ（`#comm2BaseHelpMask`）在配置现场承担；原第 1 段的实现细节（分配 Sheet 副文显示图标与方案名、「也在：」字面）**删除**。文案原文见下（可整段照抄） |
...
| **提成怎么设、钱怎么算** | **方案编辑页**标题右侧 ⓘ（`comm2EditHelpBtn`）打开（`comm2RuleHelpMask`）。**白话条目版**，面向文化水平有限的小店主 / 店员；与列表页「一个人挂几个方案，钱怎么算」**分工**：列表页讲**方案之间怎么比 / 方案内重复卡怎么比**，本弹窗讲**「这张卡怎么填 + 钱怎么算一遍」**（两者文风统一为白话序号卡片，条目层面允许重复，见 §6.9 附 2）。分两节：**「一、怎么设置（5 步）」/「二、钱怎么算出来（8 条）」**，条目渲染为**序号圆点卡片**（序号 18×18 圆形 + 加粗 13px 结论 + 12px `#929292` 补充），序号与分节标题**左对齐**，正文区可滚动。文案原文见下（可整段照抄） |
...
**§6.9 附 2 · 「一个人挂几个方案，钱怎么算」文案原文（原型 `#comm2HelpMask`，须逐字一致，二十四次白话重写）**
...
**与编辑页弹窗的关系**：两份弹窗**文风统一为白话序号卡片**，条目层面**允许重复**（第 3–5 条与编辑页「单独加的卡更优先 / 一个人挂了好几个方案怎么办 / 不同业务各算各的」同义）—— 两条 ⓘ 分挂列表页与编辑页，读者可能只从其中一个入口进来，**各自自洽优于强行去重**。二者分工仍成立：**列表页只讲方案之间怎么比、方案内重复卡怎么比**（不涉及某张卡怎么填），**编辑页讲「这张卡怎么填 + 钱怎么算一遍」**。

**样式契约**：与编辑页弹窗**完全同源**，复用同一批 `.comm2-hint-list` / `.comm2-hint-item` / `__no` / `__main` / `__t` / `__d` 规则与 `dialog dialog--emp-help` + `dialog__scroll emp-help-body` 容器 —— **零新增 CSS**。序号在两个分节内**各自从 1 起**（与编辑页 5 + 8 同约定）。
...
| 「按项分摊 · 适用范围」 | 原价模式基数 = 原价 ×（范围内实付 ÷ 该行全部实付）；实收模式 = 范围内实付之和；固定额 × 分摊系数；卡付面值 / 计次折算 | **整条删除** —— 改由 Sheet「计算基数」行右侧 ⓘ（`#comm2BaseHelpMask`）承担；**列表页不得出现公式** |
...
> **【范围说明】本章属于「提成侧口径」**：它规定**下游承载页**（员工薪资明细 / 员工业绩提成明细 / 门店流水三页）必须与提成设置 5 类口径**对齐**，是提成设置的**验收外延** —— 改提成设置必然牵动这些页面。
> **这些页面的本体交互规格不在本文范围**（§6.10.1 / §6.10.2 的页面主体属员工薪资链路，见**源 PRD §6.12**；§6.10.3 流水三页不在 hub，保留作跨系统参考）。
> 因此本章内出现的 `§6.12.x` 引用一律指向**源 PRD**，本文未收录该章节。
...
| 列表页配套 | 删「生效中」；卡面「合计」左置 + 中部三行（基本工资 / 提成合计 / 奖惩合计带进度条，奖惩合计行 → 奖惩明细）+ 提成 2×2（项目 / 产品 / 办卡·充卡 / 快捷开单，无进度条；点格可直达业绩提成明细，**返回回员工薪资列表**）；月份 pill 可点（选择月份/结算期 ±3 期；跨自然月仅首尾日期，见**源 PRD §6.12.1**）；结算周期页结算日/发薪日均可 **1–31**（当月无该日则 clamp 月末；**仅店主/合伙人可改结算周期全部项**） |
...
> **hub 原型说明**：§6.10.1 / §6.10.2 口径已并入 **源 PRD §6.12**（以 `index.html` 为准）。§6.8 / §6.10.3 流水页**不在 hub**，保留作跨系统参考。

> **【不在本文范围】§6.11 员工管理（hub 链路二）/ §6.12 员工薪资（hub 链路三）。**
> 本文两条线只**最小依赖**其数据：员工池（`EmployeeStore.staff[]`，§7.7）、方案分配（`assigneeIds`，§3.4 · §6.6）、敏感权限 key（`roleSensitivePerms`，§7.8 · §9.2）、提成侧展示口径（§6.10）。
> 两链路的逐屏字段与跳转见**源 PRD §6.11 / §6.12**。

---

## 6.13 方案改动 → 本期提成怎么算（二十六次新增）

> 场景：一个**已经分配了员工**的提成方案被改动。改动会影响这些员工**本期已经算出来的提成**，所以保存前必须由用户明确选定口径 —— **不允许静默按默认口径落库**。
> 依赖：本文只用到员工薪资侧的 2 个**只读**接口（`getPeriodInfo` / `getCurrentPeriodInfo` / `nextPeriodInfo`；二十八次起 `countEffectiveCommLines` 已随影响面展示删除，三十一次起 `nextPeriodInfo` 已随 `next` 口径删除），结算周期本身的定义见**源 PRD §6.12.3**。
> 术语一律白话：弹窗里**不出现**「结算周期 / 生效口径 / 落地 / 重算引擎 / 快照」等词。

### 6.13.1 触发条件（三与条件，缺一不弹）

| 条件 | 判据 | 说明 |
|------|------|------|
| ① 已分配员工 | `assigneeIds` 非空 | **新方案**首次保存时是草稿、`assigneeIds` 为空 → **不弹**（此时无历史提成可影响） |
| ② 改动影响金额 | `schemeMoneyFingerprint(before) !== schemeMoneyFingerprint(after)` | 指纹只含 **`defaults` / `overrides` / `stationIds` / `stationLabels`** |
| ③ 有改动前快照 | `store._snapshot.id === sch.id` | 进入编辑页时自动拍（`openEdit`），草稿无快照 |

**不弹的情形**（③ 仍成立但 ② 为假）：**纯改名**、**改员工分配**、**未改任何字段**的重复保存。这三类保存一律直接落库 + toast，不打扰用户。

### 6.13.2 两种口径（三十一次：取消「下期生效」）

| 口径 | `effectiveMode` | 语义 | 本期已算好的行 | 本期之后的单子 | 下个结算周期 |
|------|-----------------|------|----------------|----------------|--------------|
| **本期全部重算** | `recalc` | 改错了、想让这一期立刻按新规则来 | **按新规则重算** | 新规则 | 新规则 |
| **算好的不动，从现在起用新方案**（**默认**） | `forward` | 不想动已经发生的账 | **保持原样**（老规则） | **新规则** | 新规则 |

**切分依据**：`forward` 按**行的日期**（`ymd`）与 `effectiveAt`（= 保存当天）比较 —— 同一期内**会切分**（改前的老规则、改后的新规则各算各的）；`recalc` **不切分**，整期新规则。

**两种口径 ↔ 弹窗控件一一对应（三十一次）**：弹窗里**两枚按钮就是两个结果** —— 上按钮 = `recalc`，下按钮 = `forward`。不再有开关、不再有中间态，「点了哪个就是哪个」，没有任何需要用户推断的间接层。

> **三十一次的删除说明**：原第三口径 `next`（本期先不动、下期再用新方案，`effectiveFrom` 字段）经领导拍板**整条取消**。连带删除：`schForLine()` 的 `next` 分支、`cmNextPeriod()`、列表页「下期生效」角标（§6.13.5 整节作废）、`EmployeeDemo.nextPeriodInfo()`。数据侧只剩 3 个字段（`effectiveMode` / `effectiveAt` / `_prev`）。

**人工修正优先（所有口径一律）**：已经人工改过的提成行（`commManual` 或带 `commEditLogs`）**金额恒不变**。由 `invalidateCommLineCache()` 的既有机制保证（重建前把原值搬进 `_commManualPreserve`，重建后回填）。

### 6.13.3 弹窗规格与文案（三十一次：**两枚按钮即两个结果**）

**形态**：居中 Dialog（`#comm2RecalcMask`），沿用 ModalPopup 规范 —— `max-width 326px`、`max-height min(560px, 78vh)`、正文区 `overflow-y: auto`、`z-index 50`（高于提成设置 Sheet 的 `30`）。

**二十八次精简**：弹窗**不再展示任何影响面信息** —— 原「这个方案有 N 名员工在用…」说明行、原灰条「本期 9.01–9.30 · 已算好 M 条」**整段删除**（连带删除 `countEffectiveCommLines()` 这个只为统计条数而存在的接口）。

**三十一次精简**：删除开关与「本期不动，下期换」按钮，改为**两枚竖排按钮**。用户看到的只有：**标题 + 一句提示 + 两枚按钮**。

| 区块 | 文案（须逐字一致） |
|------|--------------------|
| 标题（**固定**，不引用方案名） | 改方案后，提成怎么计算？ |
| 提示（左侧带 ⓘ 图标） | 已经手工改过的订单，金额不受影响。 |
| 上按钮（**描边次要**） | 本期全部重算 |
| 下按钮（**品牌主按钮**） | 从现在起用新方案 |

**两枚按钮各自说什么**：
- 上「**本期全部重算**」 → `recalc`：本期已经算出来的按新规则重算一遍，**钱会变**
- 下「**从现在起用新方案**」 → `forward`：之前算出来的**保持原样**，从现在开始的新单子按新规则算

**为什么改成按钮而不是开关**：只剩两种结果时，开关会多出一层「开关＝什么」的推断 —— 对文化水平有限的店主是纯增负担。按钮把**结果直接写在按钮上**，读一遍就懂，点一下即生效。代价是**没有回退机会**，所以两个结果的措辞必须**各自说清钱会不会变**（见上表）。

**排版细节（竖排）**：`.dialog__actions--stack`（`flex-direction: column`、`gap: 8px`、两枚按钮各自 `width: 100%`），**次要按钮在上、主按钮在下**（主按钮落在拇指最自然的位置，且与全站 Dialog 的「主按钮收尾」一致）。因文案较长，本弹窗内的按钮收到 `font-size: 14px` + `padding: 0 8px`（`.dialog--recalc .dialog__actions .btn-main`），实测两枚按钮**均不溢出、不折行**（`scrollWidth - clientWidth = 0`、Range 行框数 = 1）。

**提示行水平居中（三十二次）**：`.comm2-recalc__note` 加 `justify-content: center` + `text-align: center` —— **图标 + 文字作为一个整体居中**（不是只让文字居中而图标留在左侧）。实测弹窗内容宽 `284px`，提示行左留白 `30px` / 右留白 `30px`，**整体偏离中心 `0px`**，单行不折行。

**两枚按钮都是决定性动作**：因此**必须点一个才能完成保存** —— **无取消按钮**，**点遮罩空白与按 Esc 均不关闭**（全仓无通用 outside-close 机制，本弹窗不接线即天然成立）。

**未回答就退出** → 见 §6.13.8（一律兜底为 `forward`）。

### 6.13.4 数据落地与解析

**§7.1 新增 3 个字段**（`effectiveMode` / `effectiveAt` / `_prev`；原 `effectiveFrom` 已随三十一次删除），落库时机 = 用户在弹窗点任一枚按钮时（`applyEffectiveChoice()`）：

| 口径 | `effectiveMode` | `effectiveAt` | `_prev` |
|------|-----------------|---------------|---------|
| `recalc` | `'recalc'` | `''`（立即生效，不需要分界日） | 改动前的规则集快照 |
| `forward` | `'forward'` | 保存当天 `YYYY-MM-DD` | 同上 |

`_prev` 只存**金额相关字段**（`defaults` / `overrides` / `stationIds` / `stationLabels`），由 `schemeRuleSnapshot()` 从 `store._snapshot`（**改动前**的对象）抽取。

**解析函数** `schForLine(sch, periodKey, ymd)` —— 试算链路的**唯一改动点**：

```
schForLine(sch, periodKey, ymd):
  if (!sch._prev) return sch                      // 历史方案（无快照）→ 一律新规则
  if (effectiveMode === 'recalc') return sch      // 立即生效，本期也走新规则
  useOld = String(ymd).slice(0,10) < effectiveAt  // forward：分界日之前走老规则
  return useOld ? Object.assign({}, sch, _prev)   // 影子对象：套老规则，其余字段照旧
                : sch
```

因为影子对象只是「换掉 `defaults` / `overrides`」，`schemeLineAmount` / `resolveLineBlock` / `lineRateMeta` / `lineBaseSide` / `lineExtraAmount` **全部原样复用，零改动**。调用链：

| 位置 | 改动 |
|------|------|
| `calcStaffTrial(staffId, lines, periodKey)` | 新增第 3 参；内部 `schemeLineAmount(schForLine(sch, pk, line.ymd), line)` |
| `employee.js` `finalizeSeedCommLines` | 传 `state.salaryMonth`；`trialPayload` 每行带上 `ymd`（`_trial` 分支用 `Object.assign({}, base, {ymd})`，不污染原对象） |
| `Comm2Demo` 暴露 | `schForLine` 内部使用；`EmployeeDemo` 提供 `getPeriodInfo(key)` / `getCurrentPeriodInfo()`（二十八次删 `countEffectiveCommLines()`；三十一次删 `nextPeriodInfo()`） |
| 薪资侧重算 | `notifySalarySync()` → `invalidateCommLineCache()`（**既有能力**，无新增代码） |

### 6.13.5 列表页「下期生效」角标（**三十一次作废，整节删除**）

原 `effectiveMode === 'next'` 时在方案卡名称右侧渲染 `<span class="comm2-scheme__badge">下期生效</span>`。三十一次取消 `next` 口径后该角标**永不可能出现**，故连 CSS（`.comm2-scheme__badge`）一并删除，不留死代码。

### 6.13.6 「本期」的定义（易错点）

**「本期」= 含今天的那一期**（`getPeriodContaining(new Date())`），**不是**用户当前正在查看的期。两者可以不同：员工薪资列表的月份 pill 允许切 ±3 期（含过去），而改动重算问的**永远是当下这一期**。

**二十八次后弹窗不再显示日期**（原「本期 9.01–9.30」「下期是 2026年10月」等文案已删），但「本期」这个口径**仍然被内部使用**，且必须取自正确来源：

| 用途 | 取值 |
|------|------|
| `forward` 的分界日 `effectiveAt` | `cmToday()`（= 保存当天） |
| `recalc` | 不取期信息（立即生效） |

即：涉及「含今天那一期」时一律走 `cmCurrentPeriod()`（内部转调 `EmployeeDemo.getCurrentPeriodInfo()`），**不得**读 `state.salaryMonth`（那是用户正在查看的期，可能被切到过去）。若哪天要在弹窗里重新展示日期，也必须从这个来源取。

### 6.13.7 边界

| 情形 | 行为 |
|------|------|
| 新方案首次保存（`_draft`） | `assigneeIds` 为空 → 不弹，直接落库 |
| 员工被移出方案后再改该方案 | 只要 `assigneeIds` 非空仍弹（**二十八次起不再统计「影响面」**，仅判是否非空） |
| 多方案「重叠取最高」 | 改一个方案可能让**原本另一方案胜出**的行换人胜出 —— 属于设计内的连带影响（规则 3）。**二十八次起弹窗不再提示这一点**（影响面已删），仅在本文备案 |
| 员工同挂多方案但都不是本方案 | 同上：只看本方案 `assigneeIds` 是否非空，不判断其他方案 |
| 点错按钮（`recalc` 选成 `forward` 或反之） | **无撤销入口** —— 这是「按钮即结果」的固有代价。挽救路径：再进编辑页改一个金额字段再保存，会**再次弹窗**，重选一次即可 |
| 历史方案（无 `_prev`） | 一律按新规则算，口径字段对结果无影响（向后兼容） |

### 6.13.8 弹窗未回答就退出 → 默认「从现在起用新方案」（二十九次新增）

> **一句话**：用户**已经点过「保存」**（改动意图已表达），只是**没回答「本期怎么算」**。此时既不能把改动丢掉（等于吞掉用户的编辑），也不能默认重算（会动已经发出的账）—— 所以兜底口径**只能是 `forward`**，也就是弹窗默认态的那个选择。

**触发**：`recalcState.sch` 非空（弹窗悬着未回答）时，任何一条「弹窗消失了但没人做选择」的路径都先按 `forward` 落库：

| # | 退出路径 | 落点 | 说明 |
|---|----------|------|------|
| ① | 关闭 / 刷新页面 | `window` 的 `pagehide` + `beforeunload` | 原型**无持久化**（全仓无 `localStorage` / `indexedDB`），此条在原型里只体现为**契约** |
| ② | 返回按钮 | `requestComm2Exit()` | 先兜底落库并返回，**不再弹「尚未保存」**（改动已保存，没有未保存内容） |
| ③ | 任何回到列表的路径 | `openList()` 开头 | 返回按钮 / 顶部导航 / 程序化调用**都汇到这里**；若在此处不清口径，下面的 `_draft` / `_snapshot` 会被清掉 → **改动静默丢失** |
| ④ | 已选按钮 | `commitRecalcChoice()` | 正常路径，先清 pending 再落库，**不会**被兜底二次触发 |

**实现要点**：`recalcState.sch` 就是「有没有悬着的弹窗」这一个判据。`recalcFallbackForward()` 幂等 —— 先 `clearRecalcPending()` 再落库，所以 `openList()` → 兜底 → `commitSchemeSave()` → `openList()` 的**重入是安全的**（第二次进来 pending 已空，走正常渲染）。

**用户反馈**：兜底后弹 toast「**已按「从现在起用新方案」保存**」。（① 路径下页面即将销毁，toast 实际不可见；真正可感知的是 ② ③。）

**不留标记**：落库结果与「用户手动选了否」**完全等价**（不写额外的 `_autoDefault` 之类字段）。

**真实产品的对应要求**：原型无持久化，① 必须由**服务端**保证同一语义 —— **请求里没带 `effectiveMode` 时按 `forward` 落库**，否则用户在提交后杀进程会得到一个「方案改了、但口径未定」的中间态。

> **回归验证（二十六次一并修账）**：`_tmp_ui_verify.mjs` 的 `D7` / `D12` / `D13` / `I2` / `I3` 五项原本是**为「工位三选一 + 换工位替换」写的**（二十五次改为多选后未同步）。本轮已按多选口径重写：`D7` 断言 hint 含「**工位可多选**」；`D12` 断言**工位多选追加**（大工 · 中工 · 顾客指定，旧勾保留）；`D13` 改为**逐个点掉全部已勾选项**才完全取消该员工；`I2` / `I3` 由「换工位 → 旧卡并行红色交接」改为**「加选工位 → 旧勾全程保持红色、不走反向收勾」**（A8 已按二十五次废止红色交接）。现全套 **0 失败**。
...
| effectiveMode | `'recalc'` \| `'forward'` | **二十六次新增**：本方案上次被改动时用户选定的「本期提成怎么算」口径。缺省 / 历史方案按 `'forward'` 处理（**有 `_prev` 才参与解析**，无 `_prev` 时口径不影响结果） |
| effectiveAt | string | **二十六次新增**：`forward` 的分界日 `YYYY-MM-DD`（= 保存当天）；`recalc` 时为空串（立即生效）。`forward` 下**行日期 `ymd` 早于它**的行走老规则 |
| _prev | object | **二十六次新增**：本次改动**之前**的规则集快照 `{ defaults, overrides, stationIds, stationLabels }`（只含金额相关字段）。由 `schemeRuleSnapshot()` 从 `store._snapshot` 抽取；`schForLine()` 在老口径期间返回「套了它的影子对象」。**历史方案无此字段 → 一律按新规则算** |
...
> **【本文只锁最小依赖】** 提成设置 / 选择服务员工只消费本表的 **`achCommSet`**（提成设置入口）与**流水提成编辑**的权限判定（§9.2）。**12 项权限 key 全表与默认值**属员工管理链路 —— 见**源 PRD §6.11.3**。

- 结构：`{ [角色名]: { [key]: boolean } }`；**全量 key 表见源 PRD §6.11.3**。
- 角色管理可编辑四角色（合伙人/店长/高级店员/店员）；店主固定全开、不进列表。
- 运行时按员工 `perm` 只读解析；**无**个人覆盖字段。
- 初始默认见源 PRD §6.11.3 默认表。


> **【不在本文范围】§7.9 薪资周期 `PayCycle` / §7.10 奖惩 `Reward`。**
> 二者属员工薪资链路，与提成设置 / 选择服务员工无字段耦合。定义见**源 PRD §7.9 / §7.10**。
...
23. **敏感权限**：完全跟随 `EmployeeStore.roleSensitivePerms[角色]`（全量 key 表见**源 PRD §6.11.3**）；**无**个人 Switch / `staff.sensitivePerms`；业务 enforcement 待联调。
...
33. **方案改动后的本期重算口径（二十六次，详见 §6.13）**：**已分配员工**的方案被改动且改动**影响金额**时，保存前必须由用户选定本期怎么算 —— `recalc` 本期全按新方案重算 / `forward` 算好的不动、从现在起用新方案（默认）；**不得静默按默认口径落库**（弹窗无取消、遮罩与 Esc 均不关闭）。要点：
    - **触发**只认「影响金额」的改动（`defaults` / `overrides` / `stationIds` / `stationLabels` 的指纹变化）；**纯改名 / 改员工分配 / 无改动不弹**（新方案首次保存 `assigneeIds` 为空，也不弹）。
    - **弹窗只有两枚按钮**（三十一次：取消「本期不动，下期换」）—— 上「本期全部重算」= `recalc`、下「从现在起用新方案」= `forward`，**点即生效**。
    - **「本期」= 含今天的那一期**（`getPeriodContaining(new Date())`），**不是**用户正在查看的期（月份 pill 可切 ±3 期）。涉及当期时一律从 `getCurrentPeriodInfo()` 取。
    - **人工修正优先**：所有口径下，已人工改过的提成行（`commManual` 或带 `commEditLogs`）**金额恒不变**（由 `invalidateCommLineCache()` 的既有保留/回填机制保证）。
    - **切分粒度**：`forward` **按行日期**切分（`ymd` vs `effectiveAt` = 保存当天），同一期内新老规则各算各的；`recalc` **不切分**。
    - **向后兼容**：无 `_prev` 的历史方案一律按新规则算，口径字段不影响其结果。
    - **与规则 3 的关系**：计提侧仍按「重叠取最高」跨方案取高 —— 改一个方案可能让**原本另一方案胜出**的行换人胜出，属设计内连带影响，本期提示不逐行列差异。
...
> **【不在本文范围】§8.3 员工管理业务规则（规则 30–41）/ §8.4 员工薪资业务规则（规则 42–51）。**
> 与提成无关。仅**一条**必须遵守的交叉约束：员工表单里的「提成方案」（§7.7 `assigneeIds`）与提成设置的「分配员工」Sheet 是**同一份数据**，双向同步（本文 §3.4）。
> 完整规则见**源 PRD §8.3 / §8.4**。
...
**方案改动重算弹窗（§6.13，二十六次新增 / 二十八次改版 / 三十一次精简）**：改方案后，提成怎么计算？、已经手工改过的订单，金额不受影响。、本期全部重算、从现在起用新方案；**未回答即退出时的兜底 toast**：已按「从现在起用新方案」保存。**该弹窗全篇不得出现「结算周期 / 生效口径 / 生效方式 / 落地 / 重算引擎 / 快照 / 回溯 / 计提」等术语**，也不得出现公式；**二十八次起也不再展示任何影响面信息**（无「N 名员工在用」、无「本期 {起}–{止}」、无「已算好 M 条」）；**三十一次起不再有开关，也不再有「本期不动，下期换」与列表页「下期生效」角标**。两枚按钮的措辞必须**各自说清钱会不会变**（「本期全部重算」＝金额会变 / 「从现在起用新方案」＝已算好的不动）。
...
> **未被收录的三段文案（属员工管理 / 员工薪资链路）**：原 §9.1 的 **「员工（表单/详情/列表）」**（员工列表 / 详情 / 我的资料 / 创建 / 邀请 / 头衔 / 头衔校验 / 状态 等文案）、**「头衔/角色」**（头衔管理相关全部文案）、**「薪资」**（薪资列表 / 明细 / 结算周期 / 奖惩 / 业绩提成明细 等文案）三段本文**不再收录** —— 见**源 PRD §9.1**。
> 两条线**实际需要**的交叉文案只有一处：**「提成方案」**（员工表单内的方案选择 sheet 标题）与 **「选择提成方案」** —— 与提成设置「分配员工」Sheet 共用同一份方案数据（§3.4）。

**空态**：还没有提成方案、点下方按钮添加、该分类下暂无内容、暂无提成方案、暂无在岗员工、信息未完善。
...
> **同一「改提成」权限也适用于「员工业绩提成明细」页**（承载页属员工薪资链路）：店主 / 合伙人 / **店长**可改、保存即生效并留修改日志；高级店员 / 店员只读且可查看修改记录。该页**无标题栏「员工视角 / 店主视角」切换**（正式产品按登录身份自动落位；原型上的切换仅供演示，**不得**交付）。

**剔除以外的权限表（属员工管理 / 员工薪资链路）**：原 §9.2 的 **「员工管理链路」**（员工列表/详情查看、`staffCreate`、头衔管理、角色管理）与 **「员工薪资链路」**（查看薪资列表/明细、结算周期、设奖惩、月份切换、员工视角）两张表本文**不再收录** —— 见**源 PRD §9.2**。

> 本文两条线**只需**上表的「提成设置链路」一表 + 权限点两行：`achCommSet` 决定工作台「提成设置」入口是否拦截，「改提成」另走一套（与 `achCommSet` 分离，含店长）。
...
40. **Given** 店主 / 合伙人 / 店长在**员工业绩提成明细**改提成，**When** 保存，**Then** 直接生效并出现「改」标记；点击可查看修改记录；无待确认流程。（承载页属员工薪资链路，页面本体见源 PRD §6.12）

> **剔除以外的用例（属员工管理 / 员工薪资链路）**：原 §10.2 的 **37 / 38 / 39**（员工薪资明细的业绩统计 / 提成统计 / 服务售卡比例条）、**41 / 41a**（结算周期设置与店长只读）、**39 / 40**（员工业绩提成明细月合计的业绩分列 / 提成分列 —— 与上面 37/38 重复）、**65f**（员工详情「社区形象」卡）本文**不再收录**，见**源 PRD §10.2**。
> 其中与提成有关的口径**已由本文 §6.10.0 / §6.10.1 / §6.10.2 覆盖**（业绩按原价加总；提成 5 类且「项目」扣快捷开单；五列之和 = 总计；服务售卡为实收）。
...
56l2. **Given** 方案编辑页，**When** 点标题右侧 ⓘ，**Then** 弹出「**提成怎么设、钱怎么算**」（`comm2RuleHelpMask`，`dialog--emp-help`，`max-width 326px` / `max-height min(520px,75vh)`、正文可滚动、底部单按钮「知道了」）：两节 **「一、怎么设置（5 步）」** 与 **「二、钱怎么算出来（8 条）」**，共 **13 条**；每条为**序号卡片** —— 序号圆点 `18×18`（`border-radius:50%`、`#F0F0F0` 底、`11px/600`）与分节标题**左对齐**、与文字间距 `8px`，右侧为**加粗 13px 白话结论** + **12px `#929292` 灰字补充**（文案逐字见 §6.9「文案原文」表）；**统一用白话**，不得出现「取高」「分摊」「覆盖优先于默认」等术语。**And When** 点「知道了」或点遮罩空白，**Then** 关闭；**And Given** 列表页，**Then** 其 ⓘ 打开**列表页自己的**弹窗（`comm2HelpMask`，二十四次已白话重写，见 `56n`），两处**触发互不串台**。

56n. **Given** 方案列表页，**When** 点标题「提成设置」右侧 ⓘ（`comm2HelpBtn`，`20×20` 热区 / `16×16` 线性 ⓘ / `#B2B2B2`、`aria-label` = **弹窗标题原文**），**Then** 弹出「**一个人挂几个方案，钱怎么算**」（`comm2HelpMask`，`dialog--emp-help`，`max-width 326px` / `max-height min(520px,75vh)`、正文可滚动、底部单按钮「知道了」）：两节 **「一、谁能挂几个方案（2 条）」** 与 **「二、钱怎么算（5 条）」**，共 **7 条**；每条为**序号卡片** —— 序号圆点 `18×18`、序号与分节标题左对齐、与文字间距 `8px`，右侧为**加粗 13px 白话结论** + **12px `#929292` 灰字补充**（文案逐字见 §6.9 附 2 表）；**两个分节的序号各自从 1 起**（与编辑页弹窗同约定）；样式**复用**编辑页那批 `.comm2-hint-list` / `.comm2-hint-item*`，**零新增 CSS**。**And** 正文**不得出现**「取高 / 分摊 / 覆盖 / 分类默认 / 订单行 / 试算 / 计提 / 并列 / 命中 / 金额最大 / 写入 / 胜出 / 适用范围」等内部术语，**也不得出现任何公式**（`×` `÷`）—— 原「按项分摊 · 适用范围」段（含基数公式 / 卡付面值 / 计次折算）**已整条删除**，该内容由 Sheet「计算基数」行右侧 ⓘ 承担；原「分配 Sheet 副文显示图标与方案名（无「也在：」字面）」实现细节**已删除**。**And When** 点「知道了」或点遮罩空白，**Then** 关闭。
56o. **Given** 方案 S 已分配 ≥1 名员工，**When** 只把方案名改掉（不改任何规则参数）后点「保存」，**Then** **不弹**重算弹窗，直接落库 + toast「提成方案已保存」并回列表（`schemeMoneyChanged()` 为假）；**And Given** 方案 S 未做任何改动就点「保存」，**Then** 同样不弹；**And Given** 新建方案（`_draft`）首次保存，**Then** `assigneeIds` 为空 → 也不弹。
56o2. **Given** 方案 S 已分配 ≥1 名员工，**When** 改动**影响金额**的任一字段（提成参数 / 适用范围 / 计算基数 / 分配模式 / 工位值 / 规则项）后点「保存」，**Then** **不立即落库**，先弹 `#comm2RecalcMask`（居中 Dialog、`max-width 326px`、`max-height min(560px,78vh)`、正文可滚动、`z-index 50`）：标题「改方案后，提成怎么计算？」，**其下只有一行带 ⓘ 图标的灰字提示**「已经手工改过的订单，金额不受影响。」，底部**两枚竖排按钮** —— 上「**本期全部重算**」为描边次要样式、下「**从现在起用新方案**」为品牌主按钮。**And** 弹窗内**不得出现**任何影响面信息 —— 无「**{N}** 名员工在用」说明行、无灰条、无「本期 {起}–{止} · 已算好 {M} 条」、**无三张单选卡**（`#comm2RecalcLead` / `#comm2RecalcImpact` / `#comm2RecalcOpts` / `#comm2RecalcOk` 四个节点在 DOM 中均不存在）。**And** 三十一次起**也不得有开关与「本期不动，下期换」**（`#comm2RecalcToggle` / `#comm2RecalcToggleLbl` / `#comm2RecalcNext` / `#comm2RecalcNow` / `#comm2RecalcRow` 与 `.comm2-recalc__switch` / `.comm2-recalc__row` / `.comm2-recalc__row-lbl` 均不存在）。**And** 两枚按钮**竖排**（容器 `dialog__actions--stack`、`flex-direction: column`、`gap 8px`）、**左边缘对齐、等宽且撑满内容宽**（实测均 `284px`）、**文案均不溢出也不折行**（14px / `padding 0 8px`，实测 `scrollWidth - clientWidth = 0`、Range 行框数 = 1）。**And** 弹窗**无取消按钮**；**When** 点遮罩空白或按 Esc，**Then** **不关闭**（必须二选一）。**And** 全篇**不得出现**「结算周期 / 生效口径 / 落地 / 重算引擎 / 快照 / 回溯」等术语与任何公式。
56o3. **Given** 重算弹窗已打开，**When** 点**上按钮「本期全部重算」**，**Then** 落 `recalc`（`effectiveMode = 'recalc'`、`effectiveAt` 为**空串**、`_prev` = **改动前**的规则集快照），弹窗关闭、落库、回列表，方案卡**无任何角标**；**And When** 点**下按钮「从现在起用新方案」**，**Then** 落 `forward`（`effectiveMode = 'forward'`、`effectiveAt` = **保存当天**、`_prev` 同上），同样关闭落库回列表、**无角标**。「当期」一律取**含今天的那一期**（`getPeriodContaining(new Date())`），**不得**用正在查看的期（`state.salaryMonth`，月份 pill 可切 ±3 期）。**And** 数据模型**不得**再出现 `effectiveFrom` 字段（三十一次已删）。
56o5. **Given** 重算弹窗已打开，**When** 点下按钮「从现在起用新方案」，**Then** 本期已算好的提成**保持原样**，从现在起的新单子按新规则算；**And When** 点上按钮「本期全部重算」，**Then** 本期已算出来的提成**按新规则重算一遍**（`_prev` 供 `schForLine` 判定）。两种情况下 `_prev` 均为**改动前**的规则集快照。
56o6. **Given** 任一口径下、本期已算好的行里有**人工改过**的（`commManual` 或带 `commEditLogs`），**When** 落库后重算，**Then** 该行金额**保持人工值不变**，其余行按所选口径处理（由 `invalidateCommLineCache()` 的保留 / 回填机制保证）。**And Given** 弹窗底部提示，**Then** 该句左侧有 `14×14` 灰 ⓘ 图标（内联 SVG，`#B2B2B2`），文字为「已经手工改过的订单，金额不受影响。」。
56o7. **Given** 重算弹窗已打开且**未做任何选择**，**When** 触发以下任一条退出路径 —— ① 关闭 / 刷新页面（`pagehide` / `beforeunload`）、② 点返回按钮、③ 任何回到列表的路径（`openList()`）—— **Then** 一律按**默认口径 `forward` 落库**：方案改动**被保存**（不丢）、`effectiveMode = 'forward'`、`effectiveAt` = 保存当天、`_prev` = 改动前快照，且弹窗关闭、回到列表、**toast「已按「从现在起用新方案」保存」**。**And** 落库结果与「用户直接点下按钮」**完全等价**（不留任何「系统默认」标记字段）。**And** 点返回按钮时**不再弹「尚未保存」**（改动已保存，没有未保存内容）。**And** 兜底函数幂等：`openList()` → 兜底 → `commitSchemeSave()` → `openList()` 的重入**不得**造成二次落库或状态错乱。**And Given** 弹窗内两枚按钮各自的正常路径，**Then** 不会被兜底二次触发（`commitRecalcChoice()` 先清 pending 再落库）。
56o4. **Given** 方案 S 的 `_prev` 已写入（老口径 10% / 新口径 20%，两行各 1000 元，一行在改动日**之前**、一行在**之后**），**When** 计提，**Then** `recalc` = **400**（两行都用新规则、**不切分**）、`forward` = **300**（改前 100 + 改后 200、**按行日期 `ymd` 切分**）；**And Given** 该方案**无 `_prev`**（历史方案），**Then** 两种口径结果一致、一律按新规则算（向后兼容）。**And Given** 本期已算好的行里有**人工改过**的（`commManual` 或带 `commEditLogs`），**When** 点上按钮「本期全部重算」，**Then** 该行金额**保持人工值不变**，其余行按新规则重算（由 `invalidateCommLineCache()` 的保留 / 回填机制保证）。**And Given** 列表页方案卡，**Then** **不得**出现「下期生效」角标（`.comm2-scheme__badge` 已随三十一次删除）。
...
> **剔除以外的用例**：原 §10.2.1 之后的 **65f**（员工详情「社区形象」卡，属员工管理链路）本文不再收录，见**源 PRD §10.2**。


> **【不在本文范围】§10.3 链路二 · 员工管理 P0 / §10.4 链路三 · 员工薪资 P0。**
> 验收用例见**源 PRD §10.3 / §10.4**。
...
| 列表拖动排序起拖 | `12ms` | 员工列表 / 角色管理（**不在本文范围**，见源 PRD §12.5 `C1`） |
...
> **【不在本文范围】§12.5 C 组 · 员工管理 / 员工薪资（列表长按拖动排序 `C1` / 行定位闪烁 `C2`）。**
> 两条线均不涉及。规格见**源 PRD §12.5**。
...
> **剔除以外的行**：原 §12.7.2 的 **`C1 拖动 ↔ 行点击`**（员工管理链路）本文不再收录 —— 见**源 PRD §12.7.2**。
...
> **剔除以外的行**：原 §12.8 的 **`C1 拖动排序`**（员工管理链路）本文不再收录 —— 见**源 PRD §12.8**。
...
> **剔除以外的用例**：原 §12.9 的 **N23**（员工列表长按拖动排序，属 §12.5 C 组 · 员工管理链路）本文不再收录，见**源 PRD §12.9**。
...
2. 再改原型（本文两条线：`comm2.css` / `comm2.js`；壳层基座另涉 `base.css` / `shell.css`）。
3. 跑 §12.9 的 `N1–N26`；其中 **N5–N9e / N10 / N14 / N14b / N15–N17c / N18 / N19–N19c / N21 / N25** 是回归必跑项（勾选动效、门控、**收起 Morph**、一次性标记、「只勾顾客指定」态最易回归）。（`N22` 属 B 组左滑、`N23` 属员工管理链路已剔除、`N24` 属 D 组 Toast、`N26` 属 A 组门控。）
4. 本节新增动效时，必须补齐 §12.1.1 的 10 个字段，缺项视为不合格。
5. **改完必须回写源 PRD**（源 PRD 是主档，本文是其抽取版；两文同批更新，见文件头「维护」）。
...
| 交付 | 说明 |
|------|------|
| 本文 `PRD-提成设置-开发版.md` | **提成设置 + 选择服务员工** 两条线的开发准稿 |
| `PRD-提成设置-开发版.html` | 阅读预览（与 .md 同步） |
| 独立原型本目录 `index.html`（+ `comm2.js` / `comm2.css`） | **唯一可视依据** |
| 上游全量来源 `PRD-提成设置.md` | 三链路合一（提成设置 · 员工管理 · 员工薪资） |
| 关联页配图 `assets/related/` | 薪资明细 / 业绩提成明细 / 流水三页现状截图（源：`提成关联页面/`） |
| 附录 A · 原型文件索引 | 本文独有：`index.html` / `comm2.js` / `comm2.css` 里与本两条线相关的函数与位置 |

**开发只需本文 + 原型**。与价目 / 会员卡交叉时，覆盖规则目标数据以 `PRD-项目创建与管理.md`、`PRD-会员卡管理.md` 为准，提成逻辑以本文为准。冲突时以原型界面为准修正 PRD。

**范围外**：员工管理、员工薪资两条链路的完整规格 → 源 PRD §3.2–3.3 / §4.8–4.9 / §6.11–6.12 / §7.9–7.10 / §8.3–8.4 / §10.3–10.4 / §12.5。
...
> 本附录是本文独有内容，帮助开发**直接跳到原型里对应的实现**。
> **行号以当前工作区版本为准**（`index.html` 1176 行 / `comm2.js` 4194 行 / `comm2.css` 1509 行）；原型迭代后行号会漂移，**函数名与类名才是稳定锚点**。
> 用法：先在本文找到规格章节 → 用本附录定位原型函数 → 对照实现即可；**不要**用行号做自动化引用。

## A.1 文件总览

| 文件 | 行数 | 职责 | 与本两条线的关系 |
|---|---:|---|---|
| `index.html` | 1176 | 页面骨架（`screen-*` 容器 + Sheet / Dialog 蒙层） | 两条线的 DOM 宿主；**选人 Sheet 在 §332–363** |
| `comm2.js` | 4194 | **提成设置全部逻辑** + **选择服务员工全部逻辑** | 本文规格的**唯一实现来源**；§3178 起为选人段 |
| `comm2.css` | 1509 | 提成设置 + 选人 Sheet 全部样式与关键帧 | §1158 起为选人段（含 `--sp-*` 变量与 `staff-*` 类） |

**变量同源红线**：`comm2.css` 的 `:root`（第 7 行起）定义全部动效时长 / 缓动变量；`comm2.js` **直接读取**这些变量算门控时长（`spCssMs()`），**任何一处不得各写常量** —— 详见 §12.2.1 与 §12.3.0.8。

## A.2 选择服务员工线（§4.5.1 · §6.7.1 · §12.3）

`comm2.js` 自 **3178 行**起是选人段（与提成设置段以 `SP_*` / `sp*` 前缀区分）。

| 分组 | 行号区间 | 关键函数 |
|---|---|---|
| 常量 | 3178–3211 | `SP_CHEV` · `SP_CHECK_SVG` · `SP_ROLE_IDS` · `SP_ROLE_DEFAULT` · `SP_GRID_GAP` · `SP_SPRING` · `SP_EASE_STD` · `SP_EXPAND_MS` · `SP_COLLAPSE_FALLBACK_MS` · `SP_FALLBACK_STAFFS` |
| 基础工具 / 方案读值 | 3212–3268 | `spEl` · `spEsc` · `spHaptic` · `spReduceMotion` · `spScheme` · `spStationLabel` · `spStationLabelsJoined` · `spStationIds` · `spStaffPool` |
| **状态与派生量** | 3269–3309 | `SP_PICK_AVG`（不分工位哨兵）· `spEnsureState` · `spNeedStation` · `spNeedExtra` · `spNeedsPick` · `spIsAvg` · `spSheetOpen` |
| **选项模型与摘要** | 3310–3411 | `spOptionList` · `spOptionChecked` · `spOptionByKey` · `spStaffIsChosen` · `spBasePicked` · `spExtraOnly` · `spSummaryParts` · `spSummaryText` · `spSummaryHtml` · `spCardPickLineHtml` |
| **门控与动效**（§12.3 A6–A9 / A13） | 3412–3580 | `spCssMs` · `spCheckMs` · `spUncheckMs` · `spCheckMsFor` · `spGateBusy` · `spCheckAnims` · `spSnapChecks` · `spGateAfter` · `spGateHoldOnly` · `spCollapseMs` · `spCollapseTarget` · **`spCollapseMorph`（A2 收起 Morph）** |
| **状态变更**（§4.5.1 口径落地） | 3581–3756 | `spEditChange` · `spApplyEdit` · `spGateFlush` · `spIntend` · `spCheckHost` · `spPlayCheck` · `spOptBoxEl` · `spSelectStaff` · **`spDropStaff`** · **`spApplyOptionToggle`（点选/取消唯一入口）** · `spTapOption` · `spToggleStaff` |
| **渲染** | 3757–3824 | `spJobTitleHtml` · `spAvatarHtml` · `spOptionsPanelHtml` · **`spRenderPickerHtml`** |
| 展开 Morph 与屏幕 | 3825–4040 | **`spAnimateStaffMorphLayout`（A1 / A2 几何）** · `spAfterStaffPickerPaint` · `spRenderSummaryHtml` · `spRenderEntryHtml` · **`spHintText`（四态 hint）** · `spRenderScreen` · `spRenderSheet` · **`spRedraw`** · `spOpenSheet` · `spCloseSheet` · `spOpen` · `spSyncModeButtons` · `spCurrentSchemeName` · `spEnterEdit` |
| 事件绑定 | 4041–4194 | `wire`（点击分派：选项卡 / scrim / 编辑卡空白 / 换卡）· `spRemoveStaff`（入口摘要行 ×） |

**建议阅读顺序（照 §12.3.0.1 的实现顺序）**：
`spEnsureState` → `spOptionList` / `spOptionChecked` → `spApplyOptionToggle` → `spGateAfter` / `spCollapseMorph` → `spRenderPickerHtml` → `spAnimateStaffMorphLayout` → `wire`。

## A.3 提成设置线（§6.1–§6.6 · §6.8 · §6.9）

| 分组 | 行号区间 | 关键函数 |
|---|---|---|
| 常量与值模式工具 | 5–130 | `COMM2_CATS` · `COMM2_STATIONS` · `COMM2_PAY_SCOPE` · `COMM2_QUICK_OV_ID` · `defaultPair` · `normalizeValueMode` · `isAmtMode` · `nonIsAmt` · `extraIsAmt` · `extraVal` · `extraKey` · `stationIsAmt` · `ruleAllAmount` · `syncLegacyValueMode` |
| **旧数据迁移**（§6.3 / 验收 56j） | 124–194 | `migrateExtraFields` · `migratePerValueModes` · `applyExtraSplitFlag` |
| 分类规则 / 工位 / 支付范围 | 195–345 | `defaultCatRule` · `defaultStationLabels` · `getStationIds` · `ensureCat` · `defaultPayScope` · `ensurePayScopeBlock` · `defaultCardBlock` · `kindToBelongCat` · `isQuickOverride` · `isQuickLine` · `ensureSystemQuickOverride`（第 5 张「快捷开单」系统覆盖） |
| 方案归一化 / 取值 | 346–482 | `normalizeScheme` · `catRulePct` · `catRuleAmt` · `catRuleStationPct` · `buildDefaults` · `overrideRow` |
| 演示数据 / 格式化 | 483–758 | `seedDemoSchemes`（3 个演示方案，§8.2）· `defaultScheme` · `schemeById` · `pickModeLabel` · `baseModeLabel` · `stationLabel` · `fmtMoney` / `fmtVal` / `formatExtraVal` / `formatPairFlat` / `formatStationPair` / `formatBlockSummary`（§12.2.3 两位小数 + 小数点后小一号） |
| 方案列表 | 778–870 | `formatSchemeRuleParts`（列表摘要 4 段）· `schemesAlsoContainingStaff` · `assignedStaffSet` · `renderUnassignedTip` · `renderList` |
...
| 行号区间 | 内容 |
|---|---|
| 1–6 | 文件头：**「动效时长 JS 与 CSS 同源」** 红线说明 |
| **7–100** | `:root` 全部变量 —— 含 `--sp-check-red: 60ms` / `--sp-check-draw: 110ms` / `--sp-uncheck-draw: 55ms` / `--sp-uncheck-red: 30ms` / `--sp-collapse: 220ms` / `--sp-ease-std: cubic-bezier(.22,.82,.24,1)`（§12.2.1 / §12.2.2） |
| 101–136 | 方案列表卡 · 未分配提示条 |
| 137–300 | 规则卡：适用范围胶囊 · chip · 分段控件 |
| 299–350 | 卡内标题 `#929292` · 取值行布局 · **两段式胶囊** · 金额输入框宽度约束 |
| 332–414 | 虚线卡（`+ 顾客指定提成`）· 参数区行布局 |
| 415–530 | 覆盖规则卡 · 左滑删除 |
...
| 561–574 | **「一个人挂几个方案，钱怎么算」帮助弹窗**（**列表页** `#comm2HelpMask` / `#comm2HelpBtn`；**二十四次白话重写**：两节 2 + 5 条 `.comm2-hint-item` 序号卡片，§6.9 附 2） |
...
| 我要改… | 从哪进 |
|---|---|
| 计提金额不对 | `comm2.js` → `schemeLineAmount` → `lineRateMeta` → `resolveLineBlock` / `linePayParts`（规则 2 分摊） |
| 「只勾顾客指定」金额算错 | `comm2.js` → `lineBasePicked` → `lineRateMeta`（`basePicked=false` → `rate: 0`）+ `schemeLineAmount` 的费率标签分支 |
| 选人勾选状态不对 | `comm2.js` → `spApplyOptionToggle`（唯一入口）→ `spOptionChecked` / `spStaffIsChosen` |
| 选人摘要文案不对 | `comm2.js` → `spSummaryHtml`（**卡片第三行**）/ `spSummaryText`（**入口摘要行**）—— 两处口径不同，见 §12.3.0.3 |
| 选人动效时序不对 | `comm2.js` → `spGateAfter`（门控）→ `spCollapseMorph`（收起）→ `spAnimateStaffMorphLayout`（展开）；CSS 变量在 `comm2.css:7` |
| Sheet 参数卡布局不对 | `comm2.js` → `commissionParamsHtml`（行结构）+ `comm2.css:1044–1095`（行标签 / 两行居左 / 虚线卡等高） |
...
| 列表页弹窗文案不对 / 又冒出术语 | `index.html:561–574`（`#comm2HelpMask`，**文案原文见 §6.9 附 2 表**，两节 2 + 5 条）；样式**复用** `comm2.css:1167–1174` 的 `.comm2-hint-item*`（**不要为列表页另写一套**） |
