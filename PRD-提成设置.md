...
| 文档版本 | v1.9 |
| 日期 | 2026-08-31 |
| 交互原型（唯一可视依据） | 本目录 `index.html`（独立原型） |
| 画布 | 390 × 844 |
...
> **v1.7（2026-08-31）审计对齐**：支付范围改为三类；规则卡以原型「卡面可点」为准；覆盖删除改为左滑/长按+撤销；工位缺失兜底对齐原型；分配副文为图标+方案名；并补边界规则。§6.8 / §6.10 等关联页**保留**，标注「本系统现有页面」。

> **v1.8（2026-08-31）经理签单开关**：方案卡新增「经理签单计算提成」switch（默认关闭）。经理签单为**支付方式之一（相当于免单）**，订单行不计入现金/卡付/团购三类支付范围，由方案级 `signComm` 开关决定是否计提。试算新增经理签单演示行。

> **v1.9（2026-08-31）去大原型引用**：本原型与 `demo.html` / `剑琅联盟-RTB重构.html`（`commission2`）**非同一系统**，删除全部大原型文件 / 页面引用；流水 / 薪资关联页为本系统现有页面（§6.8 / §6.10 保留）；模块 13 迁移口径修正：原系统 `payScope.sign` 迁移为方案级 `signComm`。
...
| 经理签单独立开关 | 方案级决定经理签单（相当于免单）订单是否计算提成 | 方案卡第二行 switch；关闭时经理签单行不计提成 |
...
提成设置（多方案）
 ...
 │    ├─ 方案卡：图标 · 名称 · 经理签单计算提成（switch）· 已分配员工
 │    │         · 分配入口 · ⋯ 菜单（重命名 / 复制方案 / 删除）
 │    └─ 底部「+ 添加提成方案」/ 空态
├─ 方案编辑
...
下列字段、按钮、跳转均须与原型一致；视觉以原型为准（规则卡布局见 §6.2；右列胶囊为黑白灰立体段控 + 选中莫兰迪字色 `#E89D64` / `#E06767`；固定金额时右上显示只读「固定」灰标）。
...
- 方案卡（`emp-comm-card`）：
  ...
  - 经理签单行：「经理签单计算提成」+ 右侧 switch（`comm2-signcomm`）；点击切换方案 `signComm`，立即生效并 toast「经理签单已计入提成 / 经理签单不计入提成」，无需进编辑页。
...
| 复制方案 | 生成「{原名} 副本」，`assigneeIds` 清空，`signComm` 原样保留；toast「方案已复制」 |
...
规则卡为**左右两列网格**：左列标题 + 发丝线 + 提成参数；右列计算基数控件 + 提成范围控件。默认规则左竖条品牌色；覆盖规则 `#FF8A3D`。用户覆盖外包左滑容器。

```
┌─▌ 标题（≤6 字完整 / 超出省略）          [ 原价 | 实收 ] 或「固定」 ┐
│  ──────── 发丝线 ────────                                     │
│  点 10%   散 10%（或 大/中/小 …）     [ 现金 | 卡付 | 团购 ]   │
└─▌                                                              ┘
```

| 热区 | 行为 |
|------|------|
| 标题 / 发丝线 / 左列垫高空白 | 打开规则设置 Sheet |
| 提成参数区 | 打开 Sheet，并滚动/高亮「提成参数」锚点 |
| 右列「原价/实收」段控 | **卡面直接切换** `baseMode`（置脏，不经 Sheet 确定）；固定金额时改为只读「固定」灰标，不可点切 |
| 右列「现金/卡付/团购」段控 | **卡面直接多选切换** `payScope`（至少保留 1 类，否则 toast） |
| 比例/固定 | **仅在 Sheet**「提成取值」切换；卡上不提供比例/固定控件 |
...
| 规则 | 说明 |
|------|------|
| 短标 | 不分工位：`点` / `散`；按工位：工位名首字（默认大/中/小） |
| 数值 | 字号加大、主文字色；比例带 `%`，固定带 `¥`；工位段为 `值·值` |
| 完整 | 数值不可省略号截断；空间不足时可换行 |
| 工位改名 | 即时反映 `stationLabels` |
...
|------|------|
| 未选 | 浅灰底、淡灰字 |
| 选中（原价/实收） | 白底 + 字色 `#E89D64` + 轻阴影 |
| 选中（现金/卡付/团购） | 白底 + 字色 `#E06767` + 轻阴影 |
| 固定金额 | 右上只读灰标「固定」，替代原价/实收段控 |
...
| 区 | 控件 | 说明 |
|----|------|------|
| 提成范围 | chips 多选（现金 / 卡付 / 团购） | 至少 1 类；选中态品牌色 |
| 计算基数 | 分段：原价模式 / 实收模式（固定金额时仍可展示；计提不乘基数） | `baseMode`；分段保持可切换，**但当提成取值切换为「固定金额」时，规则卡上的计算基数标签改显「固定」，不再随分段变化** |
...
| signComm | boolean | **经理签单计算提成**开关；经理签单为支付方式之一（相当于免单）。默认 `false`；复制方案原样保留 |
...
2. **按行过滤**：对每个候选方案，每笔订单行先按**支付方式**对照该方案命中块的「提成范围」；不在范围则该方案本行**不计**、不参与取高。**经理签单行（`sign=true`）不参与支付范围过滤**，由方案级 `signComm` 开关控制（规则 29）。
...
15. **支付范围三类**：现金、卡付、团购；原型 `COMM2_PAY_SCOPE` 与此一致。**经理签单为独立支付方式（相当于免单），不进入三类多选**，由方案级 `signComm` 控制计提（规则 29）。
...
23. **卡面直改置脏**：在规则卡上切换 `baseMode` / `payScope` 立即写入内存并 `markDirty`，无需打开 Sheet 点确定；与 Sheet 内修改等价。
...
29. **经理签单开关**：方案级 `signComm` 控制经理签单订单行（`sign=true`，相当于免单）是否计提，默认**关闭**。开启后按常规口径计提（命中覆盖/默认块、按基数与参数计算，**不受三类 `payScope` 限制**）；关闭则整行 `skipped='sign'`、无 `winnerSchemeId`。复制方案原样保留；老数据兜底 `false`。
...
计提验证步骤：取员工所属**全部**方案（列表顺序）→ 每行对每个方案解析覆盖/默认块 → 非经理签单行按支付范围过滤，经理签单行按方案 `signComm` 开关过滤 → 计算金额 → **取金额最大者**（并列保留列表更靠前方案）→ 汇总。结果行带 `winnerSchemeId`。此口径与 §8 一致，作为联调参考样例（非正式接口）。
...
3 个演示方案 `signComm` 默认**关闭**；开启「资深技师综合方案」后，经理签单·深层补水护理行按深层补水覆盖 18% × 原价 268 计提 ¥48.24。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、反选、清空、重命名、复制方案、删除、改名、恢复为默认、分配员工、已分配、未分配、已选 N 项、已选 N 人、还有 N 人未分配任何提成方案、**经理签单计算提成**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、**关联页面、选择员工（3工位）、选择员工（2工位）、选择员工（1工位）**。

**规则卡**：原价、实收、**固定**、点客、散客、提成范围、计算基数、分配模式、不分工位、按工位分配、按比例 %、固定金额 ¥、现金、卡付、团购、提成参数。
...
**Toast**：请输入方案名称、方案已重命名、方案已复制、方案已删除、提成方案已保存、已更新规则、已添加规则项、已删除规则项、撤销、已分配 N 人、已清空分配、**已选 N 人**、至少选一种支付方式、「{类}」至少选一种支付方式、覆盖规则「{标题}」至少选一种支付方式、请输入有效的点客比例/金额、请输入有效的散客比例/金额、比例需在 0–100%、名称不能为空、已删除工位、不可新增工位、**经理签单已计入提成、经理签单不计入提成**。
...
23. **Given** 方案 `signComm` 关闭，**When** 计提经理签单订单行（支付方式不在现金/卡付/团购三类），**Then** 该行不计提成（`skipped='sign'`）、无 `winnerSchemeId`。
...
33. **Given** 按工位分配且三工位均有参数，**When** 参数区宽度不足，**Then** 放不下的工位短标段整段换行；数值完整可见。
34. **Given** 任意规则卡，**When** 查看布局，**Then** 左列标题+参数、右列基数段控+支付段控；无整卡 chevron 主路径。
35. **Given** 不分工位规则，**When** 查看参数，**Then** 展示「点」「散」短标 + 数值。
36. **Given** 规则卡，**When** 点击右列「实收」或「卡付」，**Then** 即时切换并置脏；点标题打开 Sheet；点参数区打开 Sheet 并锚定参数。
...
51. **Given** 方案列表任一张方案卡，**When** 查看第二行，**Then** 显示「经理签单计算提成」+ 右侧 switch；默认关闭（灰）。点击切换立即生效并 toast（无需进编辑页）。
52. **Given** 方案 `signComm` 开启，**When** 计提「经理签单 · 深层补水护理」（原价 268 / 实收 0），**Then** 按命中块计提：资深技师方案深层补水覆盖 18% → ¥48.24；不受三类支付范围限制。
53. **Given** 方案 `signComm` 关闭，**When** 查看试算，**Then** 经理签单行金额 0、note「经理签单不计提成」；其余 8 行不变。
54. **Given** 复制已开启 `signComm` 的方案，**When** 查看副本，**Then** `signComm` 保留开启；分配清空。
...
| 独立原型本目录 `index.html` | 唯一可视依据（配置链路）；关联流水/薪资见 §6.8/§6.10（本系统现有页面） |
...
| 签单（经理签单） | 原系统实收 = 0，按此逻辑计算（实收模式下提成自然为 0）；新模型由方案级 `signComm` 控制：原系统 `payScope.sign=true`（参与计提）→ 迁移为 `signComm=true`，否则 `false` |
...
| 经理签单 / 记为欠款 | 新模型 `payScope` **无** `sign` 键（仅现金/卡付/团购三类）；经理签单由方案级 `signComm` 控制（默认 false）。迁移时勿再写 `payScope.sign` |
...
新模型仅三类：默认 `cash / memberCard / groupBuy`（**无 `sign`**）。  
原系统 `payScope.sign=true`（经理签单参与计提）→ 迁移为方案级 `signComm=true`（否则 `false`），保证迁移前后一致。  
