...
53q. **Given** 员工已选「大工 + 顾客指定」，**When** 展开后点「中工」，**Then** **旧勾（大工）先反序收回**、随后**新勾（中工）画出**，动效播完后才收缩，摘要变为 `中工 · 顾客指定`（工位之间**单选**）。
53r. **Given** 员工已勾选，**When** 点击其右侧放大勾选控件，**Then** 该勾**反序收回**、随后该员工**取消选择**（入口摘要回到「服务员工 未选择」）。
