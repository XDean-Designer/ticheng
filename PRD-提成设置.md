...
8. 本期**仅单店**。**本期不做阶梯提成方案的功能**（不在新模型新建 / 编辑阶梯方案，也不把阶梯迁入新模型）；旧「提成设置」中的**阶梯提成**方案 **完整保留并继续生效**（走旧阶梯引擎，见 §13.1）。旧**单项提成**按模块 13 迁为新方案。新老并存时：阶梯员工读旧引擎，已迁单项员工读新引擎。
...
| 分配灵活 | 方案整体分配员工；工位（默认大 / 中 / 小工，可改名 / 新增至最多 3 个，**不可删除**）参与按工位分配 |
...
- 规则设置 Sheet：适用范围（3 类支付方式）、计算基数（原价 / 实收）、分配模式（不分工位 / 按工位）、提成取值（按比例 % / 固定金额 ¥）、点客 / 散客参数（按工位时逐工位）；按工位时工位名行内改名 / 「+ 添加工位」（≤3，不可删除）
- 添加规则项页：类型 Tab（项目 / 产品 / 会员卡）+ 分组筛选 + 多选（含整类分组）；**允许**不同覆盖卡重复添加同一项目 / 产品 / 会员卡（选择页不禁用，名称旁灰色胶囊「已被添加设置 N 次」）
...
- 另含「头衔空态 sheet」「方案空态 sheet」演示入口（强制空态，不改真实 store；创建员工时真实空态 tip+跳转仍保留）
- 试算口径：示例订单行按员工方案集合计算提成（原型试算引擎）
- 流水订单 / 薪资关联口径（§6.8 / §6.10）：**规格保留（本系统现有页面）**
- 关联页面口径对齐：员工薪资明细（业绩统计 / 提成统计）、员工业绩提成明细（见 §6.10；非本包 UI）
- 工作台移除「业绩设置」入口；旧单项提成配置下线；**旧阶梯提成入口与方案保留并继续生效**（兼容现网）；**本期不做新阶梯提成方案能力**（见 §1.4）
...
- 正式提成结算单 / 工资条打印
- 原型的「无数据 / 有数据」造数开关
- **阶梯提成方案功能（本期明确不做）**：不在新模型中新建 / 编辑阶梯方案；不把阶梯迁入新模型；本原型也不交付新阶梯 UI。旧阶梯仅**保留配置并继续走旧引擎生效**（兼容现网，见 §13.1）；新阶梯能力另期设计后再迁
...
| 阶梯提成 | **本期不做新阶梯方案功能**；**不迁移**进新模型；配置保留，结算继续走旧阶梯引擎 |
| 特殊项目单独提成 | 覆盖规则项：指定项目 / 产品 / 会员卡 / 整类分组 |
| 快速消费单独提成 | 第 5 项默认「快速消费」；数据为系统项目覆盖项（`ov_sys_quick`），不吃「项目」默认 |
| 按工位分别设提成 | 分配模式 = 按工位分配，逐工位设点客 / 散客参数；工位在规则 Sheet 内联改名 / 新增（≤3，不可删除） |
...
└─ 弹层：方案命名 / 重命名、删除方案确认、未保存返回、帮助说明；覆盖删除确认 Dialog（左滑垃圾桶 / 长按底部操作 Sheet，§5.5）
```
...
| 添加规则项 | 添加规则项 | 类型 Tab / 分组轨 / 全选·反选 / 下一步；可重复添加（chip 提示 N 次） |
| 规则设置 | 分类名 / 覆盖名 / 添加规则项 · 设置 | 底部「取消 / 确定」；按工位时内联改名 / 添加工位 |
| 分配员工 | 分配员工 | 底部「取消 / 确定」 |
...
删除：左滑露出垃圾桶点删 / 长按底部操作 Sheet「删除」→ 均弹确认 Dialog → 确认后移除，置脏；不可撤销（§5.5）
重命名：长按 Sheet「重命名」→ ≤6 字
选择适用项：长按 Sheet「选择适用项」→ 进入选择页并预勾当前适用项
```

- 覆盖规则的**优先级高于**默认规则（§8 规则 5 解析顺序）；命中覆盖后**不再走**默认分类。
- **允许重复添加**：不同覆盖卡可包含同一项目 / 产品 / 会员卡；选择页**不禁用**；名称旁灰色胶囊「已被添加设置 N 次」（N = 其它覆盖卡引用次数，**不含**当前编辑卡）。
- 同方案多覆盖重叠：按订单行试算金额**取高**；金额并列按规则卡**添加顺序**（先添加者胜出）。

## 4.5 工位

```
默认 3 工位：senior 大工 / mid 中工 / junior 小工（最多 3 个）
改名 ──► 点工位名或右侧 edit-02 图标行内编辑（失焦 / 回车确认）；stationLabels 更新；全局同步本店全部方案
新增 ──► 少于 3 个时底部「+ 添加工位」；自由输入名称（≤6 字）；新工位写入全部方案；各规则块补默认参数
不可删除工位
```

- 改名 / 新增在**规则设置 Sheet · 按工位**内完成；**无**独立「工位设置」Sheet。
- 空名 toast「名称不能为空」；超过 6 字截断或拦截（以原型为准）；达 3 个时隐藏「+ 添加工位」，toast「最多 3 个工位」。
...
## 5.5 覆盖规则：长按操作 Sheet / 左滑删除 / 选择适用项 / 重命名

> 以独立原型为准；删除**必须**确认 Dialog，确认后**不可撤销**（无撤销 toast）。

| 方式 | 行为 |
|------|------|
| **长按操作 Sheet** | 长按约 480ms → 底部操作 Sheet 三项：「选择适用项」/「重命名」/「删除」；**无「取消」行**；点遮罩关闭 |
| **选择适用项** | 进入添加规则项式选择页，**预勾**当前覆盖卡已有适用项；确定后写回 `targets` / 标题，置脏 |
| **重命名** | Dialog / 行内输入，名称 ≤ **6** 字；空名 toast「名称不能为空」 |
| **左滑删除** | 用户覆盖卡外层 `.comm2-rule-swipe-wrap`；向左滑露出右侧垃圾桶 → **点垃圾桶** → 确认 Dialog → 确认后从 `overrides` 移除；未滑开时红区不可见（`opacity:0` + wrap `overflow:hidden`） |
| **Sheet 删除** | 长按 Sheet 点「删除」→ **同一**确认 Dialog → 确认后移除 |
| **确认文案** | 「确认删除该规则项？」「删除后不可恢复。」（以原型为准） |
| **系统项** | 快速消费（`system`）不可删；无操作 Sheet / 左滑宿主 / 重命名入口 |

- 两种删除路径（左滑垃圾桶 / 长按「删除」）均须确认，确认后**不可撤销**。
- 手势进行中消费 click，避免误开 Sheet；与卡面胶囊点击互斥（胶囊 `pointerdown` 不启动手势）。
- 允许不同覆盖卡重复引用同一目标；选择页不禁用已添加项，名称旁灰色胶囊「已被添加设置 N 次」（N = 其它覆盖卡次数，不含当前编辑卡）。
...
| 项 | 说明 |
|----|------|
| 长按 | 底部操作 Sheet 三项：「选择适用项」/「重命名」/「删除」（无取消；点遮罩关闭；§5.5） |
| 删除 | 左滑露出垃圾桶点删，或长按 Sheet「删除」；**均须**确认 Dialog，确认后不可撤销 |
| 系统快消 | 不可删、无左滑宿主、无重命名 / 选择适用项 |
...
| 分配模式 | 分段：不分工位 / 按工位分配 | 按工位时展开工位参数区（内联改名 / 添加工位，见 §6.4）；**无**独立工位设置 Sheet |
...
> **无**专属「工位设置」Sheet；工位管理全部在规则设置 Sheet「按工位」态完成。

| 控件 | 行为 |
|------|------|
| 工位名行 | 工位名文案 + 右侧 edit-02 图标均可点 → 行内改名（失焦 / 回车确认）；maxlength **6** |
| 「+ 添加工位」 | 当前工位数 **少于 3** 时显示于参数区底部；自由输入名称（≤6 字）；达 3 个时隐藏 |
| 删除 | **不可删除**（无删除入口） |

- 改名 / 新增：**全局同步本店全部方案**的 `stationIds` / `stationLabels`；新工位在各方案全部规则块（默认 + 覆盖）写入**默认参数**。
- 空名 toast「名称不能为空」；达上限 toast「最多 3 个工位」。
- 改名 / 新增后：重绘编辑卡片；若规则 Sheet 打开则刷新其参数区。

## 6.5 添加规则项

**入口**：编辑页「+ 添加规则项」；覆盖卡长按「选择适用项」亦进本页（预勾当前适用项）。
...
- **可重复添加**：**不禁用**已在其它覆盖卡出现的项；名称旁灰色胶囊「已被添加设置 N 次」（N = 其它覆盖卡引用次数；从「选择适用项」进入时**不含**当前编辑卡）。
- 底部：取消 / **下一步（N）**（N=已选数；为 0 时禁用；从「选择适用项」进入可为「确定」以原型为准）。
...
- 「选择适用项」路径：预勾后确定 → 写回当前覆盖卡 `targets` / 标题，toast「已更新适用项」（以原型为准）→ 回编辑页。

**覆盖项数据**：`targets` 保存目标（kind / refId / name；整类为 `group` + `groupKind`；会员卡带 `cardRole`）；标题 = 目标名以「、」连接（重命名后以自定义标题为准）。
...
## 6.7 关联页面 · 选择员工 Sheet & 空态演示

> **范围**：**不在 hub 三链路**（左侧导航「关联页面」仅演示）。与提成方案编辑无数据联动。

左侧导航「**关联页面**」区域中入口，与提成方案编辑链路无数据联动，**仅作关联功能演示**。

**工位选择（3 / 2 / 1）**：三者**共用**灰底页 `screen-comm2-staff` 与同一 Sheet 壳，按入口切换 `stationMode`。
...
| 头衔空态 sheet | `staff-empty-role` | 打开创建员工 + 头衔 sheet，**强制空态**（`demoEmptyRolePicker`）；关闭清除 flag |
| 方案空态 sheet | `staff-empty-scheme` | 打开创建员工 + 提成方案 sheet，**强制空态**（`demoEmptySchemePicker`）；关闭清除 flag |
...
**页面宿主（工位 Sheet）**：先切灰底页（`#F7F7F7`），再开 Sheet；不叠在方案列表/编辑上。关闭后回进入前页面。
...
| 分配模式说明 | 不分工位：开单无需选工位，按点客 / 散客统一提成；按工位分配：按大 / 中 / 小工分别设置提成；可点工位名改名、少于 3 个时可添加工位（不可删除） |
...
| 月份 pill | 点击弹出「选择月份 / 选择结算期」sheet（±3 期；自定义周期展示区间副文），切换后列表/明细/业绩提成明细/设奖惩按新期重算 |
...
10. **允许重复添加**：不同覆盖卡可包含同一项目 / 产品 / 会员卡；选择页**不禁用**；名称旁灰色胶囊「已被添加设置 N 次」（N = 其它覆盖卡次数，不含当前编辑卡）。（整类与单项勾选互斥仍见规则 11。）
11. **整类与单项互斥**：添加规则项勾选整类组会清掉组内单项勾选；勾选单项会清掉涵盖它的整类勾选。
12. **工位**：可改名 / 新增（最多 **3** 个）；**不可删除**；改名 / 新增全局同步本店全部方案；新工位各规则块补默认参数。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、反选、清空、重命名、复制方案、删除、改名、分配员工、已分配、未分配、已选 N 项、已选 N 人、还有 N 人未分配任何提成方案、**基数、适用范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、迁移完成、迁移结果提醒、重新创建方案、稍后处理、**关联页面、选择员工（3工位）、选择员工（2工位）、选择员工（1工位）、头衔空态 sheet、方案空态 sheet**、**设置、头衔管理、角色管理、选择头衔、创建员工、邀请员工、查看记录、+ 添加工位、选择适用项、已被添加设置 N 次**。
...
6. **Given** 项目类分配模式「按工位分配」，**When** 进入规则设置 Sheet 按工位态，**Then** 可见工位名行内改名；少于 3 个时底部有「+ 添加工位」；**无**独立工位设置 Sheet。
7. **Given** 规则设置 Sheet 按工位，**When** 点「大工」改名「总监」失焦/回车，**Then** 本店全部方案规则卡按工位参数行显示「总监 X·Y」（全称）；全局同步。
8. **Given** 规则设置 Sheet 按工位且已有 3 个工位，**When** 查看底部，**Then** 无「+ 添加工位」；试图新增则 toast「最多 3 个工位」。
9. **Given** 按工位分配的方案，**When** 查看工位行，**Then** **无**删除入口（工位不可删除）。
10. **Given** 添加规则项（项目 Tab），**When** 勾选项目 A 与项目 B 下一步并确定，**Then** 新增覆盖规则卡「A、B」；再进添加规则项，A、B **仍可选**，名称旁灰胶囊「已被添加设置 1 次」（或对应次数）。
11. **Given** 分组轨选择自定义组 G，**When** 勾选「整类 · G」，**Then** 组内单项勾选被清空；下一步确定后生成整类覆盖；组内单项仍可被其它覆盖卡再次添加（chip 提示次数）。
...
45. **Given** 用户覆盖规则卡，**When** 长按弹出底部操作 Sheet，**Then** 可见「选择适用项 / 重命名 / 删除」且**无取消行**；点遮罩关闭；点「删除」同样走确认 Dialog，确认后不可撤销。
45a. **Given** 用户覆盖规则卡，**When** 长按「选择适用项」，**Then** 进入选择页并预勾当前适用项；「重命名」名称 ≤6 字。
...
68c. **Given** 薪资列表 / 薪资明细，**When** 点月份 pill，**Then** 弹出「选择月份 / 选择结算期」sheet（±3 期）；自定义周期显示区间副文；选定后全链路重算。
...
76. **Given** 本期范围，**When** 查看提成设置能力，**Then** **不交付**新阶梯提成方案功能（无新建/编辑阶梯方案、不迁入新模型）；旧阶梯配置仍可保留并走旧引擎。
...
| **阶梯提成** | **本期不做新阶梯方案功能**；**暂不迁入**新模型；现网阶梯仍走旧引擎 |
| 新「业绩」统计 | 与提成解耦，不参与提成 |

### 13.1.1 阶梯（暂不迁入 · 本期不做新阶梯方案功能）

- **本期明确不做**：不在新模型新建 / 编辑阶梯方案；不把阶梯迁入新模型；本原型不交付新阶梯 UI
- 阶梯配置只读保留，迁移 Job **禁止**改写阶梯表
- 结算：绑阶梯员工 → 旧阶梯引擎；已迁单项员工 → 新引擎
- 新列表可只读展示阶梯标签（「阶梯·沿用旧版」）；编辑进旧阶梯页（兼容现网）
- 新阶梯能力**另期**设计后再迁；本期不安排原型迭代阶梯方案功能
...
```
新: base 按适用范围分摊（原价×范围内实付/全部实付 | 实收=范围内实付 | 固定额×分摊系数）；pct → base×rate/100；amount → 固定¥×分摊
经理签单: 跳过适用范围；baseMode=list → 用原价；paid → 比例结果为 0；amount → 仍取固定额
```
