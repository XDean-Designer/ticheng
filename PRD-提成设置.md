...
| 修订（2026-09-20 二十六次） | **已分配员工的方案被改动 → 保存前必须选定「本期提成怎么算」（新增 §6.13）**。**① 触发**：方案**已分配 ≥1 名员工**（`assigneeIds` 非空）且本次改动**影响金额**（`defaults` / `overrides` / `stationIds` / `stationLabels` 的指纹有变化）时，点「保存」**不直接落库**，先弹 `#comm2RecalcMask`；**纯改名 / 改分配 / 无改动不弹**（`schemeMoneyChanged()`）。**② 三种口径**：`recalc` 本期全按新方案重算（**人工改过的值仍保留**）· `forward` 算好的不动、从改动时刻起用新方案（**默认选中**）· `next` 本期整期仍按老规则、下个结算周期起用新方案。**③ 「本期」的正确定义**：= **含今天的那一期**（`getPeriodContaining(new Date())`），与「正在查看的期」**区分开** —— 月份 pill 可切 ±3 期，但改动重算问的是当下这一期；三个选项说明里的区间/期名**动态注入真实日期**。**④ 数据模型（§7.1 新增 4 字段）**：`effectiveMode`（`recalc`/`forward`/`next`）· `effectiveAt`（分界日）· `effectiveFrom`（生效期 key）· `_prev`（改动前的规则集快照：`defaults` / `overrides` / `stationIds` / `stationLabels`）。**⑤ 解析（关键设计）**：新增 `schForLine(sch, periodKey, ymd)` —— 老口径期间返回**套了 `_prev` 的影子对象**（`Object.assign({}, sch, _prev)`），因此试算链路 `calcStaffTrial → schemeLineAmount` **只需换掉传入的方案对象，其余零改动**（`schemeLineAmount` / `resolveLineBlock` / `lineRateMeta` / `lineBaseSide` 全部原样复用）；`calcStaffTrial(staffId, lines, periodKey)` 新增第 3 参，薪资侧 `finalizeSeedCommLines` 传入 `state.salaryMonth` 并把每行 `ymd` 带进 `trialPayload`。**⑥ 落库与重算**：`notifySalarySync()` → `invalidateCommLineCache()` 是**既有能力**（重建本期提成行、先把 `commManual` / `commEditLogs` 的行原值搬进 `_commManualPreserve` 再回填）—— 因此「重算但保护人工值」**无需新增代码**，`recalc` 只靠 `effectiveMode` 让本期全部走新规则。**⑦ UI**：居中 Dialog（沿用 ModalPopup 规范 `max-width 326px`）＝标题「**改方案后，提成怎么计算？**」+ 说明「这个方案有 **N** 名员工在用，改动会影响他们本期已经算出来的提成。」+ **影响面**（本期区间 · 已算好 **M** 条）+ **3 张单选卡**（借用结算周期页 `.emp-pay-cycle-opt` 的视觉语言，但样式定义在 `comm2.css` 内，避免 comm2 与 salary 两模块互相依赖）+ 提示「已经手工调整过的单子，金额不受影响。」+ 底部单按钮「确定」。**必须选一项**：**无取消按钮、点遮罩空白与按 Esc 均不关闭**（全仓无通用 outside-close，本弹窗不接线即天然成立）。**⑧ 列表页角标**：`next` 口径下方案卡加「**下期生效**」（`#FFF3E8` / `#C2701A` / `11px`）—— 本期仍按老规则跑时，明确告知用户改动已存下、下期才生效。**⑨ 文案**（面向文化水平有限的小店主 / 店员，**全篇无「结算周期 / 落地 / 生效口径 / 重算引擎」等术语**）：选项1「**本期全部重算**」/「本期（9.01–9.30）已经算出来的提成，按新规则重算一遍 —— 金额会变」；选项2「**算好的不动，从现在起用新方案**」/「之前已经算出来的保持原样；从现在开始的新单子按新规则算」；选项3「**本期先不动，下期再用新方案**」/「本期（9.01–9.30）整期还按老规则算，本次发薪不受影响；2026年10月起用新规则」。**⑩ 新建抓取路由**：`comm2-recalc`（改计算基数 → 保存 → 弹窗）与 `comm2-recalc-next`（再选第 3 项并确定 → 列表页「下期生效」角标）。**实测**（Edge headless 390×880）：新建 `_tmp_probe_recalc.mjs` **44 项全 PASS** —— 触发（无改动保存不弹 / 改「计算基数」则弹）、形态与文案（标题含方案名、说明含「N 名员工在用」、影响面 2 项、3 张卡、默认 `forward`、每卡有单选控件、`max-width 326px`、`z-index 50`）、必须选一项（遮罩空白 / Esc 均不关）、切换（选中态唯一 + `aria-checked` 同步）、落库（`effectiveMode=next` / `_prev` 2043 字节 / `effectiveFrom` = 下一期 `2026-10` / 不写 `effectiveAt` / 角标「下期生效」）、非金额改动（纯改名不弹）、**三口径计算差异**（两行 1000 元、老 10% / 新 20%：`recalc` **400**、`forward` **300**〔改前 100 + 改后 200〕、`next` **200**；`forward` 在期内**按日期切分**、`recalc` 不切分、**无 `_prev` 的历史方案一律新规则**）；`_tmp_smoke.js` **ALL PASS**（计提口径无回归）。**⑪ 已知既有欠账（非本次引入）**：`_tmp_ui_verify.mjs` 的 `D7` / `D12` / `D13` / `I2` / `I3` 在 HEAD 上**同样失败** —— 这 5 项是**为「工位三选一 + 换工位替换」写的**，二十五次改多选后未同步（`D12` 仍断言「切到中工只选中工」），**待与多选口径一并重写**。同步 §6.13（新增）/ §7.1 / §8 规则 33 / §9.1 / §10.2（新增 `56o` / `56o2` / `56o3` / `56o4`）/ **开发版 PRD**。 |
| 修订（2026-09-20 三十一次） | **改动重算弹窗「三选一 → 两枚按钮即两个结果」（领导拍板取消「本期不动，下期换」）**。**① 口径收敛**：数据侧只剩 `recalc` / `forward` 两种（**删除第三口径 `next`**），`§6.13.2` 由「三种口径」改为「两种口径」，切分粒度只剩「`forward` 按行日期切分 / `recalc` 不切分」。**② UI 重做**：删除开关行（`#comm2RecalcToggle` / `#comm2RecalcToggleLbl` / `.comm2-recalc__row*` / `.comm2-recalc__switch*`）与两枚并排按钮（`#comm2RecalcNext` / `#comm2RecalcNow`），改为**两枚竖排按钮** —— 上「**本期全部重算**」（描边次要，落 `recalc`）、下「**从现在起用新方案**」（品牌主按钮，落 `forward`），新增容器修饰类 `.dialog__actions--stack`（`base.css`：`flex-direction: column` + 按钮 `width: 100%`）。实测两枚按钮**均 `284px` 宽、左缘对齐、不溢出、不折行**。**③ 无间接层**：只剩两个结果时，开关会多出一层「开关＝什么」的推断 —— 按钮把结果直接写在按钮上；`recalcNowLabel()` / `recalcSyncUi()` 与 `recalcState.scope` 随之删除（`recalcState` 收窄为 `{ sch, before }`）。**④ 连带清理（不留死代码）**：`schForLine()` 的 `next` 分支、`cmNextPeriod()`、`schemePendingNext()` 与列表页「下期生效」角标（`comm2.js` 渲染分支 + `.comm2-scheme__badge` 样式整段删）、`COMM2_EFFECTIVE_LABEL`（本就没用）、`applyEffectiveChoice()` 的 `effectiveFrom` 写入、`EmployeeDemo.nextPeriodInfo()`（已无调用方）、`#comm2RecalcRow` 相关 DOM —— 全部删除；`§6.13.5` **整节作废**；`§7.1` 字段由 4 个收为 **3 个**（`effectiveMode` / `effectiveAt` / `_prev`）。**⑤ 保留不变**：触发条件（已分配员工 + 影响金额）、`_prev` 快照与 `schForLine` 影子对象、人工修正恒优先、「本期 = 含今天那一期」的口径来源、**必须点一个**（无取消、遮罩与 Esc 均不关闭）、**未回答就退出仍兜底 `forward` + toast**（见 §6.13.8）。**⑥ 新增备案**：`§6.13.7` 增「点错按钮**无撤销入口**」一行 —— 这是「按钮即结果」的固有代价，挽救路径 = 再改一个金额字段保存、**会再次弹窗**重选。**⑦ 抓取路由**：`comm2-recalc-next` / `comm2-recalc-on` → **`comm2-recalc-all`**（点上按钮）+ **`comm2-recalc-forward`**（点下按钮）。**实测**：重写 `_tmp_probe_recalc.mjs` **62 项全 PASS**（形态 19 项含「开关节点与样式全零残留」「column 竖排 / 等宽对齐 / 撑满 / 不溢出 / 不折行」「字段里不再有 `effectiveFrom`」；两按钮各落各的口径；`recalc` 400 / `forward` 300；兜底三条路径 + 幂等 + 两按钮不被兜底二次触发）。**⑧ 文案与术语**：`§9.1` 文案清单改写；`§10.2` 改写 `56o2` / `56o3` / `56o4` / `56o5` / `56o7`；`§8` 规则 33 与 `62` 处「本期」口径说明同步。同步 **§6.13.2 / §6.13.3 / §6.13.4 / §6.13.5（作废）/ §6.13.6 / §6.13.7 / §7.1 / §8 规则 33 / §9.1 / §10.2 / 开发版 PRD**。 |
| 修订（2026-09-20 三十二次） | **改动重算弹窗的提示行改为水平居中**。`§6.13.3` 的提示（ⓘ + 「已经手工改过的订单，金额不受影响。」）原为**左对齐**，改为**水平居中** —— `.comm2-recalc__note` 增 `justify-content: center`（图标 + 文字**作为一个整体**居中，不是只居中文字、图标留在左侧）+ `text-align: center`（万一在更窄屏折行，两行文字各自居中）。实测（Edge headless 390×880）：弹窗内容宽 `284px`、提示行**左右留白各 `30px`**、整体**偏离中心 `0px`**、单行不折行。同步 §6.13.3 / 开发版 PRD。 |
| 修订（2026-09-20 三十次） | **重算弹窗标题改为固定文案**：`改「{方案名}」后，提成怎么算？` → **`改方案后，提成怎么计算？`** —— 标题**不再引用方案名**（店家在编辑页里本就知道自己改的是哪个方案，标题重复一遍方案名只会让长名称挤成两行）；`openRecalcDialog()` 里对应的动态赋值整行删除，标题直接由 `index.html` 静态承载（`#comm2RecalcTitle`）。同步 §6.13.3（表格首行改为「标题（**固定**，不引用方案名）」）/ §9.1 / §10.2（改写 `56o2`）/ 开发版 PRD。 |
| 修订（2026-09-20 二十九次） | **弹窗文案定稿 + 未回答即退出的兜底口径**。**① 兜底默认（新增 §6.13.8）**：弹窗悬着未回答时，任何「未选择就消失」的路径（`pagehide` / `beforeunload`、返回按钮 `requestComm2Exit`、任何回列表的 `openList()`）一律按**默认口径 `forward`** 落库 —— 理由：用户**已点过「保存」**，改动的意图已表达，既不能丢改动也不能默认动账。新增 `recalcFallbackForward()`（幂等：先 `clearRecalcPending()` 再落库）与 `clearRecalcPending()`；兜底后 toast「已按「从现在起用新方案」保存」；**不留** `_autoDefault` 之类标记，与用户手动选「否」完全等价；点返回按钮时**不再弹「尚未保存」**。**② 左按钮文案**：「本期不动，下期**再**换」→「**本期不动，下期换**」—— 实测弹窗内容宽 `284px`、按钮内宽 `124px`，原文案 9 字 × 14px = 126px **会折行**（第二行只剩「换」），去掉「再」后 8 字 = 112px **单行**。**③ 提示文案**：「已经手工**调整过的单子**」→「**已经手工改过的订单**」。**④ 字号**：保持 `14px`（16px × 8 字 = 128px > 124px 会折行）。同步 §6.13.3 / **§6.13.8（新增）** / §6.13.7（删掉随影响面一起失效的表述）/ §9.1 / §10.2（改写 `56o2` / `56o3` / `56o6` 文案，**新增 `56o7`**）/ 开发版 PRD。 |
| 修订（2026-09-20 二十八次） | **改动重算弹窗改版：三选一 → 「一个开关 + 两个按钮」**（§6.13.3 重写）。**① 删除影响面**：弹窗不再展示任何影响信息 —— 「这个方案有 N 名员工在用…」说明行、灰条「本期 {起}–{止} · 已算好 {M} 条」**整段删除**，连带删掉只为统计条数而存在的 `EmployeeDemo.countEffectiveCommLines()`（`employee.js` 无残余）。**② 开关取代三选一**：`#comm2RecalcToggle`（`role="switch"`、`44×26`、`22px` 白滑块、关 `#E5E5E5` → 开 `var(--brand)`、滑块 `translateX(18px)`），问句「**本期已经算好的提成，要重新算吗？**」，**默认关**；开 = `recalc`（本期已算好的重算），关 = `forward`（保持原样、从现在起用新方案）。**③ 两个按钮**：左「**本期不动，下期再换**」（描边次要）固定落 `next`；右「**立即重新计算**」/「**从现在起用新方案**」（品牌主按钮）**文案随开关联动**（`recalcSyncUi()`），落开关所指口径 —— 关闭态不写「重新计算」以免误导。**④ 提示加图标**：底部「已经手工调整过的单子，金额不受影响。」左侧加 `14×14` 灰 ⓘ（内联 SVG），文字与规则**不变**。**⑤ 保留**：必须二选一（无取消、遮罩与 Esc 均不关闭）、`_prev` 快照、`next` 的「下期生效」角标、「本期 = 含今天那一期」的口径来源、人工修正恒优先。**⑥ 连带**：旧节点 `#comm2RecalcLead` / `#comm2RecalcImpact` / `#comm2RecalcOpts` / `#comm2RecalcOk` 与 `.comm2-recalc__opt*` / `__impact*` / `__lead` 样式、`RECALC_OPTS` / `recalcOptsHtml()` / `recalcImpactHtml()` 全部删除；`app.js` 抓取路由改接新按钮并新增 `comm2-recalc-on`（开关打开态）。同步 §6.13.3 / §6.13.4 / §6.13.6 / §9.1 / §10.2（改写 `56o2` / `56o3`，**新增 `56o5` / `56o6`**）/ **开发版 PRD**。 |
| 修订（2026-09-20 二十七次） | **回归套件修账（二十五一～二十六次遗留）**：`_tmp_ui_verify.mjs` 的 `D7` / `D12` / `D13` / `I2` / `I3` 五项原为「工位三选一 + 换工位替换」时代所写，二十五次改多选后未同步。按多选口径重写：`D7` → hint 断言含「**工位可多选**」；`D12` → **工位多选追加**（大工 · 中工 · 顾客指定，旧勾保留）；`D13` → **逐个点掉全部已勾选项**才完全取消该员工；`I2` / `I3` → 由「换工位 → 旧卡并行红色交接」改为「**加选工位 → 旧勾全程保持红色、不走反向收勾**」（A8 已按二十五次废止红色交接）。全套 **0 失败**。**产品代码零改动**（仅校验脚本与文档）。同步 §6.13.7（「已知欠账」改写为「回归验证（二十六次一并修账）」）/ **开发版 PRD**。 |
...
| 上按钮（**描边次要**，`#comm2RecalcAll`） | 本期全部重算 |
| 下按钮（**品牌主按钮**，`#comm2RecalcForward`） | 从现在起用新方案 |
...
> **回归验证（二十六次一并修账）**：`_tmp_ui_verify.mjs` 的 `D7` / `D12` / `D13` / `I2` / `I3` 五项原本是**为「工位三选一 + 换工位替换」写的**（二十五次改为多选后未同步，与本功能无关）。本轮已按多选口径重写：`D7` 断言 hint 含「**工位可多选**」；`D12` 断言**工位多选追加**（大工 · 中工 · 顾客指定，旧勾保留）；`D13` 改为**逐个点掉全部已勾选项**才完全取消该员工；`I2` / `I3` 由「换工位 → 旧卡并行红色交接」改为**「加选工位 → 旧勾全程保持红色、不走反向收勾」**（A8 已按二十五次废止红色交接）。现全套 **0 失败**。
