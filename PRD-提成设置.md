...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 员工列表 | 员工管理 | 行拖动柄 · 头像 · 姓名 · 状态 chip · 副文；incomplete「去完善」；底部「+ 创建员工」；底栏「头衔管理 / 角色管理」 |
...
| 状态 | 说明 |
|------|------|
| 创建 | 仅「+ 创建员工」进入；无状态字段（固定「在岗」）；性别/年龄/年限隐藏；头衔/提成方案空态可跳转 |
...
**入口**：员工薪资列表 → 点员工卡进入薪资明细；点员工卡 **2×2 提成格**可**直达**员工业绩提成明细；业绩提成明细亦可从左侧导航进入。

**返回栈（2×2 捷径）**：从列表 **2×2 提成格**进入「员工业绩提成明细」后，标题栏返回 → **员工薪资**列表；**不经**「员工薪资明细」。
...
|------|------|
| 入口 | 薪资列表点员工卡（卡体） |
| 顶栏 | 标题「员工薪资明细」；**无**「业绩提成明细」次级按钮（逐笔页由列表 2×2 / 侧栏进入） |
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、反选、清空、重命名、复制方案、删除、改名、恢复为默认、分配员工、已分配、未分配、已选 N 项、已选 N 人、还有 N 人未分配任何提成方案、**基数、范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、迁移完成、迁移结果提醒、重新创建方案、稍后处理、**关联页面、选择员工（3工位）、选择员工（2工位）、选择员工（1工位）、头衔空态 sheet、方案空态 sheet**、**头衔管理、角色管理、选择头衔、+ 创建员工、查看记录**。
...
69. **Given** 薪资明细，**When** 查看顶栏与服务/售卡卡，**Then** **无**「业绩提成明细」次级按钮；点业绩/提成/服务/**售卡**旁 **?** 可看字段释义（服务与售卡为同一弹窗）；金额标签为「金额」（无「（实收）」字样）。
