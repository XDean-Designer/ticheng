...
| 修订（2026-09-15） | 规则卡只读整卡进全量 Sheet；`guestSplit` 单提成/点客；删参数精简与选择页工具条；员工列表「头衔/角色」；「社区形象」卡 |
...
- 规则设置 Sheet：适用范围、计算基数、分配模式、提成取值；**默认单「提成」**，可选「设置点客提成」（`guestSplit`）；按工位时工位名内联改名（固定 3，不可增减；全局同步）
- 添加规则项页：类型 Tab + 分组筛选 + 多选；**无**全选/已选 N/反选工具行；允许重复添加同一目标（**无**次数标签）
- 分配员工 Sheet（员工池多选、全选、清空；已在其他方案者可勾选，副文为 **file-multiple 图标 + 方案名**）
- 规则卡：基数/适用范围**只读**；**整卡**进全量 Sheet；最右「＞」；**无**参数精简；覆盖左滑删；固定金额时基数区显「固定」
...
- **hub 链路二 · 员工管理**：员工列表（拖动排序；标题栏右侧「头衔/角色」主文字色下拉：头衔管理 / 角色管理；底栏次「创建员工」+ 主「邀请员工」；**无**列表内虚线「+ 创建员工」；点店主 → **我的资料**，点他人 → 员工详情）/ **头衔管理** / **角色管理**（12 项敏感权限按角色配置）/ 员工详情 / **我的资料**（同详情内容；**不做**线上选填项：个人项目价格 / 个人说明）/ 创建·编辑·完善员工；详情/表单底部次要「社区形象」卡（剑号/宝剑）；**头衔**与**角色**分离；敏感权限**完全跟随角色**（`EmployeeStore.roleSensitivePerms`），**无**个人覆盖；员工表单**无**敏感权限区
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 员工列表 | 员工管理 | 行拖动柄 · 头像 · 姓名 · 状态 chip · 副文；incomplete「去完善」；标题栏右侧「头衔/角色」（主文字色）下拉（头衔管理 / 角色管理）；底栏次「创建员工」+ 主「邀请员工」 |
...
规则卡为**两行主区 + 最右 chevron 列**（默认左竖条品牌色；覆盖 `#FF8A3D`；用户覆盖外包左滑容器）：

```
┌─▌ 标题（≤6 字完整）            [原价|实收]/固定」  │＞│
│   12%  或  提成 10%    点 点客提成 15%   [现金][卡付][团购] │＞│
└─▌                                                      ┘

guestSplit=false：参数区只显示一个数值
guestSplit=true：左「提成」+ 右「点」标签「点客提成」；工位态竖排（未开点客每工位一值；开点客为 值·值）
```

| 行 | 左 | 中（120px 槽） | 最右 |
|----|----|----------------|------|
| **行1** | 标题（图标+文案） | 原价/实收连体段控外观（或「固定」）**只读**；**无**「基数」文案 | 「＞」相对整卡**垂直居中** |
| **行2** | 提成参数；按工位时竖排；`flex:1`；**参数区左缘与标题图标左缘对齐** | 三 chip 均分单行**只读**；与参数间距 ≥10px，**禁止重叠** | （同列） |

| 热区 | 行为 |
|------|------|
| **整卡**（含标题/参数/只读控件/＞） | 打开**全量**规则设置 Sheet；**无**参数精简入口 |
| 基数 / 适用范围 | **不可**在卡上直改；仅 Sheet 内可改 |
...
| 规则 | 说明 |
|------|------|
| 单提成 | `guestSplit=false`：只显示**一个数值**（比例带 `%`，固定带 `¥`） |
| 开启点客 | `guestSplit=true`：摘要形如 `10%·点10%`（前为散客/提成，后为点客；「点」为小标签） |
| 按工位 | 未开点客：工位全称 + 单值竖排；开点客：工位全称 + `10%·点10%` 竖排三行 |
...
| 控件 | 样式 |
|------|------|
| 基数段控 | 连体胶囊外观；选中白底 + 字色 `#E89D64`；卡面**只读**；卡面**无**「基数」文案 |
| 适用范围 chip | 独立圆角 **8px**、高 **26px**、字号 11px；卡面**只读**；右侧槽 **120px**、三 chip 均分**强制单行** |
| 固定金额 | 基数区只读灰标「固定」 |
| 整卡 chevron | 最右一列，相对整卡垂直居中 |
...
| 提成取值 | 分段：按比例 % / 固定金额 ¥ | 输入框内用 `%`/`¥` 或 placeholder；**无**「比例」「金额」字段标签 |
...
| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | 仅一张「提成」输入卡；文字链「设置点客提成」（品牌色 + 小 chevron） | 点客/散客（及各工位）写**同一数值** |
| 开 `true` | 「提成」改名为「散客」；另出「点客」卡；点客卡标题旁 **×** 关闭收起 | 两端可不同 |
| 旧数据 | 若点客≠散客（含任一工位） | **自动视为已开启**（显示两卡） |
| 按工位 + 关 | 每工位只填/显示一个参数 | 开单选员工**不再**选点客/散客（联调页须隐藏） |
...
| 分配模式说明 | 不分工位：按点客 / 散客统一提成；按工位分配：固定大 / 中 / 小工分别设置提成；可点工位名改名（全局同步，不可增减）。**开单是否展示「选择工位」**不单看当前规则卡，而按 **§4.5.1 类内 OR · 门店维度**（默认卡 + 该类全部覆盖，任一为按工位 → 该类整类需选工位） |
...
| guestSplit | boolean | 是否拆分点客/散客；默认 `false`；旧数据两端不等时自动为 `true` |
| designated | number | 点客比例（pct）；`guestSplit=false` 时与散客同值 |
| nonDesignated | number | 散客/统一提成比例（pct） |
| designatedAmt | number | 点客金额（amount） |
| nonDesignatedAmt | number | 散客/统一提成金额（amount） |
...
原型内置 9 条示例订单行（`COMM2_TRIAL_LINES`）用于演示与验证：开卡·尊享组合卡、充卡·老客续充、深层补水护理、染发、剑琅玻尿酸精华液、团购体验·洗头、卡付·时尚洗吹、**快捷开单**、**经理签单·深层补水护理**（原价 268 / 实收 0，`sign=true`）。
...
5. **Given** 项目类固定金额且已设置点客提成，**When** 输入散客 ¥3、点客 ¥5 保存，**Then** 规则卡显示「提成¥3」与「点 点客提成¥5」。
...
34. **Given** 任意规则卡，**When** 查看布局，**Then** 行1=标题+基数、行2=参数+适用范围；最右「＞」垂直居中；整卡可点进全量 Sheet。
35. **Given** 不分工位且未开点客，**When** 查看参数，**Then** 只显示一个数值；开启点客后左「提成」右「点客提成」（前有「点」标签）。
36. **Given** 规则卡，**When** 点击基数段控或适用范围 chip，**Then** 无直改；点击整卡打开全量 Sheet（无参数精简）。
...
51. **Given** 规则卡，**When** 查看布局，**Then** 行1=标题+只读基数、行2=参数+只读适用范围；右侧槽 120px；最右「＞」；≤6 字标题完整显示。
...
56d. **Given** 未开点客的规则，**When** 打开 Sheet，**Then** 仅一张「提成」卡 +「设置点客提成」文字链；确定后点客=散客。
56e. **Given** 已开点客，**When** 点点客卡上的 ×，**Then** 回到单「提成」，两端写同一数值（保留散客侧）。
...
58. **Given** 员工列表，**When** 查看标题栏与底栏，**Then** 标题栏右侧有主文字色「头衔/角色」下拉（头衔管理 / 角色管理）；底栏次「创建员工」+ 主「邀请员工」；**无**列表内虚线「+ 创建员工」。
