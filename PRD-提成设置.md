...
- 规则设置 Sheet：适用范围、计算基数、分配模式、提成取值；**默认单「提成」卡** + 「+ 点客提成」虚线条（`guestSplit`）；按工位时工位名内联改名（固定 3，不可增减；全局同步）
...
- **开单侧「选择服务员工」口径**（跨系统约束；**仅锁口径，不在本仓库改开单代码**）：按**业务类**判定（类内 OR · 门店维度）——该类任一启用方案块为按工位 → 整类需选**工位**（§4.5.1）；该类**未开启「点客提成」** → 不再像旧版一样必选「**点客 / 散客**」（§4.5.2）；该类**既未开按工位、也未开点客提成** → **点击员工即完成勾选**，无二次选择、**不播放**「员工变为按钮」的展开动效（§4.5.2）
...
| 按工位分别设提成 | 分配模式 = 按工位分配，逐工位设提成 / 点客参数；工位在规则 Sheet 内联改名（固定 3，不可增减；全局同步） |
| 开单选员工必选「点客 / 散客」 | 按业务类判定：该类未开启「点客提成」→ **不再采集**「点客 / 散客」；既未开按工位、也未开点客提成 → **点选员工即完成勾选**（无展开动效）（§4.5.2） |
...
| 按工位分配 | 大 / 中 / 小工分别设提成·点客参数 | 规则卡显示逐工位参数；改名全局同步本店全部方案 |
...
| 提成取值 `valueMode` | `pct` 比例 / `amount` 固定金额 | 输入框用 `%`/`¥`；`guestSplit=false` 时两端同值；**固定金额时规则卡计算基数标签显示「固定」** |
...
### 4.5.1 跨系统约束：开单是否展示「选择工位」（类内 OR · 门店维度）

> **约束对象**：开单侧「选择服务员工」及相关嵌入式选人交互。本期**仅在本 PRD 锁定口径**；开单原型/实现按本条联调，**不在本仓库改开单代码**。

**判定范围（门店维度）**：对本店全部**启用中**的提成方案做判断（**不**按当前选中的服务员工过滤方案集合）。

**类内 OR（按业务类）**：对某一业务类（项目 / 产品 / 办卡 / 充卡 / 快捷开单），同时考察：

1. 该类**默认卡**的 `pickMode`；
2. 该方案内、归属该类的全部**覆盖规则项**的 `pickMode`（覆盖归属：项目类覆盖影响「项目」、产品类覆盖影响「产品」，依此类推；系统快捷开单覆盖归属「快捷开单」）。

若上述任一处为 `pickMode=station`（按工位），则该类在开单选服务员工时**整类**需要选择工位（该类下所有可点选的项目/产品等，而非仅命中那条覆盖的目标）。

**示例**：默认「项目」=`avg`（不分工位），但存在一条**项目类覆盖**=`station` → 开单选择服务员工时，**全部项目**均需选工位。

**对称**：若默认已是 `station`，即便该类全部覆盖均为 `avg`，整类仍需选工位（OR）。

**何时展示**：必须在「选择服务员工」时提供工位选择；若「加入购物车」等 Sheet **内嵌**了选服务员工，同一规则一并生效。

**与计提取值的关系**：本条只约束开单 UI 是否采集工位；计提仍按 §8 规则 7（命中块自身的 `pickMode` / 工位缺省回落）。

### 4.5.2 跨系统约束：开单「选择服务员工」的点客/散客采集与交互降级（类内 OR · 门店维度）

> **约束对象**：开单侧「选择服务员工」及相关嵌入式选人交互。本期**仅在本 PRD 锁定口径**；开单原型/实现按本条联调，**不在本仓库改开单代码**。与 §4.5.1 同框架、同判定范围。

**判定范围（门店维度）**：对本店全部**启用中**的提成方案做判断（**不**按当前选中的服务员工过滤方案集合）。

**类内 OR（按业务类）**：对某一业务类（项目 / 产品 / 办卡 / 充卡 / 快捷开单），同时考察该类**默认卡**与该方案内归属该类的**全部覆盖规则项**（覆盖归属：项目类覆盖影响「项目」、产品类覆盖影响「产品」，依此类推；系统快捷开单覆盖归属「快捷开单」）的两个维度：

1. **点客提成** `rule.guestSplit` 是否为 `true`；
2. **分配模式** `pickMode` 是否为 `station`（按工位）。

**四态矩阵**（判定结果作用于该**整类**，即该类下所有可点选的项目 / 产品 / 会员卡等，而非仅命中那条覆盖的目标）：

| 该类 `guestSplit` | 该类 `pickMode` | 开单「选择服务员工」交互 |
|-------------------|-----------------|--------------------------|
| 全部为 `false` | 全部为 `avg` | **点击员工即完成勾选**：不展开任何二次选择、**不播放**「员工变为按钮」的展开动效；该员工按「提成」单值计提 |
| 全部为 `false` | 任一为 `station` | 需选**工位**（§4.5.1）；**不需要**选「点客 / 散客」 |
| 任一为 `true` | 全部为 `avg` | 需选「**点客 / 散客**」；不需要选工位 |
| 任一为 `true` | 任一为 `station` | 需选「**点客 / 散客**」+ **工位** |

**口径要点**

- **不再必选「点客 / 散客」**：旧版开单选服务员工时一律要标记该员工是「点客」还是「散客」；新版仅在**该业务类存在启用中的 `guestSplit=true` 规则块**时才采集。任一处开启即整类需要（OR），与 §4.5.1 的对称性一致。
- **交互降级（点选即勾选）**：仅当该类**同时**满足「`guestSplit` 全为 `false`」且「`pickMode` 全为 `avg`」时成立。此时点选员工直接完成勾选，**不展开**「员工变为按钮」的动效，也不需要任何二次选择。
- **只要有一个维度需要采集，就保留展开交互**：该类 `guestSplit` 任一为 `true` **或** `pickMode` 任一为 `station` → 仍播放展开（变为按钮）动效，用于采集「点客 / 散客」和 / 或「工位」。
- **内嵌选人一并生效**：若「加入购物车」等 Sheet **内嵌**了选服务员工，同一规则一并生效。
- **未采集时的落库与计提**：该类未采集「点客 / 散客」时，订单行该员工**不标记点客**（`staffDesignated` 为非点客），按命中块的 `nonDesignated`（「提成」侧）取值；计提仍按 §8 规则 7 / 规则 7b。
- **旧数据兼容**：`guestSplit` 缺省判定沿用 §6.3（两端值不等 → 自动视为已开启），即旧数据若点客 ≠ 提成，该类仍需采集「点客 / 散客」。
...
┌─▌ 标题（≤6 字完整）                          │＞│   ← 卡头
│ ────────────────────────────────────────────────  │   ← 1px #F2F2F2
│ 提成 10% │ 点12%                 [原价|实收]/固定   │   ← 内容区
│ 大工 10% │ 点12%                 [现金][卡付][团购] │
└─▌                                                  ┘

标签（不分工位=「提成」；按工位=工位名）与数值同为 5px 间距
guestSplit=false：`提成 10%`（标签 + 单值）
guestSplit=true：`提成 10% │ 点12%`（前为提成值、后为点客值；中间为 1px 竖线分隔；「点」小标签在点客数值**左侧**）
按工位：每行「工位名 + 同格式」（未开点客 `大工 10%`；开点客 `大工 10% │ 点12%`）
```

| 区域 | 左 | 右（120px 槽） |
|------|----|----------------|
| **卡头** | 分类图标 + 标题（≤6 字完整） | 「＞」16px（`#C2C2C2`；**不再**放在卡最右） |
| **内容区** | 提成参数（标签 + 数值；不分工位标签为「提成」、按工位为工位名，同款样式）；按工位时竖排；`flex:1`；**参数区左缘与标题图标左缘对齐**；**参数区顶部与右栏首个控件顶部对齐** | 竖排两控件（**可直接点选**）：上「原价/实收」连体段控（或只读「固定」），下「现金/卡付/团购」三 chip 均分单行 |

| 热区 | 行为 |
|------|------|
| **整卡**（卡头 / 参数 / ＞ / 空白区） | 打开**全量**规则设置 Sheet；**无**参数精简入口 |
| 「原价/实收」段控 | **卡上可直改** `baseMode`；点后仅切换、**不**打开 Sheet；固定金额时为只读灰标「固定」（点它仍进 Sheet） |
...
| 开启点客 | `guestSplit=true`：`提成 10% │ 点12%`（前为提成值，后为点客值；中间 **1px 竖线**分隔；「点」为小标签，位于点客数值**左侧**，标签中轴与数值中轴对齐） |
| 按工位 | 工位名（同款标签）+ 同格式：未开点客 `大工 10%`；开点客 `大工 10% │ 点12%` 竖排三行 |
...
| 固定金额 | 基数区只读灰标「固定」（不可点选，进 Sheet 改回比例） |
...
| 入口 | 含适用范围/基数 | 其他区 |
|------|:-----------:|--------|
| 整卡点击；「添加规则项 · 设置」 | ✓ | 分配模式 + 会员卡（覆盖卡时）+ 提成取值 + 提成/点客参数（+工位） |

| 区 | 控件 | 说明 |
|----|------|------|
| 适用范围 | chips 多选（现金 / 卡付 / 团购） | 至少 1 类；按项金额分摊（见 §8） |
| 计算基数 | 分段：原价模式 / 实收模式 | 固定金额时卡上仍显「固定」 |
| 分配模式 | 分段：不分工位 / 按工位分配 | 按工位时展开工位参数区（固定 3 工位内联改名，见 §6.4） |
| 会员卡 | 分段：办卡 / 充卡 | **仅**覆盖规则且目标为会员卡时显示 |
| 提成取值 | 分段：按比例 % / 固定金额 ¥ | 输入框内用 `%`/`¥` 或 placeholder；**无**「比例」「金额」字段标签；比例输入框**按数值宽度自适应**，`%` **紧贴数值右侧**（固定金额为 `¥` 前缀，不变） |
...
| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | 左「提成」输入卡 + 右**虚线卡**「+ 点客提成」（灰虚线 `#D0D0D0`、灰字 `#929292`、与提成卡**等比并排等高**）；卡片名恒为「提成」 | 提成侧（`nonDesignated`）与各工位写**同一数值** |
| 点虚线卡 | 全局 `guestSplit=true`：右卡变为「点客」卡（标题旁 **×** 取消收起）；按工位时**每个工位行**各出现一张虚线卡，点任一即全局开启 | 两端可不同 |
| 开 `true` | 左「提成」+ 右「点客」等比并排；点客卡 **×** 收起后回到虚线卡 | 提成侧仍存 `nonDesignated`（**不再**改名为「散客」） |
| 旧数据 | 若点客≠提成（含任一工位） | **自动视为已开启**（显示两卡） |
| 按工位 + 关 | 每工位只填/显示一个参数（提成卡 + 虚线条并排） | 开单选员工**不再**选「点客 / 散客」（见 §4.5.2） |
...
| 分配模式说明 | 不分工位：开单不分工位，统一按提成计提、点客可单独设置；按工位分配：固定大 / 中 / 小工分别设置提成；可点工位名改名（全局同步，不可增减）。**开单是否展示「选择工位」**不单看当前规则卡，而按 **§4.5.1 类内 OR · 门店维度**（默认卡 + 该类全部覆盖，任一为按工位 → 该类整类需选工位） |
...
| designated | number | 点客比例（pct）；`guestSplit=false` 时与提成侧同值 |
| nonDesignated | number | 提成（原「散客」）比例（pct） |
| designatedAmt | number | 点客金额（amount） |
| nonDesignatedAmt | number | 提成（原「散客」）金额（amount） |
...
7. **分配取值**：`pickMode=avg`（UI「不分工位」）用规则级提成 / 点客（`nonDesignated` / `designated`）；`pickMode=station` 用订单行 `station` 对应工位参数；**工位缺失**：用方案 `stationIds[0]` 对应参数，若无则 `defaultPair()`（与原型 `schemeLineAmount` 一致）。
7a. **开单「选择工位」展示（类内 OR · 门店维度）**：对本店**启用中**方案（不按当前服务员工过滤），按业务类考察「默认卡 + 该类全部覆盖」的 `pickMode`；**任一**为 `station` → 该类在开单「选择服务员工」（及内嵌选人 Sheet）时**整类**需选工位。覆盖归属按规则目标类别（项目覆盖→项目类等）。详见 §4.5.1。本条约束 UI 采集；计提仍按规则 7。
7b. **开单「点客 / 散客」展示与交互降级（类内 OR · 门店维度）**：同规则 7a 的判定范围与归属，按业务类考察「默认卡 + 该类全部覆盖」的 `guestSplit`；**任一**为 `true` → 该类在开单「选择服务员工」（及内嵌选人 Sheet）时需选「点客 / 散客」；**全部**为 `false` → **不采集**「点客 / 散客」（不再是旧版必选）。当该类**同时**满足「`guestSplit` 全为 `false`」且「`pickMode` 全为 `avg`」时，开单**点击员工即完成勾选**，**不播放**「员工变为按钮」的展开动效、无任何二次选择；只要任一类维度需要采集（`guestSplit` 任一 `true` **或** `pickMode` 任一 `station`）则保留展开交互。未采集时订单行该员工不标记点客（`staffDesignated` 为非点客），按命中块 `nonDesignated`（「提成」侧）取值。详见 §4.5.2。
8. **金额计算**：`pct` → `round(base × rate / 100)`；`amount` → 固定金额（点客用 `designatedAmt` / 提成用 `nonDesignatedAmt`）再乘分摊系数（规则 2）。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、清空、重命名、复制方案、删除、改名、分配员工、已分配、未分配、已选 N 人、还有 N 人未分配任何提成方案、**基数、适用范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、**关联页面、头衔空态 sheet、方案空态 sheet**、**设置、头衔管理、角色管理、选择头衔、创建员工、邀请员工、查看记录**。
...
33. **Given** 按工位分配且三工位均有参数，**When** 查看规则卡，**Then** 「大工」「中工」「小工」全称竖排三行于行2左侧，适用范围 chip 顶部与参数区**顶部对齐**；数值完整可见；参数区左缘与标题图标左缘对齐。
...
53a. **Given** 默认「项目」=`avg`，但本店某启用方案存在一条项目类覆盖=`station`，**When** 开单进入「选择服务员工」（或内嵌选人 Sheet），**Then** **全部项目**均需选工位（类内 OR · 门店维度；见 §4.5.1 / 规则 7a）。
53b. **Given** 默认「项目」已=`station`，该类全部覆盖均为 `avg`，**When** 开单选服务员工，**Then** 项目类仍需选工位。
53c. **Given** 本店启用方案中，「项目」类默认卡与全部项目类覆盖的 `guestSplit` 均为 `false`，**When** 开单进入「选择服务员工」（或内嵌选人 Sheet），**Then** **不出现**「点客 / 散客」选择（不再是旧版必选）。
53d. **Given** 「项目」类全部块 `guestSplit=false` 且 `pickMode` 全为 `avg`，**When** 开单点击某员工，**Then** **直接完成勾选**；**不播放**「员工变为按钮」的展开动效、无任何二次选择。
53e. **Given** 本店某启用方案存在一条项目类覆盖 `guestSplit=true`（其余项目类块均为 `false` 且 `avg`），**When** 开单进入「选择服务员工」，**Then** **全部项目**均需选「点客 / 散客」（类内 OR；见 §4.5.2 / 规则 7b），且仍保留展开动效。
53f. **Given** 「项目」类全部块 `guestSplit=false`，但任一项目类块 `pickMode=station`，**When** 开单选择服务员工，**Then** 需选**工位**但**不需要**选「点客 / 散客」。
...
56d. **Given** 未开点客的规则，**When** 打开 Sheet，**Then** 左「提成」卡 + 右虚线卡「+ 点客提成」等比并排（等高，灰虚线灰字）；确定后点客=提成侧数值。
56e. **Given** 未开点客，**When** 点虚线卡，**Then** 全局 `guestSplit=true`，右卡变「点客」卡；再点「点客」卡 × 收起，回到虚线卡。
56f. **Given** 按工位分配且未开点客，**When** 打开 Sheet，**Then** 每个工位行各为「提成」卡 + 虚线卡等比并排；点任一虚线卡即三个工位同时开点客。
56f. **Given** 添加规则项页，**When** 查看，**Then** **无**全选/已选 N/反选工具行；底部「下一步（N）」在 N=0 时禁用。
