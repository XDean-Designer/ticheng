...
- **hub 链路二 · 员工管理**：员工列表（拖动排序）/ 职位管理 / 员工详情 / 创建·编辑·完善员工；**头衔**与**角色**分离；编辑·完善页 **敏感权限**（12 项 Switch，默认折叠；除店主外均显示）；`staff.sensitivePerms` 个人覆盖 + 改角色重置默认
...
员工管理
 ...
 │    ├─ incomplete →「去完善」
 │    └─ 长按拖动排序 · 点行 → 员工详情
 ├─ 职位管理（screen-emp-roles）
 │    ├─ 职位列表 · 添加 / 重命名 / 删除（≤6 字；被占用不可删）
 ├─ 员工详情（screen-emp-detail）→ 编辑员工
 ├─ 创建员工（formMode=create）：无状态；隐藏性别/年龄/年限
 ├─ 编辑员工（formMode=edit）：含状态；敏感权限折叠（非店主）
 └─ 完善员工（formMode=refine）：顶栏提示 · 保存后 incomplete=false
```

**FLOW**：`staff-list` / `staff-roles` / `staff-detail` / `staff-create` / `staff-refine`。
...
| 敏感权限 | `staff.sensitivePerms` | 编辑·完善保存；enforcement **【待联调】** |
...
| **行2** | 提成参数；按工位时大/中/小各占一行；`flex:1` 不压右侧 | 右侧定宽 **120px**；三 chip 均分单行；与参数间距 ≥10px，**禁止重叠** |
...
| 奖惩 | 最近 5 条；「查看全部」→ 奖惩明细 |
| 列表页配套 | 删「生效中」；卡面「合计」左置 + 提成 2×2（项目 / 产品 / 办卡·充卡 / 快速消费，无进度条）；结算周期页设**发薪日**（仅店主/合伙人；默认结算日+12·30日制；短月夹月末；未手改则随结算日重算） |
...
| 字段 | 原型文案 | 说明 |
|------|----------|------|
| 头衔 | 原「职位」 | 展示用岗位名（美容师等）；职位管理维护，≤6 字 |
| 角色 | 原「权限」 | 五档：`店主` / `合伙人` / `店长` / `高级店员` / `店员`；决定能力模板与敏感权限默认 |
...
| 提成方案 / 基本工资* | ✓ | ✓ | ✓ |
| 状态 | 隐藏（固定逻辑在岗） | ✓ | ✓ |
| 敏感权限 | 不显示 | 见 6.11.4 | 见 6.11.4 |

**校验**：姓名 ≤20 字；手机号 11 位大陆号；基本工资 0–999,999.99；新建不可选「店主」；本店已有店主时不可把他人改为店主。

**保存**：创建 push 新 `staff`；编辑/完善 `Object.assign`；同步 `assigneeIds`；完善成功 `incomplete=false`。

### 6.11.4 敏感权限（编辑 · 完善）

**显示**：被编辑员工 `角色 ≠ 店主` 且 `formMode ∈ {edit, refine}`。**店主编辑任何非店主员工均可见**（含合伙人、店长、高级店员、店员）。

**布局**：位于基础信息**同一卡片底部**、与表单**同一 `page-body` 滚动**；复用「更多信息」折叠样式；**默认折叠**；每次进入编辑/完善页重置为折叠。

**交互**：12 项 Switch（`pay-meta-switch`）；改角色 → **整表重置为该角色默认**（丢失未保存的个人覆盖）；保存写入 `staff.sensitivePerms`。
...
> **enforcement**：Switch 存储与 UI 本期完成；与各业务模块耦合 **【待联调确认】**。
...
|----|------|
| 模式 | 自然月 / 自定义周期 |
| 每月结算日 | 仅**自定义**模式显示；左侧 **pie-chart-02** 图标；1–28 日步进 |
| 每月发薪日 | 钱包图标；**仅店主/合伙人可改** |
| 默认发薪日 | 结算日 +12（**30 日制**）；未手改时随结算日重算；可「恢复默认」 |
| 短月 | 发薪日落月末仍视为配置「第 N 日」 |
...
| 页 | 规格 |
|----|------|
| 设奖惩 | 日期/员工/奖励|扣除/金额/备注；底部当月最近 5 条；行首 **奖**绿圆徽章 / **扣**灰圆徽章 |
| 奖惩明细 | 当月全部记录列表（以原型为准） |
| 金额 | 奖励正、扣除负；展示带 +/- |
...
| sensitivePerms | object | 12 key → boolean；见 §6.11.4 |

## 7.8 敏感权限 `sensitivePerms`

- 结构与 §6.11.4 `key` 表一致。
- 默认由 `perm`（角色）查表生成；个人覆盖持久化在 `staff.sensitivePerms`。
- 改 `perm` 未保存前 UI 重置默认；保存时整对象写入。
...
| settleDay | 1–28（自定义结算日） |
| payDayOfMonth | 1–30 发薪日 |
| payDayManual | 是否手改发薪日（手改后结算日变更不自动重算） |
...
23. **敏感权限**：个人 Switch 覆盖角色默认；改角色重置默认表（§6.11.4）；`sensitivePerms` 持久化；业务 enforcement 待联调。
24. **三链路员工单源**：`EmployeeStore.staff` 为权威；员工管理增删改后薪资列表与 `assigneeIds` 一致，无需二次导入。
25. **头衔与角色分离**：表单「头衔」来自职位管理；「角色」为五档 perm；二者独立校验与展示。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、反选、清空、重命名、复制方案、删除、改名、恢复为默认、分配员工、已分配、未分配、已选 N 项、已选 N 人、还有 N 人未分配任何提成方案、**基数、范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、迁移完成、迁移结果提醒、重新创建方案、稍后处理、**关联页面、选择员工（3工位）、选择员工（2工位）、选择员工（1工位）**。

**规则卡**：原价、实收、**固定**、点客、散客、**基数**、**范围**、提成范围、计算基数、分配模式、不分工位、按工位分配、按比例 %、固定金额 ¥、现金、卡付、团购、提成参数。
...
33. **Given** 按工位分配且三工位均有参数，**When** 查看规则卡，**Then** 大/中/小竖排三行于行2左侧，范围 chip 相对参数区整体中轴对齐；数值完整可见。
...
41. **Given** 结算周期页，**When** 店主设发薪日，**Then** 仅店主/合伙人可改；默认按结算日+12（30日制）；未手改时改结算日会重算发薪日。
...
53. **Given** 按工位规则卡，**When** 查看参数，**Then** 大/中/小竖排三行，不与范围重叠。
...
58. **Given** 创建员工，**When** 不填姓名保存，**Then** toast「请输入员工姓名」。
59. **Given** 创建员工，**When** 角色选「店主」，**Then** toast 拦截「新建员工不可设为店主」。
60. **Given** 编辑「店长」员工（店主视角），**When** 滚动至卡片底，**Then** 见「敏感权限」折叠且默认收起；展开见 12 项 Switch。
61. **Given** 编辑店员，**When** 改角色为「高级店员」未保存，**Then** Switch 重置为高级店员默认（5 项关）。
62. **Given** 编辑店员并改「查看客户列表」为开，**When** 保存后再进入，**Then** `sensitivePerms.customerListView=true`。
63. **Given** 编辑店主本人，**When** 进入表单，**Then** 无敏感权限区块。
64. **Given** 员工选提成方案 A，**When** 保存，**Then** 方案 A 的 `assigneeIds` 含该员工 id；提成设置分配 Sheet 同步。

### 10.4 链路三 · 员工薪资 P0

65. **Given** 薪资列表，**When** 查看顶部，**Then** 无「生效中」区块；有全店合计与发薪日摘要。
66. **Given** 薪资列表员工卡，**When** 查看下半，**Then** 2×2 四格提成无进度条；项目格为净值（扣快消）。
67. **Given** 点员工卡，**When** 进入，**Then** 打开「员工薪资明细」非业绩提成明细。
68. **Given** 薪资明细，**When** 点「业绩提成明细」，**Then** 进入逐笔页；月合计业绩/提成口径与明细页一致。
69. **Given** 结算周期自定义模式，**When** 查看每月结算日，**Then** 左侧 pie-chart 图标；发薪日仅店主/合伙人可改。
70. **Given** 业绩提成明细快消行，**When** 查看标签，**Then** 橙色闪电图标（非「快」字方底）。
71. **Given** 店主在明细改提成，**When** 保存，**Then** 立即生效、行显「改」、可开修改记录；无待确认。
72. **Given** 设奖惩页最近记录，**When** 查看，**Then** 奖励行绿「奖」徽章、扣除行灰「扣」徽章。
73. **Given** 员工视角且当月有他人改提成，**When** 打开业绩提成明细，**Then** 顶栏下显示品牌橙 banner（可关闭、可去查看）。
