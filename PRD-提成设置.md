...
| 任一为 `true` | 全部为 `avg` | 展开 **2 张**卡：「**提成**」/「顾客指定」（原「普通」改名，与 Sheet 行标签一致）；两者**各自独立、可叠加**（先勾「提成」再勾「顾客指定」，或只勾其一）；**点任一按钮都收起**；**只勾「顾客指定」也是有效的单独分配** —— 该员工仍计入已选、只发顾客指定提成 |
| 任一为 `true` | 任一为 `station` | 展开 **4 张**卡：大 / 中 / 小工 + 「顾客指定」；工位**单选**且**必须用户手点**（可点掉、可全不选）；「顾客指定」**独立勾选、不要求先有工位**（可与任一工位叠加，也可单独勾）；**再点已勾选的工位 = 取消该工位**（不再抖动提醒）；**任一按钮点选后一律收起**——只勾「顾客指定」而没点工位也立即收起，该员工**仍计入已选**，摘要显示 `无工位 · 顾客指定`（见 §6.7.1） |
...
- **只勾「顾客指定」= 有效的单独分配**：按工位 + 开顾客指定时，若用户**只点了「顾客指定」**而没点工位，卡片**立即收起**并呈：
  ...
  ②**出现右侧勾选控件**（与普通已选一致，点它即可取消该员工）；
  ③卡片第三行摘要为 **`无工位 · 顾客指定`** —— **提成位置是灰字「无工位」**（`--text-sec`），**「顾客指定」是红字**（品牌红）；不分工位态下两侧都勾为 `提成 · 顾客指定`、只勾顾客指定为 `顾客指定`；
...
- **取消勾选（展开态）**：**任一按钮点选后一律收起**（含只勾「顾客指定」）。①**点掉工位**：**还有「顾客指定」→ 员工仍已选**（摘要回 `无工位 · 顾客指定`，只发顾客指定提成）；否则**完全取消该员工**；②**点掉「顾客指定」**：还有工位/提成 → 退回仅提成侧（员工仍已选）；无提成侧 → 该员工连人一并取消、回到完全未选态；③**点掉「提成」**（仅不分工位态）：还有「顾客指定」→ 员工仍已选（摘要回 `顾客指定`）；否则**取消该员工**选择并收起。
- **取消选择（收缩态）**：已勾选员工卡右侧的勾选控件可点，点后取消该员工选择（`staffChosen=false`），并清掉其提成侧 / 顾客指定标记（`staffRoles` + `staffExtra` 一并删除）。
...
**入口行 / 摘要行**：同前。摘要文案：工位 + 顾客指定 → `大工 · 顾客指定`（工位名不带其他后缀）；仅工位 → **仅工位名**（如 `大工`）；**只勾顾客指定、没有工位** → **`无工位 · 顾客指定`**（`无工位` 为**灰字**、`顾客指定` 为**红字**，卡片呈选中态样式且**有勾选控件**、**计入已选**、**只发顾客指定提成**）；不分工位 + 顾客指定 且勾了「提成」→ `提成 · 顾客指定`；不分工位 + 顾客指定 且未勾「提成」→ `顾客指定`；**皆否（不分工位 + 未开顾客指定）** → 卡片第三行仍为**灰色头衔**（无红色提成摘要），入口摘要行不写 meta。
...
| 点已勾选的「提成」/「顾客指定」 | **取消该勾**并**收起**：「顾客指定」→ 不分工位有「提成」则退回仅「提成」、按工位有工位则退回仅工位 / **两侧皆空则完全取消该员工**；「提成」（仅不分工位）/ 工位 → 还有「顾客指定」则**员工仍已选**（摘要回 `顾客指定` / `无工位 · 顾客指定`）、否则**取消该员工**选择 |
| 点已勾选的工位 | 走**反向动效**（收勾 `55ms` + 褪红 `30ms` = `85ms`）后**取消该工位**并收起：还有「顾客指定」→ **员工仍已选**（摘要 `无工位 · 顾客指定`，只发顾客指定提成）；否则**完全取消该员工** |
| 工位多张 | 工位之间**单选**（勾新的即替换旧的；旧勾**先反序收回**再画新勾，并行「红色交接」）；**可点掉、可全不选** |
| 「顾客指定」 | **独立勾选，不要求先有工位/提成**——可与任一工位（或「提成」）同时勾选，也可单独勾（= **只发顾客指定提成**）；可再点取消该勾 |
| 只勾「顾客指定」、没选工位/提成 | **立即收起** → 卡片呈**选中态样式**（粉底/粉描边/头像粉圈）、**出现勾选控件**、摘要 `无工位 · 顾客指定`（`无工位` 灰字 + `顾客指定` 红字）、**计入已选**（入口人数与订单行摘要列表都含它）；**只发顾客指定提成**（提成侧 0）；再次展开仍是勾选态 |
| 收缩后的员工卡 | 已勾选 → 摘要行显示结果（`大工 · 顾客指定` / `无工位 · 顾客指定` / `提成 · 顾客指定` / `顾客指定`）、右侧**放大勾选控件**（约 20px），**点它取消该员工选择**、**无**右上角 × |

**交互四态**：

| 按工位 | 开顾客指定 | 点员工卡后（展开的勾选卡） |
|:---:|:---:|------|
| ✓ | ✓ | Morph 展开为**整行 4 张**：「大工」「中工」「小工」「顾客指定」（工位名按 `stationLabels` 映射；每张右上角未勾选控件）；工位**必须手点**（不预选默认工位）、**单选且可点掉、可全不选**；顾客指定**独立可勾**（不要求先点工位）；**点任一按钮都收起**；只勾顾客指定未点工位 → **仍计入已选**、摘要 `无工位 · 顾客指定`、只发顾客指定提成；两侧叠加时收缩后员工卡显示 `工位 · 顾客指定` |
| ✓ | ✗ | 展开 **3 张**工位卡；**必须手点**；单选且**可点掉**（点掉 = 完全取消该员工）后收缩（员工卡显示工位名） |
| ✗ | ✓ | 展开 **2 张**：「**提成**」「顾客指定」；**两组各自独立、可叠加**（先勾「提成」再勾「顾客指定」）；只勾「提成」→ 摘要 `提成`，只勾「顾客指定」→ 摘要 `顾客指定`（**仍计入已选、只发顾客指定提成**），两者都勾 → `提成 · 顾客指定`；再点已勾选的任一张即取消该勾，两侧皆空才取消该员工 |
...
| 点掉已勾选工位（A12） | 走**反向**收勾动效（`55ms` + `30ms` = `85ms`）后取消该工位并收起；**原「iOS 抖动提醒」已废止**（工位改为可点掉）。若还有「顾客指定」→ 员工**仍已选**、摘要 `无工位 · 顾客指定`（见 A13b），否则完全取消该员工 |
...
**Dialog/Sheet**：方案名称、重命名方案、请输入方案名称、确认删除该方案？、删除后不可恢复。、确认删除该规则项？、尚未保存、尚未保存，确定要返回吗？、留在本页、不保存、多方案规则、计算基数说明、分配模式说明、未分配任何提成方案、暂无在岗员工、**选择员工、提成、指定、+ 顾客指定提成、顾客指定、大工、中工、小工**、**比例 / 金额（两段式胶囊）**；开单侧选人演示（§6.7.1）：按工位+开顾客指定为 **大工 / 中工 / 小工 / 顾客指定**，不分工位+开顾客指定为 **提成 / 顾客指定**。

**选人摘要 / 提示（§6.7.1）**：摘要 `大工` / `大工 · 顾客指定` / `无工位 · 顾客指定` / `提成` / `提成 · 顾客指定` / `顾客指定`（`无工位` 为灰字 `.staff-card__pick-miss`、`顾客指定` 为红字 `.staff-card__pick-x`）；提示行四态文案见 §6.7.1；分配模式说明 Dialog 含「只勾「顾客指定」也可单独分配（只发顾客指定提成）」。**界面不得再出现「普通」作为不分工位侧标签**（已改「提成」），也不得再出现「未选」作为摘要文案（已改「无工位」）。
...
53s. **Given** 不分工位 + 开顾客指定，**When** 点击员工卡，**Then** 展开 **2 张**卡「**提成** / 顾客指定」；勾选后收缩，摘要显示 `提成` / `顾客指定` / `提成 · 顾客指定`（**两组可叠加、不再二选一**）；**When** 重新展开并再点已勾选的任一张，**Then** 只取消该勾并**收起**：还有另一侧勾 → 员工仍已选（摘要回 `提成` 或 `顾客指定`）；两侧皆空 → 取消该员工。
...
53v. **Given** 按工位 + 开顾客指定且某员工卡已展开、**尚未点工位**，**When** 点「顾客指定」，**Then** 该勾选**立即画出并随之收起**（**不再保持展开等工位**），卡片呈：`is-done` 选中态样式（粉底 + 粉描边 + 头像粉圈）、**出现右侧勾选控件**、摘要 `无工位 · 顾客指定`（`无工位` 灰字 `rgb(146,146,146)` + `顾客指定` 红字 `rgb(243,47,65)`）、**员工计入已选**（入口 `已选 1 人`、订单行员工摘要列表含它）；**When** 再次展开，**Then** 该「顾客指定」**仍为勾选态**（保留记忆，不弹提示）。
...
53x. **Given** 员工已勾选「大工」，**When** 再点「大工」（已勾选的那张），**Then** 该工位被**取消**，走**反向收勾动效**（55ms + 30ms = 85ms）后收起；若该员工同时勾了「顾客指定」→ **员工仍已选**（摘要 `无工位 · 顾客指定`、保留勾选控件、只发顾客指定提成）；否则**完全取消该员工**（回到未选态）；**不再**播放 iOS 抖动、不弹 toast。
53x2. **Given** 员工已勾选「提成」（不分工位），**When** 再点「提成」，**Then** 该勾被取消并收起；若同时勾了「顾客指定」→ **员工仍已选**（摘要 `顾客指定`）；否则**完全取消该员工**。
...
| 字段 | 内容 |
|------|------|
| 触发 | 点击已勾选的「顾客指定」/「提成」（或工位）/ 收缩态勾选控件 / **再点已勾选的工位（A12）** |
...
| 生效范围 | **全部反向路径**：① 切换工位时被取消的旧工位；② 取消「顾客指定」→ 退回仅提成侧 / 完全未选；③ 取消「提成」（仅不分工位）；④ 收缩态员工卡点右侧勾选控件取消选择；⑤ **点掉已勾选工位**（A12） |
...
| 结束态 | `opacity: 1`、`translateY(0)`；文案为 `工位 · 顾客指定` / 仅工位名 / `无工位 · 顾客指定` / `提成 · 顾客指定` / `提成` / `顾客指定` |
| 时长 / 缓动 | `360ms` / Apple 标准 |
| 降级 | 直接终态 |
| 实现锚点 | `comm2.css` → `staffMetaIn`；「无工位 · 顾客指定」的灰/红双色拆分见 `comm2.css` → `.staff-card__pick-miss` / `.staff-card__pick-x` |
...
| 时长 / 缓动 | `85ms`（= 正向前一半） |
| 结束态 | 工位勾取消、收起；**还有「顾客指定」→ 员工仍已选**（摘要 `无工位 · 顾客指定`，见 A13b）；否则完全取消该员工（`staffRoles` 删除、`staffIds` 移除） |
| 触觉 | `vibrate(8)` |
...
| 选项卡（已选「顾客指定」/「提成」） | 取消勾 → 落定 → 收起；还有另一侧勾 → 员工仍已选 | 等（A7 播完 · 85ms） |
| **已勾选的工位卡** | 点掉该工位（A12 反向收勾）→ 落定 → 收起；还有「顾客指定」→ **员工仍已选**（A13b 摘要 `无工位 · 顾客指定`），否则完全取消该员工 | 等（A7 播完 · 85ms） |
...
| 过程 | 动效播完（A13 门控）→ 收起（A2 即时重绘）→ 卡片以**「只勾顾客指定」态**重绘 |
| 视觉 | ①`is-done` 选中态样式（粉底 + 粉描边 + 头像粉圈，与已选一致）；②**出现勾选控件**（`.staff-card__tick` 在该员工**计入已选**时渲染 —— 十七次起此态**计入已选**）；③摘要行 `无工位 · 顾客指定`：`无工位` 用 `.staff-card__pick-miss`（`--text-sec` 灰）+ 分隔符灰，`顾客指定` 用 `.staff-card__pick-x`（品牌红） |
...
| 退出路径 | ①点任一工位 → 叠加（摘要 `工位 · 顾客指定`，`basePicked=true` 恢复提成侧）；②点掉「顾客指定」→ 两侧皆空 → 完全取消该员工（粉底与摘要一并撤除、入口人数减 1）；③收缩态点右侧勾选控件 → 同 ② |
| 时长 / 缓动 | 摘要行入场 `360ms` / Apple 标准（A11）；卡片本身**无过渡**（状态重绘即时） |
| 降级 | 同上（仅动效降级，样式与数据不变） |
| 实现锚点 | `comm2.js` → `spExtraOnly(sid)` / `spSummaryParts(sid)` / `spSummaryHtml(sid)` / `spCardPickLineHtml()`（`done = isChosen`）+ `spRenderPickerHtml()` 的 `tickBtn = (isChosen && !isEdit) ? … : ''`；`comm2.css` → `.staff-card__pick-miss` / `.staff-card__pick-x` |
...
- **动效不得是唯一反馈**：状态变化必须同时有**静态可读反馈**（摘要行文案 `工位 · 顾客指定` / `无工位 · 顾客指定` / `提成 · 顾客指定` / `提成` / `顾客指定`、入口「已选 N 人」、Sheet 提示行文案，见 §6.7.1）。
...
| N14 | 展开态，已勾选「大工」 | 再点「大工」 | 走 **A12 反向收勾**（`55ms` + `30ms` = `85ms`）→ 取消该工位 → 收起；若同时已勾「顾客指定」→ **员工仍已选**（`is-done` 样式、**有**勾选控件、摘要 `无工位 · 顾客指定`、只发顾客指定提成）；否则**完全取消该员工**（入口回「未选择」）；`vibrate(8)`；**不再有抖动动效、不出 toast** |
| N14b | 不分工位，已勾选「提成」+「顾客指定」 | 再点「提成」 | 走 A12 反向收勾（`85ms`）→ 取消「提成」→ 收起；**员工仍已选**（摘要 `顾客指定`）；再点「顾客指定」→ 两侧皆空 → **完全取消该员工** |
| N15 | 按工位 + 开顾客指定，未选工位 | 只点「顾客指定」 | 勾选动效播完（170ms）后卡片**立即收起**（**不再保持展开等工位**）；卡片呈：`is-done` 选中态样式、**出现勾选控件**、摘要 `无工位 · 顾客指定`（`无工位` 灰字 `rgb(146,146,146)`、`顾客指定` 红字 `rgb(243,47,65)`）；员工**计入已选**（入口 `已选 1 人`、订单行员工摘要列表含它）；订单行落库 `basePicked=false` → **提成侧折 0、只发顾客指定提成** |
...
| N17c | 不分工位 + 开顾客指定 | 只点「顾客指定」→ 再展开点「提成」 | 两侧各自独立勾选、可叠加：摘要 `顾客指定` → `提成 · 顾客指定`；**不得**出现「点「提成」把「顾客指定」清掉」的二选一行为 |
...
| N26 | 已选「大工」，**展开后 120ms（`is-splitting` 仍在 DOM）** 立刻再点一次「大工」 | 观察该卡 | 走反向收勾（`55ms` + `30ms` = `85ms`）→ 工位被取消 → 收起；**入场动画 `staffRoleSplit` 不得吞掉反向动效**（`is-undraw` / 盒 `transition` 与 `animation` 互不干扰）；落定后工位为未勾选态；若还有「顾客指定」→ **员工仍已选**、摘要 `无工位 · 顾客指定` |
...
- [ ] 「只勾顾客指定、未选工位/提成」→ **有效的单独分配**：`is-done` 样式 + **有勾选控件** + 摘要 `无工位 · 顾客指定`（灰 `rgb(146,146,146)` / 红 `rgb(243,47,65)`）+ **计入** `getStaffCount()`；订单行落库 `basePicked=false` → 提成侧折 0、只发顾客指定提成
- [ ] 「提成侧 / 顾客指定侧」两组参数在 `spOptionChecked()` / `spApplyOptionToggle()` 里**各自独立**判断，`staffRoles[sid]` 统一存提成侧选择（工位 id / 不分工位的哨兵 `avg`）；**不得**再由「`chosen && !extra`」推导「提成」勾选态
- [ ] 收缩态点勾选控件取消选择时，`staffRoles` 与 `staffExtra` **一并删除**（`spDropStaff`）
...
| 2 | 「已展开不重播 Morph」依赖状态位 `opened` + `.is-opened`；十六次**废止「保持展开等工位」后该位恒为 `false`**，`.is-opened` 成为**死代码** | 每次展开都会重播 Morph（**当前符合预期**：「只勾顾客指定」卡片再展开本就该有入场）；死代码留着易误读 | 清理 `spState.edit.opened` / `.is-opened` 渲染分支与 CSS 规则（A1 判据改为「该卡本次是否由**收起**转为**展开**」） |
...
3. 跑 §12.9 的 `N1–N26`；其中 **N5–N9 / N14 / N14b / N15–N17c / N18 / N21 / N25** 是回归必跑项（勾选动效、门控、一次性标记、「只勾顾客指定」态最易回归）。
