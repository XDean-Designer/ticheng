...
- 左侧导航「**关联页面**」区域（位于「页面链路」与「文档」之间）+ 「选择员工」独立灰底页 + Sheet（员工卡 UI/UX 与开单页同款；纯演示，toast「已选 N 人」）
...
├─ 关联页面（左侧导航区域）
│    └─ 选择员工：独立灰底页（`screen-comm2-staff`）+ Sheet（员工卡 morph：点客 / 散客 → 大工 / 中工 / 小工；头像图）· 清空 · 完成（toast「已选 N 人」）
...
| 工位设置 | 工位设置 | 底部「完成」 |
| 分配员工 | 分配员工 | 底部「取消 / 确定」 |
| 选择员工（关联页面） | 选择员工 | 独立灰底页 + Sheet；员工卡 UI/UX 与开单页同款（点客 / 散客 → 大工 / 中工 / 小工）；底部「完成」 |
...
**页面宿主**：点击入口后先切换到独立灰底页（`screen-comm2-staff`，背景 `#F7F7F7`），再打开 Sheet；**不叠在方案列表 / 编辑 / 添加规则项等现有页面上**。关闭后回到进入前的页面。

**Sheet 结构**：
- 标题「选择员工」。
- 员工卡网格：**5 名演示员工**（顾清扬 / 林屿森 / 何苏叶 / 阿Ken / Lisa，含头像图），**初始全部未选**。
- 底部「完成」。

**交互动效**（**与开单页面「选择服务员工」百分百同款**，CSS + morph JS 自开单页复制）：
- 点卡片 → 卡片缩窄再展开（`is-pinching` → `is-expanding`），出「**点客 / 散客**」双选项（其余卡片置灰）；点 scrim 可取消。
- 选「点客 / 散客」→ 再 morph 出「**大工 / 中工 / 小工**」三选项；选择工位后卡片复位为选中态（左上角红色对勾，标题行显示「点客·大工」等摘要）。
- 已选卡片右上角「✕」可清除选择。

**行为**：
- 「完成」→ toast「已选 N 人」→ 关 Sheet → 回到进入前页面。
- 点遮罩关闭 Sheet 并回到进入前页面。
