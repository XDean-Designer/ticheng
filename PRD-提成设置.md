...
> **v2.0（2026-08-31）规则卡与迁移重写**：删除方案列表「经理签单计算提成」开关与 `signComm` 字段；经理签单（实收=0）按命中块 `baseMode` 计提（原价计 / 实收不计）；规则卡改为两行布局（标题+基数 / 参数+范围）、独立 chip 范围控件、Sheet 拆分（标题全量 / 参数精简）；模块 13 改为结构必迁、不保证金额一致，迁移结果弹窗（一致/差异）+ 差异时引导重新创建；阶梯暂不迁入；无回滚开关。
...
| 按工位 | **竖排三行**，与行2 范围 chip **顶对齐**；不与范围横向重叠 |
| 不分工位 | 「点」「散」横排于行2左侧 |
...
33. **Given** 按工位分配且三工位均有参数，**When** 查看规则卡，**Then** 大/中/小竖排三行于行2左侧，与范围顶对齐；数值完整可见。
...
> 文档版本自 v2.0 起：**结构必迁、不保证金额一致**；迁移完成后弹窗告知结果。
...
分类 / 高级设置 / 单项提成 → `defaults` / `overrides` 同 v1.x 字段表（见 §7），但：
