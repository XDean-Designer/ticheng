...
| 文档版本 | v1.6 |
| 日期 | 2026-08-28 |
| 交互原型（唯一可视依据） | 同目录 `demo.html`（中文镜像：`剑琅联盟-RTB重构.html`；功能链路「提成设置」`commission2`，及「开单记账」流水详情） |
| 画布 | 390 × 844 |
...
| 6 | 功能规格 | ✓ | 了解 | ✓ | 逐屏字段与跳转；含 §6.10 关联页面（薪资 / 流水） |
...
1. 本期为**新版提成设置**（原型「提成设置」`commission2`），采用 **多方案** 模型：每个提成方案可整体分配给若干员工，方案内按「项目 / 产品 / 办卡 / 充卡 / **快速消费**」**5 项默认** + N 条**覆盖规则项**配置提成。**每人可分配多个提成方案**；开单计提时对该员工所属全部方案按**订单行**分别计算，**计算项重叠时取提成金额更高者**（并列按方案列表顺序先出现者胜出；见 §8）。不同行分别取高后可相加。**快速消费**在 UI 为第 5 张默认卡，数据上是系统项目覆盖项（`system: true`，`refId=quick`），不可删除。
2. **提成计算不再依赖旧「业绩设置」**：旧「业绩设置」功能（简单模式 / 进阶基础设置）**本期被完全移除**，工作台不再提供入口；其“按业绩计算提成”的旧口径一律废除。存量配置按 **模块 13** 折叠迁入新提成方案，保证迁移前后提成金额一致。
3. 系统内「业绩」与「业绩提成」**解耦**：业绩**只作为经营统计维度**，**不参与提成计算**，也**不可人工编辑**（见 §8 规则 16）。金额口径统一为对应项的**开单原价（标价）加总**（不用实收）。展示分列因页面而异：流水 / 「员工业绩提成明细」的业绩仍可按支付方式（现金 / 卡付 / 团购 / 经理签单 / 记为欠款）展示；「员工薪资明细 · 业绩统计」按提成设置 5 类（项目 / 产品 / 办卡 / 充卡 / 快速消费）展示（见 §6.10）。
4. **流水订单**：订单行内「业绩」**只读展示**，不再可编辑；「提成」仍可编辑，但需权限（**店主 / 合伙人**），且每次改动记录**修改日志**（操作人 / 时间 / 前后值）。覆盖页：开单流水详情、流水订单修改、项目流水修改（见 §6.8、§6.10、§8 规则 18–19）。
5. **提成生效时点**：开单（结算）时按员工所属方案自动计算提成并**写入订单行** `staffCommissions`；此后可在流水详情人工修正（有权限者）。薪资侧「提成统计 / 提成分列」字段对齐提成设置 5 类，且 **「项目」列 = 项目类提成合计 − 快速消费提成**（快速消费另列；五列之和 = 提成总计）；详见 §6.10。
...
- 分配员工 Sheet（员工池多选、全选、清空；已在其他方案者可勾选，副文「也在：…」）
...
- 流水订单：业绩只读展示（去编辑 icon）；提成可编辑（店主 / 合伙人）；修改日志记录与查看（开单流水详情 / 流水订单修改 / 项目流水修改）
- 关联页面口径对齐：员工薪资明细（业绩统计 / 提成统计）、员工业绩提成明细（见 §6.10）
...
| 分配员工 | 把方案分配给人，**可一人多方案** | 已在其他方案者可勾选并提示「也在：…」；列表“已分配”显示人名 |
...
- **多方案可并存**：同一员工可出现在多个方案的 `assigneeIds`；分配 Sheet 不灰显禁用，副文提示「也在：{方案名…}」。
...
| 弹窗 | 内容 |
|------|------|
| 多方案规则（列表页帮助） | ①每人可分配多个提成方案（已在其他方案提示「也在：…」仍可勾选）；②计算项重叠按行取金额更高者（并列按列表顺序）；③按行过滤支付范围；④方案内覆盖优先；⑤不同业务行分别取高后可相加 |
...
> 本章描述因本期提成设置改造而联动的**现网关联页**规格；配图见 `assets/related/`（源目录：`提成关联页面/`）。薪资页完整交互规格不在本期展开，**仅锁定口径与字段对齐**。
...
**配图**：`assets/related/MuMu-20260828-100433-951.png`（业绩统计）、`assets/related/MuMu-20260828-171110-007.png`（提成统计）

| 区块 | 现状（截图） | 变更后 |
|------|--------------|--------|
| 业绩统计 · 圆环 / 图例 | 售卡业绩、产品业绩、现金类（服务）、卡付（服务）、团购（服务）、经理签单（服务） | 改为 **项目 / 产品 / 办卡 / 充卡 / 快速消费**；金额 = 各类**原价加总**；项目不含快速消费；五列之和 = 「总计」 |
| 提成统计 · 圆环 / 图例 | 现金类、卡付、团购、经理签单 | 改为 **项目 / 产品 / 办卡 / 充卡 / 快速消费**；**项目列扣减快速消费提成**；五列之和 = 「总计」 |
| 本月合计薪资 / 提成总计 | — | 数值仍取订单行提成汇总；与提成统计总计一致 |
| 服务 / 售卡人次摘要等 | 服务人次、售卡数量等 | **本期不改字段结构**（仅保证其「金额」若表示业绩，与原价口径不冲突） |

### 6.10.2 工作台 → 员工薪资 → 员工业绩提成明细

**配图**：`assets/related/MuMu-20260828-100124-920.png`

| 区块 | 现状（截图） | 变更后 |
|------|--------------|--------|
| 月合计 · 业绩 | 分列：现金类 / 卡付 / 团购 / 经理签单 / 记为欠款；金额偏实收或旧口径 | **分列字段名保持支付方式五格**；金额改为对应支付方式下相关项的**原价加总**；业绩总计 = 五格之和 |
| 月合计 · 提成 | 同上五格（支付方式） | 改为 **项目 / 产品 / 办卡 / 充卡 / 快速消费**；**项目列 = 项目类提成 − 快速消费提成**；五列之和 = 提成总计 |
| 日合计 / 单笔记录 | 展示该日 / 该笔业绩与提成 | 业绩仍只读；金额口径同上（原价）；提成展示订单行写入值 |
...
2. **按行过滤**：对每个候选方案，每笔订单行先按**支付方式**对照该方案命中块的「提成范围」；不在范围则该方案本行**不计**、不参与取高。
...
16. **业绩与提成解耦**：业绩**不参与提成计算**、**不可人工编辑**；金额口径为对应项**开单原价加总**（见 §6.10.0）。展示分列：流水与「员工业绩提成明细」的业绩可按支付方式分列；「员工薪资明细 · 业绩统计」按提成设置 5 类分列。提成在薪资侧按 5 类分列，且「项目」列扣减快速消费提成（§6.10）。
...
22. **关联页字段对齐**：员工薪资明细 / 员工业绩提成明细的提成分列、员工薪资明细的业绩统计分列，必须与提成设置 5 类一致；流水三页业绩只读去编辑 icon（§6.8、§6.10）。

## 8.1 试算口径（原型试算引擎）

原型内置 8 条示例订单行（`COMM2_TRIAL_LINES`）用于演示与验证：开卡·尊享组合卡、充卡·老客续充、深层补水护理、染发、剑琅玻尿酸精华液、团购体验·洗头、卡付·时尚洗吹、**快速消费**。

计提验证步骤：取员工所属**全部**方案（列表顺序）→ 每行对每个方案解析覆盖/默认块 → 支付范围过滤 → 计算金额 → **取金额最大者**（并列保留列表更靠前方案）→ 汇总。结果行带 `winnerSchemeId`。此口径与 §8 一致，作为联调参考样例（非正式接口）。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、反选、清空、重命名、复制方案、删除、改名、恢复为默认、分配员工、已分配、未分配、已选 N 项、已选 N 人、还有 N 人未分配任何提成方案、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、**关联页面、选择员工（3工位）、选择员工（2工位）、选择员工（1工位）**。
...
**Toast**：请输入方案名称、方案已重命名、方案已复制、方案已删除、提成方案已保存、已更新规则、已添加规则项、已删除规则项、已分配 N 人、已清空分配、**已选 N 人**、至少选一种支付方式、「{类}」至少选一种支付方式、覆盖规则「{标题}」至少选一种支付方式、请输入有效的点客比例/金额、请输入有效的散客比例/金额、比例需在 0–100%、名称不能为空、最多 5 个工位、已删除工位。
...
13. **Given** 方案 S 已分配员工 A，**When** 打开方案 S2 的分配 Sheet，**Then** A **可勾选**，副文显示「也在：S」（或含 S）；全选会选中 A。
...
23. **Given** 流水订单行支付方式为「经理签单」且规则范围含「签单」，**When** 计提，**Then** 按该行原价 / 实收照常计提。
...
32. **Given** 覆盖规则标题「染发、烫发、剑琅玻尿酸精华液」，**When** 卡片横向空间不足，**Then** 第一行标题至少显示 6 字并以 `…` 截断；`title` 属性含全文；第一行右侧提成参数完整可见。
33. **Given** 按工位分配且三工位均有参数，**When** 第一行宽度仅容两个工位段，**Then** 放不下的工位段（如「小工 …」）**整段**换至参数第二行且右对齐；chevron 不与参数重叠。
34. **Given** 任意规则卡，**When** 查看布局，**Then** 第一行 = 标题 + 提成参数（同行）；第二行 = 计算基数标签 + 提成范围胶囊；chevron 相对整卡垂直居中。
35. **Given** 不分工位规则，**When** 参数区极窄，**Then** 「点客X·散客Y」整段换行，不在「点客」「散客」之间拆开。
36. **Given** 规则卡列表，**When** 查看卡片，**Then** 无「比例 / 固定」切换控件；点击卡片打开规则设置 Sheet。
37. **Given** 员工薪资明细 · 业绩统计，**When** 查看圆环图例，**Then** 字段为项目 / 产品 / 办卡 / 充卡 / 快速消费；项目不含快速消费；金额为各类原价加总；五列之和 = 总计。
38. **Given** 员工薪资明细 · 提成统计，**When** 查看圆环图例，**Then** 字段为项目 / 产品 / 办卡 / 充卡 / 快速消费；项目列 = 项目类提成 − 快速消费提成；五列之和 = 总计。
...
| 交付 | 说明 |
|------|------|
| 本文 `PRD-提成设置.md` | 开发与测试准稿（含模块 13 迁移；§6.10 关联页） |
| 交互原型 `demo.html` / 独立原型 `提成设置/` | 唯一可视依据；功能链路「提成设置」（`commission2`）+ 流水详情 |
...
| 阶梯与新方案互斥 | 不得同时生效「阶梯 + 新单项」；历史双绑 → `needs_review` 并 **阻断** 该员工切换，默认 **优先阶梯** 直至人工处理 |
