...
| 全部为 `false` | 任一为 `station` | 展开 **3 张**工位卡（大 / 中 / 小工）；工位**单选**；无「顾客指定」卡 |
| 任一为 `true` | 全部为 `avg` | 展开 **2 张**卡：「普通」/「顾客指定」，**二选一**（「普通」= 非顾客指定） |
| 任一为 `true` | 任一为 `station` | 展开 **4 张**卡：大 / 中 / 小工 + 「顾客指定」；工位**单选**，「顾客指定」**可与任一工位同时勾选**（再点即取消该勾） |
...
- **取消勾选**：已勾选员工卡右侧的勾选控件可点，点后取消该员工选择（`staffChosen=false`），并清掉其工位 / 顾客指定标记。
...
| 提成参数 | 区头 hint「点「比例 / 金额」切换」（**无**全局「按比例 / 固定金额」段控） | 每张参数卡：卡头**仅标题**（「提成」/「额外」，字色 `#929292`）；取值行=取值（占满该行，长数值仅在框内裁切）+ **两段式胶囊**（**宽卡同行、窄卡换行**，贴卡片右下角、**尺寸不变**、不溢出卡片）；胶囊**每点一次切换一次**本卡取值模式，**热区为整块控件**；切换时保留另一模式已填值；输入行 `%`/`¥` 只读（金额为 `¥` 前缀），比例输入框宽度自适应、`%` 紧贴数值右侧 |
...
| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | 左「提成」输入卡 + 右**虚线卡**「+ 顾客指定提成」（与提成卡**等比并排等高**）；点单位切换本卡 `nonDesignatedValueMode` | `nonDesignated` / `nonDesignatedAmt` + `nonDesignatedValueMode` |
| 开 `true` | 左「提成」+ 右「额外」等比并排（卡内标题「提成」「额外」）；各卡独立 `%`/`¥`（行内右对齐胶囊切换）；「额外」卡卡头右侧 **×** 收起 | 提成 / 顾客指定各值 + 各自 `*ValueMode` |
...
| 点已勾选卡片 | 再点同一张即**取消该勾** |
| 工位多张 | 工位之间**单选**（勾新的即替换旧的） |
| 「顾客指定」 | **可与任一工位同时勾选**（互不替代）；再点取消该勾 |
...
|:---:|:---:|------|
| ✓ | ✓ | Morph 展开为**整行 4 张**：「大工」「中工」「小工」「顾客指定」（工位名按 `stationLabels` 映射；每张右上角未勾选控件）；工位单选 + 顾客指定可叠加；勾选后收缩，员工卡显示 `工位 · 顾客指定` |
| ✓ | ✗ | 展开 **3 张**工位卡；单选后收缩（员工卡显示工位名） |
| ✗ | ✓ | 展开 **2 张**：「普通」「顾客指定」；二选一后收缩（员工卡显示 `普通` / `顾客指定`） |
| ✗ | ✗ | **点选即勾选**，无展开、无勾选卡（员工卡第三行仍为灰色头衔） |
...
| 勾选控件 | `staffCheckIn` 同 spring；未勾选 → 勾选有描边弹入 |
...
7a. **开单「选择服务员工」的工位 / 顾客指定采集与交互降级（项 × 员工 维度）**：开单每添加一项 `I`、对候选每个员工 `E` 独立判定——只考察 `E` 所属启用方案，按规则 5 解析该项在每个方案内的唯一命中块（单项覆盖 > 整类分组覆盖 > 整类默认；快捷开单只命中系统覆盖），得命中块集 `B`；`B` 中任一 `pickMode=station` → 该员工需选**工位**（单选），任一 `extraSplit=true` → 还需勾选「**顾客指定**」（**可与任一工位叠加**）；两者皆否 → **点选即勾选**（不展开、无二次选择）。`E` 无方案或项无提成配置（如商城订单）→ 点选即勾选。未勾选顾客指定按规则 7「无顾客指定」路径取值（`avg` 取 `nonDesignated`、`station` 取该工位 `nonDesignated`）；未采集工位按规则 7 缺省回落；计提仍按规则 3 取高。**顾客指定虽仍采集工位，但其顾客指定值一律按全局值加算，不受所选工位影响。** 本条约束 UI 采集，详见 §4.5.1。
...
53o. **Given** 4 张勾选卡已展开，**When** 点「大工」，**Then** 该卡变为**已勾选**并立即**收缩**回员工卡；员工卡显示红色摘要 `大工`，右侧出现**放大勾选控件**（约 20px），且**无**右上角 ×。
53p. **Given** 员工已选「大工」，**When** 再次点击该员工卡，**Then** 重新展开 4 张卡且「大工」为已勾选态；此时再点「顾客指定」，**Then** 「顾客指定」也被勾选，收缩后摘要为 `大工 · 顾客指定`（工位单选 + 顾客指定叠加）。
53q. **Given** 员工已选「大工 + 顾客指定」，**When** 展开后点「中工」，**Then** 「大工」自动取消勾选、「中工」勾选，摘要变为 `中工 · 顾客指定`（工位之间**单选**）。
53r. **Given** 员工已勾选，**When** 点击其右侧放大勾选控件，**Then** 该员工**取消选择**（入口摘要回到「服务员工 未选择」）。
53s. **Given** 不分工位 + 开顾客指定，**When** 点击员工卡，**Then** 展开 **2 张**卡「普通 / 顾客指定」；勾选后收缩，摘要显示 `普通` 或 `顾客指定`。
53t. **Given** 不分工位 + 未开顾客指定，**When** 点击员工卡，**Then** **点选即勾选**（无展开卡），员工卡第三行仍为**灰色头衔**（无红色摘要）。
