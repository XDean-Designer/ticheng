...
- 规则设置 Sheet：适用范围、计算基数、分配模式、提成取值；**不分工位**=单「提成」卡 + 「+ 点客提成」虚线卡（`guestSplit`）；**按工位**=3 个工位行（各 1 张「提成」卡）+ 第 4 行「点客」行（未开点客时为虚线卡）；工位名内联改名（固定 3，不可增减；全局同步）
...
- **开单侧「选择服务员工」口径**（跨系统约束；**仅锁口径，不在本仓库改开单代码**）：按「**具体项 × 具体员工**」判定（§4.5.1）——对每一项、每一个候选员工，只看该员工所属启用方案按 §8 规则 5 解析出的命中块集，任一命中块 `pickMode=station` → 该员工需选**工位**；任一命中块 `guestSplit=true` → 该员工需选**点客 / 散客**；两者皆否 → **点击员工即完成勾选**，无二次选择、**不播放**「员工变为按钮」展开动效
...
| 单项提成（提成/点客/%/¥/工位） | 新方案 `CatRule` / 按项 `overrides` |
| 阶梯提成 | **本期不做**新阶梯方案；试点历史问题线下处理 |
| 特殊项目单独提成 | 覆盖规则项：指定项目 / 产品 / 会员卡 / 整类分组 |
| 快捷开单单独提成 | 第 5 项默认「快捷开单」；数据为系统项目覆盖项（`ov_sys_quick`），不吃「项目」默认 |
| 按工位分别设提成 | 分配模式 = 按工位分配：**逐工位各 1 个「提成」参数（3 个）**；「点客提成」为 `CatRule` 级**全局单值（1 个）**、与工位无关——两者**独立叠加**共 4 个参数（旧体系为正交 3×2 = 6 个）。工位在规则 Sheet 内联改名（固定 3，不可增减；全局同步） |
| 开单选员工必选「点客 / 散客」 | 按**项 × 员工**判定（§4.5.1）：该员工命中块任一 `guestSplit=true` → 采「点客 / 散客」；任一 `pickMode=station` → 采「工位」；两者皆否 → **点选即勾选**（无展开动效） |
...
| 按工位分配 | 大 / 中 / 小工**各自设「提成」**（3 个）；「点客提成」可选、开启后为**全局单值**（+1，共 4 个） | 规则卡显示 3 段工位提成，开点客后追加「点客」段；工位改名全局同步本店全部方案 |
...
│    ├─ 整卡进入：适用范围 + 基数 + 分配 + 提成取值 + 提成/点客（`guestSplit`）
...
| 提成取值（每值独立） | 见 §7.2.1：`nonDesignatedValueMode` / `designatedValueMode` / `stations[sid].valueMode` | **不再**规则级统一切换；Sheet 点 `%`/`¥` 单位切换本卡；遗留字段 `valueMode` 仅作迁移/派生（全量固定 → `amount`，否则 `pct`）；**全部生效取值为固定金额时**卡上基数区显「固定」 |
...
### 4.5.1 跨系统约束：开单「选择服务员工」的工位 / 点客采集与交互降级（项 × 员工 维度）

> **约束对象**：开单侧「选择服务员工」及相关嵌入式选人交互（含「加入购物车」等内嵌选人 Sheet）。本期**仅在本 PRD 锁定口径**；开单原型/实现按本条联调，**不在本仓库改开单代码**。

**判定范围（项 × 员工）**：开单每添加一项 `I` 并为其选择服务员工时，对候选列表中的**每一个员工 `E` 独立**判定是否需要采集「工位」和 / 或「点客 / 散客」。判定**只考察 `E` 本人所属的启用中方案**；`E` 不属于任何方案 → 无命中块 → 该员工**点选即勾选**（不需工位、不需点客 / 散客）。
...
1. **工位**：`B` 中**任一**块 `pickMode=station` → 该员工在本项上需选**工位**；
2. **点客 / 散客**：`B` 中**任一**块 `rule.guestSplit=true` → 该员工在本项上需选**点客 / 散客**。

**四态矩阵**（作用于「本项 × 该员工」；同一项下不同员工可呈现不同交互）：

| 该员工命中块集 `B` · `guestSplit` | 该员工命中块集 `B` · `pickMode` | 开单「选择服务员工」交互 |
|-----------------------------------|---------------------------------|--------------------------|
| 全部为 `false` | 全部为 `avg` | **点击员工即完成勾选**：不展开任何二次选择、**不播放**「员工变为按钮」的展开动效；该员工按「提成」单值计提 |
| 全部为 `false` | 任一为 `station` | 需选**工位**；**不需要**选「点客 / 散客」 |
| 任一为 `true` | 全部为 `avg` | 需选「**点客 / 散客**」；不需要选工位 |
| 任一为 `true` | 任一为 `station` | 需选「**点客 / 散客**」+ **工位** |

**口径要点**

- **按员工差异化**：同一项下，不同员工按其各自命中块集 `B` 独立呈现——有的直接勾选、有的仅选工位、有的选工位 + 点客 / 散客，互不影响。
- **点客值全局、与工位无关**：开启点客后，该块的点客提成为 `CatRule` 级**单值**；按工位态下**点客顾客一律取该值**，3 个工位参数只作用于**非点客**的「提成」侧（§6.3 / §7.2.1）。
- **点客顾客仍采集工位**：采集口径维持「两个维度各自独立」（下表四态）——命中块 `pickMode=station` 时仍选工位（用于落库与非点客侧），但点客侧金额一律按全局点客值计算，不受所选工位影响。
- **交互呈现（点击后自适应）**：选人列表**不预打标**；点选某员工时按该员工是否需二次选择决定是否展开「员工变为按钮」动效——需采集（工位或点客 / 散客**任一**）才展开，两者皆否则直接完成勾选。
- **内嵌选人一并生效**：若「加入购物车」等 Sheet **内嵌**了选服务员工，同一规则一并生效。
- **未采集时的落库与计提**：未采集「点客 / 散客」时，订单行该员工**不标记点客**（`staffDesignated` 为非点客），按命中块「提成」侧取值——`pickMode=avg` 取 rule 级 `nonDesignated`，`pickMode=station` 取该工位 `stations[station].nonDesignated`；未采集「工位」时按 §8 规则 7 缺省回落（`stationIds[0]`）。计提仍按 §8 规则 3 取高、规则 7 取值。
- **无提成配置的项**：商城订单（`mall`）无任何提成块命中 → 若开单侧也要为其选服务员工，一律**点选即勾选**。
- **旧数据兼容（3×2 → 3+1 迁移）**：旧模型「按工位 × 点客」为正交（3 工位 × 2 值 = 6 值），新模型为独立叠加（3 工位提成 + 1 全局点客 = 4 值）。迁移时点客值提升为 `CatRule` 级单值，**取 `stationIds[0]` 对应工位的点客值**；`stations[*]` 仅保留提成侧。`guestSplit` 缺省判定沿用 §6.3（两端值不等 → 自动视为已开启），即旧数据若点客 ≠ 提成，该员工命中块视为 `guestSplit=true`，仍需采集「点客 / 散客」。
...
┌─▌ 标题（≤6 字完整）                          │＞│   ← 卡头
│ ────────────────────────────────────────────────  │   ← 1px #F2F2F2
│ 提成 10% │ 点12%                 [原价|实收]/固定   │   ← 内容区 · 不分工位
│ 大工 12%                         [现金][卡付][团购] │   ← 内容区 · 按工位
│ 中工 10%                                          │
│ 小工 8%                                           │
│ 点客 15%                                          │
└─▌                                                  ┘

标签（不分工位=「提成」；按工位=工位名，点客段=「点客」）与数值同为 5px 间距
guestSplit=false：`提成 10%`（标签 + 单值）
guestSplit=true：`提成 10% │ 点12%`（前为提成值、后为点客值；中间为 1px 竖线分隔；「点」小标签在点客数值**左侧**；两端可各自为 `%` 或 `¥`）
按工位：**逐工位各 1 段单值**竖排（未开点客 `大工 12%` / `中工 10%` / `小工 8%`）；开启点客后**点客与首工位（大工）同行**、间距 **16px**（例：`大工 12%  点客 15%` / `中工 10%` / `小工 8%`）——点客为 `CatRule` 级全局单值，**不逐工位重复**
```

| 区域 | 左 | 右（120px 槽） |
|------|----|----------------|
| **卡头** | 分类图标 + 标题（≤6 字完整） | 「＞」16px（`#C2C2C2`；**不再**放在卡最右） |
| **内容区** | 提成参数（标签 + 数值；不分工位标签为「提成」、按工位为工位名；开点客后按工位时点客与大工同行）；按工位时竖排；`flex:1`；**参数区左缘与标题图标左缘对齐**；**参数区顶部与右栏首个控件顶部对齐** | 竖排两控件（**可直接点选**）：上「原价/实收」连体段控（或只读「固定」），下「现金/卡付/团购」三 chip 均分单行 |
...
| 规则 | 说明 |
|------|------|
| 单提成 | `guestSplit=false`：`提成 10%`——**恒有**左侧标签；比例带 `%`，固定带 `¥`；比例输入框 `%` 紧贴数值右侧 |
| 开启点客 | `guestSplit=true`：`提成 10% │ 点12%`（前为提成值，后为点客值；中间 **1px 竖线**分隔；「点」为小标签，位于点客数值**左侧**；两端可各自 `%`/`¥`） |
| 按工位 | 工位名（同款标签）+ **单值**，竖排 3 段；开启点客后**点客与首工位（大工）同行**、间距 **16px**（不追加为第 4 竖段） |
...
| 分隔竖线 | 提成值与点客值之间：`1px × 12px`、色 `#DCDCDC`、左右 `4px` 间距；与数值中轴对齐 |
| 「点」小标签 | 10px / 600、圆角 3px、底色 `#F0F0F0`、字色 `#8D8D8D`；位于点客数值**左侧**（右间距 3px），中轴与数值中轴对齐 |
...
| 入口 | 含适用范围/基数 | 其他区 |
|------|:-----------:|--------|
| 整卡点击；「添加规则项 · 设置」 | ✓ | 分配模式 + 会员卡（覆盖卡时）+ 提成/点客参数（+工位；每卡独立 `%`/`¥`） |

| 区 | 控件 | 说明 |
|----|------|------|
| 适用范围 | chips 多选（现金 / 卡付 / 团购） | 至少 1 类；按项金额分摊（见 §8） |
| 计算基数 | 分段：原价模式 / 实收模式 | 全部取值为固定金额时卡上仍显「固定」 |
| 分配模式 | 分段：不分工位 / 按工位分配 | 按工位时展开工位参数区（固定 3 工位内联改名，见 §6.4） |
| 会员卡 | 分段：办卡 / 充卡 | **仅**覆盖规则且目标为会员卡时显示 |
| 提成参数 | **无**全局「按比例 / 固定金额」段控；右侧 hint「点「比例 / 金额」切换」 | 每张参数卡**卡头右侧**「比例 / 金额」chip 可点翻转本卡取值模式；输入行 `%`/`¥` 只读；切换时保留另一模式已填值；比例输入框宽度自适应，`%` 紧贴数值右侧（固定为 `¥` 前缀） |
| 提成 / 点客 | 见下 | `guestSplit` 开关 |

**点客拆分（`rule.guestSplit`）**

> **模型**：按工位与点客为**独立叠加**——`pickMode=station` 且开点客时共 **4 个参数**（3 个工位提成 + 1 个全局点客）；未开点客时共 **3 个**（3 个工位提成）。点客为 `CatRule` 级**单值**，**不逐工位**。四个参数**各自**可独立选比例或固定金额。

**不分工位（`pickMode=avg`）**

| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | 左「提成」输入卡 + 右**虚线卡**「+ 点客提成」（与提成卡**等比并排等高**）；点单位切换本卡 `nonDesignatedValueMode` | `nonDesignated` / `nonDesignatedAmt` + `nonDesignatedValueMode` |
| 开 `true` | 左「提成」+ 右「点客」等比并排；各卡独立 `%`/`¥`；点客卡 **×** 收起 | 提成 / 点客各值 + 各自 `*ValueMode` |

**按工位（`pickMode=station`）**

| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | **3 个工位卡并列一行**（等宽）；**第 2 行「点客」虚线卡**，宽度 = **单个工位卡**（左对齐） | 逐工位 `stations[sid].{nonDesignated, nonDesignatedAmt, valueMode}` |
| 点点客虚线卡 | 全局 `guestSplit=true`：点客行变为「点客」卡（标题旁 **×**）；默认值与 mode 取**首个工位** | 新增 rule 级 `designated*` + `designatedValueMode` |
| 开 `true` | 3 工位并列一行 + 点客单独一行（宽同单工位）；**4 个数值各自可点 `%`/`¥`** | 逐工位提成 + rule 级全局点客 |
| × 收起 | 关回 `false`：rule 级点客跟提成侧同值，点客行回到虚线卡 | — |

**按工位取值口径**：非点客顾客按所在工位取该工位提成值及其 `valueMode`；**点客顾客一律取 rule 级点客值及其 `designatedValueMode`（与工位无关）**。见 §6.2.3 / §8 规则 7。

| 通用项 | 说明 |
|--------|------|
| 旧数据 | 若点客 ≠ 提成（含任一工位）→ **自动视为已开启**；迁移时点客值取 `stationIds[0]`；旧规则级 `valueMode` 灌入各字段的独立 mode |
| 开单采集 | 是否选「点客 / 散客」按 **§4.5.1 项 × 员工** 判定 |
...
> **无**专属「工位设置」主路径依赖；工位在规则设置 Sheet「按工位」态管理。**固定 3 工位**（大 / 中 / 小工）**+ 第 4 行「点客」行**（承载 rule 级全局点客，非工位），均**不可增减**。

| 控件 | 行为 |
|------|------|
| 工位名行 | 工位名 + edit-02 可点 → 行内改名；maxlength **6** |
| 点客行（第 4 位） | 标签恒为「点客」，**不可改名**（无 edit-02）；行内为「点客」卡或「+ 点客提成」虚线卡，见 §6.3 |
| 新增 / 删除 | **无**（固定大/中/小工） |
...
> **交互**：本页已按四态重做（非旧「点客/散客 → 再选工位」两步）；动效为 iOS 向 spring。
...
4. **点客提成**分段：`未开点客` / `开点客`（默认开）—— 与上项组合成四态
5. 白卡「服务员工」：入口行 + 已选摘要

**入口行 / 摘要行**：同前。摘要文案：仅点客 → `点客`；选了工位 → **仅工位名**（如 `大工`，不带「散客」）；不分工位+开点客非点客 → `普通`；**不分工位+未开点客** → 卡片第三行仍为**灰色头衔**（无红色提成摘要），入口摘要行不写 meta。

**底部 Sheet**：提示文案随四态切换：
- 皆否：`可多选员工；点卡片即完成选择。`
- 仅开点客：`…选择点客或普通。`
- 仅按工位：`…选择工位（小工/中工/大工）。`
- 按工位+开点客：`…选点客即完成，或点选工位再选（…）。`

**交互四态**：

| 按工位 | 开点客 | 点员工卡后 |
|:---:|:---:|------|
| ✓ | ✓ | Morph 展开为**纵向**「点客 / 选工位」→ 点「点客」即完成（`designated=true`，工位占位首工位）；点「选工位」→ 点客消失、按钮**裂成 3 工位** → 选一完成（散客+工位） |
| ✓ | ✗ | 直接裂成 **3 工位**，选一完成 |
| ✗ | ✓ | 纵向「点客 / 普通」，点一即完成（「普通」= 非点客，`designated=false`） |
| ✗ | ✗ | **点选即勾选**，无展开、无二级面板 |
...
| 三工位分裂 | `staffRoleSplit .38s` 同 spring，错开 `0/35/70ms` |
| 完成弹跳 / 勾 | `staffDonePop` / `staffCheckIn`，同 spring |
| 触觉 | `vibrate(8)` |
...
| 分配模式说明 | 不分工位：开单不分工位，统一按提成计提、点客可单独设置；按工位分配：固定大 / 中 / 小工分别设置提成；可点工位名改名（全局同步，不可增减）。**开单是否采集「选择工位」/「点客 / 散客」**不单看当前规则卡，而按 **§4.5.1 项 × 员工 维度**（按每项、每员工各自命中块 OR 判定） |
...
| designatedValueMode | 'pct' \| 'amount' | 点客提成的取值模式 |
| guestSplit | boolean | 是否拆分点客/提成（点客单独设置）；默认 `false`；旧数据两端不等时自动为 `true` |
| designated | number | **点客提成（pct）——`CatRule` 级全局单值，与工位无关**；`guestSplit=false` 时与提成侧同值 |
| nonDesignated | number | 提成（原「散客」）比例（pct）——**`pickMode=avg` 时使用** |
| designatedAmt | number | 点客提成金额（amount）——同 `designated`，全局单值 |
| nonDesignatedAmt | number | 提成（原「散客」）金额（amount）——`pickMode=avg` 时使用 |
| stations | object | `{ [stationId]: { nonDesignated, nonDesignatedAmt, valueMode } }`——按工位「提成」值 + **该工位独立** `valueMode`；点客不逐工位存储 |
| _gs2 | boolean | 内部迁移标记；`true` 表示已按「3+1」新模型归一化 |
...
| 字段 | 说明 |
|------|------|
| staffIds / staffDesignated / staffRoles | 员工、点客标记（designated）、工位（role） |
...
7. **分配取值**：**点客与工位相互独立**——凡命中块标记点客（订单行该员工为点客），**一律取 rule 级全局点客值** `designated` / `designatedAmt`，**与工位无关**；非点客时：`pickMode=avg`（UI「不分工位」）取 rule 级 `nonDesignated` / `nonDesignatedAmt`，`pickMode=station`（UI「按工位分配」）取订单行 `station` 对应工位 `stations[station].nonDesignated` / `.nonDesignatedAmt`。**工位缺失**：用方案 `stationIds[0]` 对应工位参数，若无则工位默认值 `{ nonDesignated: 10, nonDesignatedAmt: 0 }`（与原型 `lineRateFor` 一致）。按工位且开点客时共 **4 个**可配参数（3 工位提成 + 1 全局点客），未开点客共 **3 个**。
7a. **开单「选择服务员工」的工位 / 点客采集与交互降级（项 × 员工 维度）**：开单每添加一项 `I`、对候选每个员工 `E` 独立判定——只考察 `E` 所属启用方案，按规则 5 解析该项在每个方案内的唯一命中块（单项覆盖 > 整类分组覆盖 > 整类默认；快捷开单只命中系统覆盖），得命中块集 `B`；`B` 中任一 `pickMode=station` → 该员工需选**工位**，任一 `guestSplit=true` → 需选**点客 / 散客**；两者皆否 → **点选即勾选**（不展开、无二次选择）。`E` 无方案或项无提成配置（如商城订单）→ 点选即勾选。未采集点客按规则 7「非点客」路径取值（`avg` 取 `nonDesignated`、`station` 取该工位 `nonDesignated`）；未采集工位按规则 7 缺省回落；计提仍按规则 3 取高。**点客顾客虽仍采集工位，但其金额一律按全局点客值计算，不受所选工位影响。** 本条约束 UI 采集，详见 §4.5.1。
8. **金额计算**：`pct` → `round(base × rate / 100)`；`amount` → 固定金额（**点客用 rule 级全局 `designatedAmt`**；非点客用 `nonDesignatedAmt` / 按工位用 `stations[station].nonDesignatedAmt`）再乘分摊系数（规则 2）。费率取值见规则 7。
...
29. **固定金额与 baseMode**：某费率按其自身 `valueMode` 为固定金额时，按规则 2 分摊固定额；**规则卡「固定」灰标仅当该块全部生效取值均为固定金额时显示**。按工位时非点客取对应工位金额（若该工位 `valueMode=amount`），**点客取 rule 级全局 `designatedAmt`（若 `designatedValueMode=amount`）**。
...
计提验证步骤：取员工所属**全部**方案（列表顺序）→ 每行对每个方案解析覆盖/默认块 → 非经理签单行按适用范围分摊基数，经理签单行按命中块 `baseMode` / `valueMode` → 计算金额 → **取金额最大者**（并列保留列表更靠前方案）→ 汇总。结果行带 `winnerSchemeId`。费率取值：**点客恒取 rule 级全局点客值（与工位无关）**；非点客按 §8 规则 7 取工位提成 / 规则级提成。此口径与 §8 一致，作为联调参考样例（非正式接口）。
...
| 方案 | 要点 |
|------|------|
| 顾问标准提成 | 项目 12/10、产品 10/10、办卡 5/5、充卡 3/3（提成/点客 %）；分配给 st0、st1、st2、st3 |
| 卡付劳动专项 | 项目 / 产品范围仅「卡付」3/3；办卡 / 充卡全范围 0/0；分配给 **st1、st3**（st1 多方案演示） |
| 资深技师综合方案 | 项目**按工位**：逐工位「提成」大工 12 / 中工 10 / 小工 8，**点客全局 15**（开点客共 4 个参数；不开则 3 个）；产品实收模式；办卡 15/12、充卡 8/8；5 条覆盖（深层补水 18/15、染烫+精华液 20/15、尊享组合卡办卡 12/10、洗头固定 ¥5、**烫染整类按工位：提成 15 / 12 / 10 + 点客全局 20**）；分配给 st4 |

资深技师方案项目为原价模式时，经理签单·深层补水护理行按深层补水覆盖 18% × 原价 268 计提 ¥48.24；若改为实收模式则该行比例提成为 0。

**模型说明（3+1）**：按工位方案的「点客」为 **`CatRule` 级全局单值**，**不逐工位**。开点客共 **4 个**参数（3 工位提成 + 1 点客），未开点客共 **3 个**（3 工位提成）。点客顾客无论订单行落在大工 / 中工 / 小工，一律按全局点客值（如 15%）计提；非点客按所在工位提成（12% / 10% / 8%）计提。
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、清空、重命名、复制方案、删除、改名、分配员工、已分配、未分配、已选 N 人、还有 N 人未分配任何提成方案、**基数、适用范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、**关联页面、头衔空态 sheet、方案空态 sheet、选择服务员工**、**点客、普通、散客、小工、中工、大工、不分工位、按工位、未选择、已选 N 人**、**设置、头衔管理、角色管理、选择头衔、创建员工、邀请员工、查看记录**。

**规则卡**：原价、实收、**固定**、提成、点客、大工、中工、小工、**基数**、**适用范围**、计算基数、分配模式、不分工位、按工位分配、按比例 %、固定金额 ¥、现金、卡付、团购、提成参数。

**Dialog/Sheet**：方案名称、重命名方案、请输入方案名称、确认删除该方案？、删除后不可恢复。、确认删除该规则项？、尚未保存、尚未保存，确定要返回吗？、留在本页、不保存、多方案规则、计算基数说明、分配模式说明、未分配任何提成方案、暂无在岗员工、**选择员工、提成、点客、+ 点客提成、大工、中工、小工**；开单侧选人演示（§6.7.1）：不分工位+开点客为 **点客 / 普通**（业务口径仍称点客 / 散客）。

**Toast**：请输入方案名称、方案已重命名、方案已复制、方案已删除、提成方案已保存、已更新规则、已添加规则项、已删除规则项、已分配 N 人、已清空分配、**已选 N 人**、至少选一种支付方式、「{类}」至少选一种支付方式、覆盖规则「{标题}」至少选一种支付方式、请输入有效的点客比例/金额、请输入有效的提成比例/金额、比例需在 0–100%、名称不能为空、工位名最多 6 个字。
...
5. **Given** 项目类固定金额且已设置点客提成，**When** 输入提成 ¥3、点客 ¥5 保存，**Then** 规则卡内容区摘要为「提成 ¥3 │ 点¥5」。
...
33. **Given** 按工位分配且未开点客，**When** 查看规则卡，**Then** 「大工」「中工」「小工」全称竖排三行于行2左侧（共 **3 段，无点客段**），适用范围 chip 顶部与参数区**顶部对齐**；数值完整可见；参数区左缘与标题图标左缘对齐。
33a. **Given** 按工位分配且**已开点客**，**When** 查看规则卡，**Then** 「点客」与**首工位（大工）同行**、间距 **16px**（例：`大工 12%  点客 15%`），其下仍竖排中工 / 小工；点客为 `CatRule` 级全局单值，**不逐工位**。
34. **Given** 任意规则卡，**When** 查看布局，**Then** 卡头=图标+标题+「＞」；内容区左参数、右 120px 竖排两控件（可点选）；卡头分隔线从左侧装饰条右侧起画、装饰条贯通不被压线；整卡空白区可点进全量 Sheet。
35. **Given** 不分工位，**When** 查看参数，**Then** 数值左侧恒有「提成」标签（12px/500/#8D8D8D，与工位名同款，间距 5px）；未开点客为 `提成 10%`；开启点客为 `提成 10% │ 点12%`（中间 1px 竖线分隔；「点」标签在点客数值左侧、中轴与数值对齐）。
...
48. **Given** 固定金额 + 按工位，**When** 计提含部分适用范围支付的行，**Then** 非点客取对应工位 `nonDesignatedAmt`、**点客取 rule 级全局 `designatedAmt`**，再 × (范围内实付 / 全部实付)。
...
53c. **Given** 员工 `E` 命中当前项 `I` 的全部块 `guestSplit=false`，**When** 开单为该项选择服务员工，**Then** **该员工 `E`** 在本项上**不出现**「点客 / 散客」选择（不再是旧版必选）；同一项下命中块 `guestSplit=true` 的其他员工仍需选。
53d. **Given** 员工 `E` 命中块 `guestSplit=false` 且 `pickMode` 全为 `avg`，**When** 开单点击该员工，**Then** **直接完成勾选**；**不播放**「员工变为按钮」的展开动效、无任何二次选择。
53e. **Given** 员工 `E` 命中当前项 `I` 的某一块 `guestSplit=true`（其余命中块为 `false` 且 `avg`），**When** 开单为该项选择该员工，**Then** **该员工 `E`** 需选「点客 / 散客」（见 §4.5.1 / 规则 7a），且仍保留展开动效；同一项下命中块全为 `false` 的其他员工不需选。
53f. **Given** 员工 `E` 命中块 `guestSplit` 全为 `false`，但任一命中块 `pickMode=station`，**When** 开单选择该员工，**Then** 需选**工位**但**不需要**选「点客 / 散客」。
53g. **Given** 同一项 `I` 下，员工 A 命中块集全为 `false`/`avg`、员工 B 命中块含 `station`、员工 C 命中块含 `guestSplit=true`，**When** 开单为该项选择服务员工，**Then** A 点选即勾选、B 需选工位、C 需选点客 / 散客，三者按各自状态独立呈现（项 × 员工维度）。
53h. **Given** 员工 `E` 不属于任何启用方案，**When** 开单为任一项选择该员工，**Then** **点选即勾选**，不需工位、不需点客 / 散客。
53i. **Given** 当前项为商城订单（`mall`，无提成配置），**When** 开单为其选择服务员工，**Then** 一律**点选即勾选**，不需工位、不需点客 / 散客。
53j. **Given** 员工 `E` 命中块的 `guestSplit` 两端值不等（旧数据点客 ≠ 提成），**When** 开单为该项选择该员工，**Then** 视为 `guestSplit=true`，需选「点客 / 散客」（§6.3 旧数据兼容）。
53k. **Given** 员工 `E` 命中块 `pickMode=station` 且 `guestSplit=true`（按工位 **且** 开点客），**When** 开单为该项选择该员工，**Then** **既需选工位、也需选「点客 / 散客」**（两者**相互独立**、互不替代）；计提时点客取 `CatRule` 级全局点客值（与工位无关）、散客取所选工位提成值。
53l. **Given** 员工 `E` 命中块 `pickMode=station` 且 `guestSplit=false`（按工位**未**开点客），**When** 开单为该项选择该员工，**Then** **仅需选工位**，**不出现**「点客 / 散客」选择；且点选后**仍需**展开动效（因有二次选择）——与「点选即勾选」的 `avg` + 未开点客场景区分。
53m. **Given** 同一项 `I` 下员工 A 命中 `station`+开点客、员工 B 命中 `station`+未开点客，**When** 开单为该项选择服务员工，**Then** A 需选工位 + 点客 / 散客、B 仅需选工位，两者按**各自**命中块状态独立呈现（项 × 员工维度，非项级统一）。
...
56d. **Given** 未开点客的规则（`pickMode=avg`），**When** 打开 Sheet，**Then** 左「提成」卡 + 右虚线卡「+ 点客提成」等比并排（等高，灰虚线灰字）；确定后点客=提成侧数值。
56e. **Given** 未开点客（`pickMode=avg`），**When** 点虚线卡，**Then** 全局 `guestSplit=true`，右卡变「点客」卡；再点「点客」卡 × 收起，回到虚线卡。
56f. **Given** 按工位分配且未开点客，**When** 打开 Sheet，**Then** **3 个工位卡并列一行**（等宽），各仅 1 张「提成」卡；确定后写入 3 个工位提成值及其各自 `valueMode`。
56g. **Given** 按工位分配且未开点客，**When** 查看 Sheet 点客行，**Then** 单独一行、宽度 = **单个工位卡**，为虚线卡「+ 点客提成」；点该虚线卡则全局 `guestSplit=true`，点客默认值与 mode 取**首个工位**。
56h. **Given** 按工位分配且已开点客，**When** 查看 Sheet，**Then** 3 工位并列一行 + 点客单独一行（宽同单工位），共 **4 个参数**；**无**全局「按比例 / 固定金额」段控；每卡卡头「比例 / 金额」chip 可点独立切换取值模式。
56i. **Given** 按工位分配且已开点客，**When** 修改点客值并确定，**Then** 只写入 rule 级 `designated*`（全局单值），**3 个工位的提成值不受影响**；再次打开 Sheet，各工位显示各自提成、点客行显示该全局点客值。
56i2. **Given** Sheet 内任一参数卡，**When** 点卡头「比例 / 金额」chip，**Then** 仅该卡模式翻转（文案在「比例」「金额」间切换），另一模式已填值保留；输入行 `%`/`¥` 只读跟随；保存后规则卡摘要按各卡实际 mode 显示 `%` 或 `¥`。
56j. **Given** 旧数据（正交 3×2 = 6 值，存在 3 组工位点客值且互不相同），**When** 加载方案，**Then** 点客值提升为 `CatRule` 级单值并取 **`stationIds[0]` 对应工位**的点客值；`stations[*]` 仅保留提成侧；`_gs2` 标记为已归一化；`guestSplit` 按「点客 ≠ 工位提成」判定为 `true`。
56k. **Given** 按工位 + 开点客的方案，**When** 为某点客顾客开单并分别把订单行落在大工 / 中工 / 小工，**Then** 三次计提金额**完全相同**（均取 rule 级全局点客值，与所选工位无关）；改为散客后则分别取该工位提成值。
