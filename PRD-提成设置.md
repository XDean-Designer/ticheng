...
- 规则设置 Sheet：适用范围、计算基数、分配模式、提成取值；**不分工位**=「提成」卡 + 「额外」卡（未开顾客指定时为虚线卡「+ 顾客指定提成」，`extraSplit`）；**按工位**=**3 工位卡横排一行**（各 1 张「提成」卡）+ **第 2 行「顾客指定」卡**（宽 = 单工位卡、左对齐；未开为虚线卡「+ 顾客指定提成」）；卡内标题「提成」「额外」字色 `#929292`；每卡取值行右对齐**两段式胶囊**「比例 / 金额」（点整块切换取值模式；**宽卡同行、窄卡换行**贴卡片右下角，尺寸不变、不溢出卡片）；工位名内联改名（固定 3，不可增减；全局同步；工位名 13px、改名图标 16px）；**规则卡上的顾客指定段显示名为「顾客指定」（不再写 Sheet 卡标题用的「额外」）**
...
| 全部为 `false` | 任一为 `station` | 展开 **3 张**工位卡（大 / 中 / 小工）；工位**单选**且**必须用户手点**（不预选默认工位），未点工位不计入已选；无「顾客指定」卡 |
| 任一为 `true` | 全部为 `avg` | 展开 **2 张**卡：「普通」/「顾客指定」，**二选一**（「普通」= 非顾客指定）；再点已勾选的「顾客指定」→ 退回「普通」，再点已勾选的「普通」→ 取消该员工 |
| 任一为 `true` | 任一为 `station` | 展开 **4 张**卡：大 / 中 / 小工 + 「顾客指定」；工位**单选**且**必须用户手点**；「顾客指定」**可与任一工位同时勾选**（再点即取消该勾）；**再点已勾选的工位不取消**（该卡播 iOS 抖动提醒，须改点其他工位）；先勾「顾客指定」而缺工位时**保持展开**等工位（见 §6.7.1） |
...
- **工位为必选项（不可点掉）**：需采集工位时，**工位必须由用户手点才勾选**（**不预选 / 不自动带默认工位**）；未点工位的员工**不计入已选**（不落库）。**再点已勾选的工位不会取消**——该卡播 **iOS 抖动动效**提醒（**不出 toast**）；要换工位就点另一张工位卡，要取消整人则到**收缩态**点右侧勾选控件。
...
| 提成参数 | 区头 hint「点「比例 / 金额」切换」（**无**全局「按比例 / 固定金额」段控） | 每张参数卡：卡头**仅标题**（「提成」/「额外」，字色 `#929292`）；**不分工位 + 开顾客指定**时，每列卡上方另有**静态列头**「普通」/「顾客指定」（见下）；取值行=取值（占满该行，长数值仅在框内裁切）+ **两段式胶囊**（**宽卡同行、窄卡换行**，贴卡片右下角、**尺寸不变**、不溢出卡片）；胶囊**每点一次切换一次**本卡取值模式，**热区为整块控件**；切换时保留另一模式已填值；输入行 `%`/`¥` 只读（金额为 `¥` 前缀），比例输入框宽度自适应、`%` 紧贴数值右侧 |
...
| 态 | UI | 存储 |
|----|-----|------|
| 默认关 `false` | 左「提成」输入卡 + 右**虚线卡**「+ 顾客指定提成」（与提成卡**等比并排等高**，**无列头**）；点单位切换本卡 `nonDesignatedValueMode` | `nonDesignated` / `nonDesignatedAmt` + `nonDesignatedValueMode` |
| 开 `true` | **两列并排**，每列卡上方有**静态列头**「普通」/「顾客指定」（列头 UI 同工位列头：圆点 + 13px、**静态不可编辑、无改名图标**；「顾客指定」圆点用品牌红）——左列「提成」卡、右列「额外」卡（卡内标题「提成」「额外」）；各卡独立 `%`/`¥`（**宽卡**：值与胶囊同行、行内右对齐胶囊切换）；「额外」卡卡头右侧 **×** 收起 | 提成 / 顾客指定各值 + 各自 `*ValueMode` |
...
| 点虚线卡「+ 顾客指定提成」 | 全局 `extraSplit=true`：第 2 行变为「额外」卡（卡头右侧 **×**）；**默认 0**、mode 取**首个工位**（叠加值，不与提成同值） | 新增 rule 级 `extraValue` / `extraAmt` / `extraValueMode` |
| 开 `true` | 3 工位横排一行（每卡：取值 + **换行**右对齐胶囊）+ 第 2 行「额外」卡（宽同单工位）；**每卡胶囊**可独立切 `%`/`¥`，恒贴卡片右下角、尺寸不变、不溢出卡片 | 逐工位提成 + rule 级全局顾客指定 |
...
> **不分工位态复用同款列头**：`pickMode=avg` + `extraSplit=true` 时，参数区为**两列并排**，每列卡上方为同款**静态列头**——「普通」（圆点用默认色）与「顾客指定」（圆点用品牌红）；**列头静态、不可编辑、无改名图标**。`pickMode=avg` + 未开顾客指定时**不加列头**（提成卡 + 虚线卡并排）。

| 控件 | 行为 |
|------|------|
| 工位名行 | 工位名（**13px**，与卡内标题同号）+ edit-02（**16px**）可点 → 行内改名；maxlength **6**；位于本工位卡上方 |
| 顾客指定行（第 2 行） | 标签恒为「顾客指定」（同字号，带工位同款圆点，**静态、不可改名**、无 edit-02）；内容为「额外」参数卡或「+ 顾客指定提成」虚线卡（宽 = 单个工位卡、左对齐），见 §6.3 |
| 不分工位列头（`avg` + 开顾客指定） | 两列各一行「普通」/「顾客指定」**静态小标题**（圆点 + 13px，同工位名行样式；**不可点、无 edit-02**） |
| 新增 / 删除 | **无**（固定大/中/小工） |
...
**入口行 / 摘要行**：同前。摘要文案：工位 + 顾客指定 → `大工 · 顾客指定`（工位名不带其他后缀）；仅工位 → **仅工位名**（如 `大工`）；不分工位 + 顾客指定 → `顾客指定`；不分工位 + 顾客指定 且未勾选顾客指定 → `普通`；**皆否（不分工位 + 未开顾客指定）** → 卡片第三行仍为**灰色头衔**（无红色提成摘要），入口摘要行不写 meta。

**底部 Sheet**：提示文案随四态切换：
- 皆否：`可多选员工；点卡片即完成选择。`
- 仅开顾客指定：`可多选员工；点卡片后勾选「普通」或「顾客指定」（顾客指定提成另计）。`
- 仅按工位：`可多选员工；点卡片后勾选工位（大工/中工/小工），工位单选。`
- 按工位 + 开顾客指定：`可多选员工；点卡片后勾选工位（大工/中工/小工），工位单选；「顾客指定」可与任一工位同时勾选，顾客指定提成叠加。`
...
| 点已勾选的「普通」/「顾客指定」 | **取消该勾**：「顾客指定」→ 退回「普通」（不分工位）；「普通」→ **取消该员工**选择 |
| 点已勾选的工位 | **不取消**：该卡播 **iOS 抖动动效**提醒（工位必选，**不出 toast**）；换工位请点另一张工位卡 |
| 工位多张 | 工位之间**单选**（勾新的即替换旧的；旧勾**先反序收回**再画新勾） |
| 「顾客指定」 | **可与任一工位同时勾选**（互不替代）；可再点取消该勾 |
| 工位未点即收起 | 员工**不计入已选**；勾选记忆保留（再次展开仍是勾选态、卡片保持展开等工位） |
| 收缩后的员工卡 | 状态：已勾选 → 摘要行显示结果（`大工 · 顾客指定`）；右侧**放大勾选控件**（约 20px），**点它取消该员工选择**；**无**右上角 × |

**交互四态**：

| 按工位 | 开顾客指定 | 点员工卡后（展开的勾选卡） |
|:---:|:---:|------|
| ✓ | ✓ | Morph 展开为**整行 4 张**：「大工」「中工」「小工」「顾客指定」（工位名按 `stationLabels` 映射；每张右上角未勾选控件）；工位**必选、必须手点**（不预选默认工位），再点已勾选工位只抖动不取消；顾客指定可叠加且可再点取消；工位点定后收缩，员工卡显示 `工位 · 顾客指定` |
| ✓ | ✗ | 展开 **3 张**工位卡；**必选、必须手点**；单选后收缩（员工卡显示工位名）；再点已勾选工位只抖动不取消 |
...
**导航/按钮**：提成设置、添加提成方案、添加规则项、保存、取消、确定、完成、下一步、全选、清空、重命名、复制方案、删除、改名、分配员工、已分配、未分配、已选 N 人、还有 N 人未分配任何提成方案、**基数、适用范围**、5 项默认、5 项默认 · N 条覆盖、项目、产品、会员卡、办卡、充卡、整类 · {组名}、修改记录、**关联页面、头衔空态 sheet、方案空态 sheet、选择服务员工**、**顾客指定、额外、普通、小工、中工、大工、不分工位、按工位、未选择、已选 N 人**、**设置、头衔管理、角色管理、选择头衔、创建员工、邀请员工、查看记录**。

**规则卡**：原价、实收、**固定**、提成、**顾客指定**（规则卡上的取值段标签；Sheet 参数卡内标题仍为「额外」）、大工、中工、小工、**基数**、**适用范围**、计算基数、分配模式、不分工位、按工位分配、按比例 %、固定金额 ¥、现金、卡付、团购、提成参数。

**Dialog/Sheet**：方案名称、重命名方案、请输入方案名称、确认删除该方案？、删除后不可恢复。、确认删除该规则项？、尚未保存、尚未保存，确定要返回吗？、留在本页、不保存、多方案规则、计算基数说明、分配模式说明、未分配任何提成方案、暂无在岗员工、**选择员工、提成、额外、+ 顾客指定提成、顾客指定、大工、中工、小工**、**比例 / 金额（两段式胶囊）**；开单侧选人演示（§6.7.1）：按工位+开顾客指定为 **大工 / 中工 / 小工 / 顾客指定**，不分工位+开顾客指定为 **普通 / 顾客指定**。
...
33a. **Given** 按工位分配且**已开顾客指定**，**When** 查看规则卡，**Then** 「顾客指定 +x」与**首工位（大工）同行**、间距 **16px**（例：`大工 12% │ 顾客指定 +5%`），其下仍竖排中工 / 小工；顾客指定为 `CatRule` 级全局单值，**不逐工位重复**，且为**叠加**（不替代工位提成）；规则卡上**不出现「额外」字样**。
...
56d. **Given** 未开顾客指定的规则（`pickMode=avg`），**When** 打开 Sheet，**Then** 左「提成」卡 + 右虚线卡「+ 顾客指定提成」等比并排（等高，灰虚线灰字）；确定后顾客指定值保持 0。
56e. **Given** 未开顾客指定（`pickMode=avg`），**When** 点虚线卡「+ 顾客指定提成」，**Then** 全局 `extraSplit=true`，右卡变「额外」参数卡（卡内标题仍为「额外」；默认 0、卡头右侧 ×）；再点 × 收起，`extraValue`/`extraAmt` 归零并回到虚线卡。
...
56h. **Given** 按工位分配且已开顾客指定，**When** 查看 Sheet，**Then** 3 工位横排一行 + 第 2 行「额外」参数卡（宽同单工位），共 **4 个参数**；**无**全局「按比例 / 固定金额」段控；每卡**两段式胶囊**「比例 / 金额」（卡窄 → **换到第 2 行右对齐**、尺寸不变）可点独立切换取值模式；卡内标题「提成」「额外」字色 `#929292`。
56h2. **Given** Sheet 内任一参数卡（含「提成」卡与「额外」卡），**When** 点胶囊由「比例」切到「金额」（或反向），**Then** 胶囊**位置不变**（窄卡仍在下行右端 / 宽卡仍在取值行右端）、**尺寸不变**且**不溢出卡片右/下边缘** —— 取值行占满可用的整行宽（金额输入框只吃该行剩余宽度、长数值仅在框内裁切），输入 6 位以上数值亦不把胶囊顶出卡外、不把胶囊压扁。
...
53s. **Given** 不分工位 + 开顾客指定，**When** 点击员工卡，**Then** 展开 **2 张**卡「普通 / 顾客指定」；勾选后收缩，摘要显示 `普通` 或 `顾客指定`；**When** 重新展开并再点已勾选的「顾客指定」，**Then** 退回「普通」（员工仍已选）；再点已勾选的「普通」，**Then** 取消该员工。
53t. **Given** 不分工位 + 未开顾客指定，**When** 点击员工卡，**Then** **点选即勾选**（无展开卡，勾选控件同款画出动效），员工卡第三行仍为**灰色头衔**（无红色摘要）。
53u. **Given** 底部 Sheet 打开，**When** 查看提示文案，**Then** 按四态分别为：皆否「可多选员工；点卡片即完成选择。」/ 仅顾客指定「…勾选「普通」或「顾客指定」（顾客指定提成另计）。」/ 仅按工位「…勾选工位（大工/中工/小工），工位单选。」/ 按工位 + 顾客指定「…工位单选；「顾客指定」可与任一工位同时勾选，顾客指定提成叠加。」
...
| 字段 | 内容 |
|------|------|
| 触发 | 点击已勾选的「顾客指定」/「普通」/ 收缩态勾选控件（工位**不可**此路取消 → A12） |
...
| 生效范围 | **全部反向路径**：① 切换工位时被取消的旧工位；② 取消「顾客指定」→ 退回「普通」/ 仅工位；③ 取消「普通」；④ 收缩态员工卡点右侧勾选控件取消选择 |
...
| 选项卡（已选「顾客指定」/「普通」） | 取消勾 → 落定 → 收起（工位例外 → A12 抖动，不落定） | 等（A7 播完 · 85ms） |
